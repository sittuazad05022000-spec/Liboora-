...
| **Status** | **`ACCEPTED` — 2026-08-15, by direct conferral of **Architecture Owner** authority by the human principal** (*"accept ADR-0039 and use its decided fee taxonomy"*). **The acceptance is SCOPED, and the scope is stated here rather than left to inference.** ✅ **What is accepted and now BINDING:** the four Product-Owner decisions in **§3** (`FEE-GAP-001` V1 refund scope), **§4** (`FEE-GAP-004` fee taxonomy), **§5** (`FEE-GAP-005` append-only correction) and **§6** (`FEE-GAP-006` due-date offset = 0 days). ✅ **§2 is accepted *as a report*, not as a determination** — it recorded a conflict and expressly declined to settle it; that conflict was subsequently **settled by `ACCEPTED` `ADR-0040` §1**, which **UPHELD `ADR-0035` §5.5b unchanged**. §2 is therefore accepted as an accurate record of a question that is **now closed elsewhere**, and **nothing in §2 binds** as a routing decision. ⚠ **§7 is SUPERSEDED BY MEASUREMENT and is retained as historical evidence — it must NOT be read as current.** §7.1 asserts that `FEE-FR-028`, `FEE-FR-042`, `FEE-BR-019` and `FEE-BR-007` each carry **`⛔ none`** acceptance criteria and concludes *"Stage 4 MUST NOT be run"*. That was **true when written against `PRD-008` v1.3** and is **false now**: `PRD-008` v1.5 §36.1 gives all four criteria (**L1882**, **L1896**, **L1931**, **L1919**), rows carrying *"none"* measure **0**, and **Stage 4 has been run and passed 6/6**. §7.2 prescribed the sequence *decide → specify in the PRD → re-measure → then Stage 4*, and **that sequence was followed**; §7 is the record of the step before it completed. The staleness was found and disclosed by **`ADR-0041` §2.5** before this acceptance, not discovered afterwards. **This acceptance corrects no byte of §7** — an accepted record is not retro-edited to look prescient. **`ADR-0035`, `ADR-0037` and `ADR-0040` are UNAFFECTED and UNCHANGED**; nothing here overrides an `ACCEPTED` ADR. *Superseded Status text retained verbatim:* **`PROPOSED`** — recording decisions **given by the human principal** acting as **Product Owner**, and **reporting one conflict rather than resolving it**. This ADR is **not self-accepted**: the four decisions it records are the principal's, not mine, and the fifth item is **escalated unchanged**. Where the instruction and an **`ACCEPTED`** ADR disagree, this ADR **reports the disagreement** — it does not pick a winner, because overriding an `ACCEPTED` Rank-governance decision is an **Architecture Owner** act. |
| **Date** | 2026-08-15 |
...
| Version | Date | Change |
|---|---|---|
| **v1.1** | 2026-08-15 | **`ACCEPTED` by direct conferral of Architecture Owner authority — with an explicitly SCOPED acceptance, and one section marked historical rather than repaired.** The impediment this ADR stated to its own acceptance in **§9** — *"§2 contains an unresolved **Architecture Owner** question"* — was **removed by `ACCEPTED` `ADR-0040`**, whose `Supersedes` row (**L11**) records that it *"**closes** `ADR-0039` §2's reported conflict by determining it, and **confirms** `ADR-0039` §2's refusal to determine it as having been correct at the time."* Ripeness was measured and recorded in **`ADR-0041` §2** *before* any conferral existed, and this row executes the conferral rather than assuming it. **Scope:** §3–§6 bind; §2 is accepted **as a report only**; **§7 is superseded by measurement** — its `⛔ none` table was measured against `PRD-008` **v1.3** and all four obligations now carry criteria, with Stage 4 run and passed 6/6, as `ADR-0041` §2.5 disclosed. **§7 is NOT edited**: an accepted record is preserved as written, and correcting it retroactively would destroy the evidence that the sequence §7.2 prescribed was actually followed. **Nothing else changed** — not one decision was added, widened, narrowed or reworded; **no** `FEE-*` identifier, enum, permission, configuration row, ledger account, endpoint, schema, signature algorithm, provider, `BC-*`, `E-*` edge or financial rule was created; **no** frozen document was touched; and **no** other `ACCEPTED` ADR (`ADR-0035`, `ADR-0037`, `ADR-0040`) was modified, demoted or reinterpreted. |
