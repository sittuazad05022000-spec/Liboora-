...
| `LCF-GAP-011`, `LCF-GAP-015`, `LCG-GAP-003`, `LCG-GAP-004`, `LCG-GAP-007`…`LCG-GAP-014` + others | OPEN — dispositions recorded across `ADR-0081`, `ADR-0082` and `ADR-0085`; predicates re-measured TRUE | ⛔ UNMET |
| ⚠ **`LCG-GAP-001` and `LCG-GAP-006` — CORRECTION to this ADR's own v1.0 text** | ⭐ **Both are CLOSED, not open**, by `Accepted` `ADR-0085` (§2.1 and §3, Product Owner and Architecture Owner, 2026-09-01), whose **Closes** field names them explicitly. `LCG-GAP-005` is closed by the same ADR and `LCG-GAP-002` by `ADR-0083` §6. ⚠ The v1.0 row also cited *"`ADR-0081` §3"* for them, and a `grep` of `ADR-0081` for either identifier returns **0** — the citation was to the wrong document. **This ADR overstated the blocker count and is corrected here rather than left standing.** | ✅ **CLOSED — not a Stage 7 blocker** |
...
| **v1.1** | 2026-09-01 | ⭐ **One in-scope repair added after the Stage 7 gate re-run: `DOCUMENTATION_BASELINE.md` §4's **Rank 2** cell was measurably STALE**, reading *“51 accepted of 64 files”* against an independently measured **77 / 63 / 13 / 1 / 0 / 0** (sum closes, unparsed bucket EMPTY). Repaired **in the same commit**, with the prior text retained verbatim. ⚠ **Why this was repaired while `D-3` was not** — the two were tested against the identical two limbs and separate on both: (a) this is a derived statement **this act changes the truth of** (accepting `ADR-0086` moves accepted 62→63 and files 76→77), whereas `PRD-020`'s header is falsified by records this act did not author; (b) §4 is **expressly named in this ADR's Amends clause** and §7 rule 3 obliges the declaration to match the repository in the same commit, whereas the subject PRD's body is named nowhere and its hash is anchored by 7 records. The precedent is exact: that cell's own prior re-derivation occurred *during* the `PRD-012a` Rank 3 admission. ⚠ Drift honestly apportioned — **twelve** of thirteen new files (`ADR-0065`, `ADR-0075`…`ADR-0085`) were added by earlier passes that did not move the cell; **only `ADR-0086` is mine**. ⚠ Also records that `ADR-0066`…`ADR-0074` are **nine numbers never used**, so the span reaches 86 while only 77 files exist. ⛔ **No gate result changed, no identifier minted, no waiver written, and nothing in §3.3, §6 or §8 of this ADR altered.** |
| **v1.2** | 2026-09-01 | ⚠⚠ **CORRECTION on re-measurement during the A4 scope conferral — this ADR's §8 was WRONG and is repaired, not defended.** The v1.0 row read *"`LCG-GAP-001`, `LCG-GAP-006`, `LCF-GAP-011` + 15 more — all OPEN (`ADR-0081` §3, predicates measured TRUE)"*. Three errors: (1) **`LCG-GAP-001` and `LCG-GAP-006` are CLOSED**, by `Accepted` `ADR-0085` §2.1 and §3, whose **Closes** field names both; (2) the citation *"`ADR-0081` §3"* is to the **wrong document** — a `grep` of `ADR-0081` for either identifier returns **0**; (3) the effect was to **OVERSTATE the Stage 7 blocker count**, which is the one direction of error a freeze-adjacent record must never take. ⭐ **Root cause: the row was written from memory of the blocker list instead of re-read from the Closes field of the ADR that closed them** — the derived-statement class (`GCP-01`/`07`/`11`/`15`) striking my own summary, the third instance recorded in this pass after the false `R-2` and the stale Rank 2 cell. ⛔ **The corrected §8 verdict is UNCHANGED: Stage 7 is still NOT conferred on `PRD-021A`** — its baseline §3 row is still measured **0**, which is sufficient on its own. Recorded in `ADR-0085` §13.2. **0 gaps closed by this correction; it only stops misreporting gaps others closed.** |
