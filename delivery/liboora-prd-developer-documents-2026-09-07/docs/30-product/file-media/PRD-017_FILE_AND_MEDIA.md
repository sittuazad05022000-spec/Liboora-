...
| **PRD** | `PRD-017` |
| **Module** | File & Media |
| **Version** | **v0.2** — incremented by `ACCEPTED` [`ADR-0056`](../../00-governance/adr/ADR-0056-file-media-v0.2-media-optimization.md) under the `PRD_LIFECYCLE.md` §4 amendment path (ADR **before** the change, then version, then changelog, then baseline). The freeze itself did **not** renumber (`ADR-0020` §4 item 2); this increment is the **amendment**, not the freeze. **Status remains `FROZEN`** — a frozen PRD is amended through a recorded act, never silently (L177). v0.1's text is preserved in full: **no identifier is renumbered, reworded, withdrawn or deleted** |
| **Status** | **`FROZEN`** — [`PRD_LIFECYCLE.md`](../../00-governance/prd-ecosystem/PRD_LIFECYCLE.md) **Stage 7 complete**, conferred 2026-08-20 by `ACCEPTED` [`ADR-0054`](../../00-governance/adr/ADR-0054-file-media-prd-v0.1-baseline.md). **This document still confers no status on itself** — the status is read off [`DOCUMENTATION_BASELINE.md`](../../00-governance/DOCUMENTATION_BASELINE.md) §3.3/§4 and [`PRD_REGISTRY.md`](../../00-governance/prd-ecosystem/PRD_REGISTRY.md) **L307**, and this cell merely records what those documents now say. ⛔ **`FROZEN` is not `VERIFIED`:** 0 of **96** acceptance criteria proven by a test, 0 of **40** `IMPL-1200`…`1239` tasks implemented, **7 of 7** required architecture tests missing, **15 of 16 `FIL-GAP-*` OPEN** — `FIL-GAP-012` is **partially closed**, its architecture half by `ADR-0055` and its implementation half still open |
...
| **Integration edge** | **`E-22`** — BC Map **v1.8** **L331**: *"`BC-01`, `BC-10`, **`BC-12`**, `BC-14` → `BC-29` File & Media \| `CF` \| Sync port \| Domain holds a `FileRef`, never bytes or a raw storage path"*. ⚠ **`BC-12` Messaging was admitted 2026-08-20** by `ACCEPTED` [`ADR-0055`](../../00-governance/adr/ADR-0055-e22-consumer-list-includes-bc-12.md) — **one consumer cell, no edge added**. `BC-11` and `BC-13` were **deliberately NOT admitted**: `BC-11` answers eligibility as a boolean and `BC-13` reaches this module **outbound** over `E-14`, so neither needs bytes or a `FileRef` |
| **Requirement prefix** | **`FIL-`** — selected by measurement, §0.4 |
| **Lifecycle stage** | **Stage 7 — Frozen.** Stage 1 evidence was gathered in the read-only audit recorded in §18.2; **no `PRD-017_STAGE1_DISCOVERY.md` artefact exists** and this document still does not claim one — **`FIL-GAP-008` remains OPEN and the freeze does not close it.** Stages 3, 4, 5, 6 and 7 are complete, each with its own record: **Stage 3** [`PRD-017_ARCHITECTURE_ALIGNMENT.md`](./PRD-017_ARCHITECTURE_ALIGNMENT.md) **✅ PASS — CONDITIONAL, 6 of 6**, re-measured against the current bytes by [`PRD-017_ARCHITECTURE_ALIGNMENT_SUPPLEMENT.md`](./PRD-017_ARCHITECTURE_ALIGNMENT_SUPPLEMENT.md) with the verdict **unchanged**; **Stage 4** [`PRD-017_STAGE4_REQUIREMENTS_REVIEW.md`](./PRD-017_STAGE4_REQUIREMENTS_REVIEW.md), 14 adversarial categories, 6 genuine defects fixed; **Stage 5** [`PRD-017_STAGE5_CONFERRAL.md`](./PRD-017_STAGE5_CONFERRAL.md) **CONFERRED**, two independent checkers both exit 0; **Stage 6** [`PRD-017_IMPLEMENTATION_TASKS.md`](../../40-implementation/file-media/PRD-017_IMPLEMENTATION_TASKS.md), `IMPL-1200`…`1239`; **Stage 7** [`PRD-017_STAGE7_FREEZE.md`](./PRD-017_STAGE7_FREEZE.md). ✅ **Stage 3's CONDITION IS NOW MET, architecturally.** The condition was that `E-22` must admit `BC-12` before Student-to-Student sharing is servable (`FIL-GAP-012`); `ACCEPTED` [`ADR-0055`](../../00-governance/adr/ADR-0055-e22-consumer-list-includes-bc-12.md) performed exactly that act on 2026-08-20 as **Architecture Owner** — the authority `ADR-0054` §3.2 correctly declined to assume. ⛔ **The condition is met in the authoritative record, NOT in code**: no `lib/` implementation of `FIL-FR-006` exists and `B-2` records all seven required architecture tests missing, so the rule is authorised and specified but **unenforced**. `FIL-GAP-012` is therefore **partially closed, not closed** |
...
| **Registry status** | **`FROZEN`** in [`PRD_REGISTRY.md`](../../00-governance/prd-ecosystem/PRD_REGISTRY.md) §4.3 **L307**, advanced from `PLANNED` 2026-08-20. **Still not changed by this document** — the register was written by the governance act `ADR-0054`, not by its subject; this cell reports the register rather than asserting it. `PRD-017` is the **fifth** PRD to leave `PLANNED` directly for `FROZEN` |
| **Rank** | **Rank 3**, conferred 2026-08-20 by [`DOCUMENTATION_BASELINE.md`](../../00-governance/DOCUMENTATION_BASELINE.md) §3.3 and §4 under `ACCEPTED` `ADR-0054` — **the thirteenth module baseline**, and the first of the two `Platform Services` contexts (BC Map §3.3 pairs `BC-29` with `BC-30` Offline Sync, which is `PLANNED`). **Conferred, not claimed here.** ⚠ **`GCP-20` is carried, not repaired:** §4 holds **12** Rank 3 rows against §3.3's **13**, a pre-existing arithmetic gap preserved deliberately rather than corrected as a side effect of this admission |
| **Baseline** | **`BASELINE-2026-08-20-C`** — re-issued by the v0.2 amendment (`DOCUMENTATION_BASELINE.md` §7 rule 4: the baseline identifier changes when a **Rank 1–3** document changes version), superseding `BASELINE-2026-08-20-B`. The **third** suffix advance within a single day. ⚠ **`ADR-0055`'s BC Map amendment did NOT trigger a re-issue** — the BC Map is **Rank 4** and rule 4 does not reach it, which is the same exemption `ADR-0016` §7 records |
| **Date** | 2026-08-20 |
...
> **Totals: 277 identifiers across 8 registers, of which 1 is declared EMPTY. 150 are obligation-bearing.**
> **No identifier is retired; none has ever been issued before.**
>
> ⚠ **Recomputed at v0.2**, not incremented by hand. v0.1 declared **232 / 132**; `ADR-0056` added **45** — 13
...
**16 gaps. 15 OPEN; 1 (`FIL-GAP-012`) PARTIALLY CLOSED — its architecture half by `ADR-0055`, its
implementation half still open. None of the remaining 15 is resolved by this document, and none may be resolved
by an implementation choice.**

> ⚠ **Corrected at v0.2.** This line read *"13 gaps. All OPEN."* A **blanket** openness claim is a claim about
> rows it may not know about, and `prd017_stage5.py` failed the run on exactly that basis once the table carried
> 16 rows. The count and the openness assertion are now both stated, so a future addition breaks the check
> instead of silently widening a claim.
...
| Version | Date | Change |
|---|---|---|
| **v0.2** | 2026-08-20 | **AMENDED — media optimization admitted to V1.** Authority: `ACCEPTED` [`ADR-0056`](../../00-governance/adr/ADR-0056-file-media-v0.2-media-optimization.md), written and accepted **before** any byte of this document changed, per `PRD_LIFECYCLE.md` §4 (ADR → version → changelog → baseline). Status stays **`FROZEN`**; baseline re-issued to **`BASELINE-2026-08-20-C`** because a Rank 3 document changed version. **45 identifiers added, every register extended contiguously, ZERO obligation-bearing or Class B identifiers renumbered, reworded, withdrawn or deleted** — verified by snapshotting all **232** v0.1 definition lines before the edit and re-matching them byte-for-byte afterwards, rather than by assertion. ⚠ **The measured result was 231 of 232 byte-identical, not 232**, and the exception is disclosed rather than rounded away: **`FIL-GAP-012`'s row was deliberately revised** to record that `ADR-0055` closed its architecture half. A `FIL-GAP-*` row is a **Class C finding**, not an obligation — recording that a gap moved is the register's entire purpose, and `ADR-0055` §5 authorises exactly this update — but the blanket phrase *"no existing identifier is reworded"* would have been **false as written**, so it is narrowed here to the registers it is true of. `ADR-0056` §8.2 is corrected to match. New: `FIL-FR-083`…`095` (13 — derivative inheritance, document-like vs photographic classification, document-aware profile, perceptual profile, adaptive selection from **measured** input properties, original-byte preservation, original-quality as a **consumed** entitlement, lossless-only document handling, serving variants, the `RECEIVED → VALIDATING → PROCESSING → READY | FAILED` lifecycle, idempotency and bounded retry, progress without leakage, and the stall timeout), `FIL-BR-018`…`019`, `FIL-INV-012`…`013`, `FIL-XC-023`, `FIL-AC-079`…`096` (18), `FIL-CFG-010`…`015` (6), `FIL-GAP-014`…`016`. Totals recomputed, **not incremented**: **277** identifiers / **150** Class A / **96** AC / **112 covered = 74.7%**. **Every one of the 18 new obligations carries a criterion (measured: missing = 0)**, so the uncovered set did not grow. ⛔ **VIDEO AND AUDIO OPTIMIZATION WAS REQUESTED AND REFUSED.** The product instruction asked for V1 adaptive video transcoding. `FIL-FR-004`, `FIL-FR-005` and `FIL-XC-016` forbid it and **no Rank 1–4 authority admits video** — `MASTER_PRD.md` contains **zero** occurrences of *"video"* or *"audio"* (measured), and the only mention anywhere is EA **L964** *"Videos (V3)"*. `MP-CON-08` precedence could not promote it as it promoted thumbnailing, because **precedence promotes from a higher-ranked document and there is nothing above V3**. The three requirements stand **unamended**, `FIL-XC-023` restates the prohibition explicitly so adaptive machinery cannot be misread as admitting video, and the request is recorded as `FIL-GAP-016` with its remedy. **A requirement that would have contradicted three frozen requirements in the same document was not written to make the register look complete.** ⚠ **Two values were owed and NOT invented**: no authority supplies a compression quality floor, a minimum document resolution or a processing timeout, so `FIL-CFG-010`/`011`/`015` are published with ranges and owners but no defaults, on `FIL-CFG-006`'s precedent — `FIL-GAP-014`. ⚠ **`FIL-FR-095` and `FIL-CFG-015` were added because writing the amendment EXPOSED A HOLE**: `FIL-FR-092` promises `READY` and `FAILED` are the only terminal states and `FIL-INV-013` makes an object servable only in `READY`, yet nothing bounded `PROCESSING`, so a stalled object would have been permanently unservable **and** never terminal. The gap was in v0.1's implied lifecycle and is closed here rather than deferred. ✅ **`FIL-GAP-012`'s architecture half is CLOSED** by `ACCEPTED` [`ADR-0055`](../../00-governance/adr/ADR-0055-e22-consumer-list-includes-bc-12.md), which admitted **`BC-12` only** to `E-22` (BC Map **v1.8**, **L331**) after re-deriving necessity **per context** from BC Map §8: `BC-12` owns the `Message` carrying the `FileRef` and needs the edge; `BC-11` answers eligibility as a boolean; `BC-13` reaches this module **outbound** over `E-14`. ⛔ **Its implementation half remains OPEN** and `IMPL-1230`…`1236` are unblocked **architecturally only** — they may not be marked done on that basis. **No ownership moved into `BC-29`**: `FIL-XC-019`/`020`/`022` are unamended, so eligibility stays with Social Graph, message semantics with Messaging and moderation with Trust & Safety. **0 `E-*` edges added, 0 aggregates minted, 0 events (`FIL-EVT-*` still DECLARED EMPTY), 0 tenancy rules relaxed, 0 authorization or audit obligations weakened, 0 Flutter code, 0 SQL.** ⚠ **`tool/module_dependencies.yaml` L242 was deliberately NOT edited** — its `domain/social` grant is module-grained while `E-22` is context-grained, so it now exceeds the context-level authorisation for `BC-11` and `BC-13`; widening the map to match would have admitted two contexts that **fail** the necessity test, so the residual asymmetry is disclosed as **`GCP-23`** instead of erased |
| **v0.1** | 2026-08-20 | **Stage 7 FREEZE conferred. The version is deliberately NOT incremented** — `ADR-0020` §4 item 2, *"Freeze confers status; it does not renumber"*; the fifth v0.x admission. Status **`DRAFT` → `FROZEN`**, registry **`PLANNED` → `FROZEN`** (L307), **Rank 3** conferred by `DOCUMENTATION_BASELINE.md` §3.3/§4 under **`BASELINE-2026-08-20-B`**, by `ACCEPTED` [`ADR-0054`](../../00-governance/adr/ADR-0054-file-media-prd-v0.1-baseline.md). **Only the six header metadata cells and this row changed — no requirement, register, count, criterion, gap or section body was touched**, which is why the identifier totals below are identical to those the Stage 5 conferral measured: **232** identifiers / **8** registers / **78** AC / **132** Class A / **94** covered = **71.2%**. ⛔ **`FROZEN` is not `VERIFIED`:** 0 of 78 criteria proven, 0 of 40 tasks implemented, 7 of 7 required architecture tests missing (`B-2`), **all 13 `FIL-GAP-*` OPEN**, and `IMPL-1230`…`1236` **BLOCKED** on `B-11`. ⛔ **Student-to-Student Text + File/Media Sharing remains in V1 scope and remains unservable** — BC Map **L331** does not list `BC-12` Messaging among `E-22`'s consumers. Stage 3 narrowed the gap by testing necessity **per context** (`BC-12` yes; `BC-11` no; `BC-13` no, `E-14` being outbound), `ADR-0016` is the exact one-cell precedent for the fix, and `ADR-0054` §3.2 **declines to exercise it** because BC Map amendment is an **Architecture Owner** act and the conferred authority was Governance Owner. **`FIL-GAP-012` is routed, not closed.** ⚠ **One finding of this pass is recorded here because it concerns this document's own history:** three of the four hashes cited by the gate records were measured **STALE**, so the convenient `PRD-016`/`PRD-023` no-supplement precedent was **falsified rather than inherited** and a [Stage 3 supplement](./PRD-017_ARCHITECTURE_ALIGNMENT_SUPPLEMENT.md) was written. The drift was measured as **`+2 FIL-AC-*` and nothing else** — `FIL-AC-077` and `FIL-AC-078` added, none removed, every Class A register identical in membership — which is why the six Stage 3 verdicts survived re-testing unchanged. **The stale citations are deliberately left stale**: rewriting a hash would make a record claim a review of bytes it never saw. ⚠ **This edit moves this document's hash again**, so every gate record's anchor is now historical; the disclosure is in [`PRD-017_STAGE7_FREEZE.md`](./PRD-017_STAGE7_FREEZE.md) §3, and the absence of any instrument to detect this class of staleness is **`GCP-22`** |
