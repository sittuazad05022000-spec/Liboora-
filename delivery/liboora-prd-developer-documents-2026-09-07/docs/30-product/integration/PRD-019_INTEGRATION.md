...
| **Identifier prefix** | **`ITG-*`** — **111 identifiers issued, unchanged at v0.4** (54 obligation-bearing + **54 `ITG-AC-*`** + 3 `ITG-GAP-*`), every range **contiguous from 001**. **Counted by script, not asserted** — see §5 |
...
| **`ITG-FR-017`** | Processed event identities **MUST** be retained for **exactly 30 days**, measured from the moment the identity is first recorded. The value is **FIXED for V1**: it is **not configurable**, **no operator or administrator setting exposes it**, and an implementation that retains a dedup identity **longer** than 30 days is as non-conformant as one that retains it for less. **`[LIB]`** — the same value, and the same derivation, published at v0.2: the **smallest** value that strictly exceeds both published provider windows, since retries continue for **24 hours** and a support-initiated replay may resend an event up to **15 days** old. Not a Razorpay-published value; **derived from two of them, and the derivation is stated so it can be checked**. ⚠ **Scope is the dedup identity ONLY** — an opaque `(tenant, x-razorpay-event-id)` pair. This rule says **nothing** about the retention of the **evidence** or **audit** records that `ITG-INV-004` names separately, and **nothing** about payment or financial-record retention, which is `BC-05`'s under `PRD-008` and is **neither set nor implied here** |
...
> by `ITG-FR-015`–`ITG-FR-018` — and the retention period in `ITG-FR-017` is the *closest true thing* to a window. **At v0.4 that window is CLOSED at both ends**: `ITG-FR-017` reads **exactly
> 30 days**, not *"at least"*, so the element `ADR-0040` **L165** assigns to this role is now
> specified as a **bounded** value rather than an open-ended one.
...
| **`ITG-AC-017`** | `ITG-FR-017` | A dedup record created at `T` is still present at **`T + 30 days`**, and a redelivery at `T + 29 days` is suppressed. **Updated at v0.4 — the criterion now tests BOTH ends of the bound**, because `ITG-FR-017` now specifies both: the record is **absent** at **`T + 31 days`**, and a redelivery at `T + 31 days` is **processed as new** rather than suppressed. ⚠ **The upper assertion is what makes the value falsifiable** — under the v0.3 *"at least"* wording an implementation that retained identities **forever** passed this criterion, and *a criterion that cannot fail is not a test*. **No retention-policy value is asserted beyond the dedup identity's own lifetime**, and the evidence and audit records of `ITG-INV-004` are **out of scope** |
...
| **Stage 4 — Requirements Review** | ⚠ **RE-RUN AT v0.4 — 6 of 6 checks PASS. The check 3 finding is CLOSED, not waived.** ⚠ **The v0.3 verdict is quoted rather than overwritten:** *"RUN AT v0.3 — 5 of 6 checks PASS, 1 check has a DEFERRED finding"*. What changed is **one word of `ITG-FR-017`** — *"at least"* became **"exactly"** — and **no number changed anywhere in this document**. The ceiling was **not invented**; it is the floor that v0.2 had already derived and published, and the derivation (*smallest value strictly exceeding a 24-hour retry window and a 15-day replay window*) determines **one** value, not a range. **`ITG-FR-017` is now FIXED and not configurable**, so check 3 is satisfied in the strongest available way: not by bounding a configurable, but by **establishing that there is no configurable to bound**. See §6.1 and §6.2. Both v0.2 reasons for not running it are gone: Stage 3 **has** now been run (row above), and `ITG-AC-*` is **no longer empty** — coverage is **54/54**, so there is something to review against. The six checks are measured in **§6.1**, mechanically where mechanically possible. ✅ **Check 3 — *"every configurable has a default and a range"* — now PASSES** (§6.2). ⛔ **What has NOT changed is the thing that matters most: the stage verdict is STILL NOT CONFERRED**: `PRD_LIFECYCLE.md` §6 assigns Stage 4 to the **requirements reviewer** — see the Stage 3 row for the same distinction |
...
| **3** | Every configurable has a default and a range | ✅ **PASS at v0.4** (was ⛔ **OPEN FINDING — DEFERRED** at v0.3) | **Three tunable values exist and all three are now bounded.** `ITG-FR-020` = **exactly 5 s** (provider-imposed, cited §4.9). `ITG-FR-004` = an **enumerated** two-value Test/Live separation whose invalid state fails validation (`ITG-AC-004`). **`ITG-FR-017` = exactly 30 days**, fixed and **not configurable** — see §6.2 for how it was closed and why that is not an invented ceiling |
...
#### 6.2 The check 3 finding — CLOSED at v0.4, and the closure is narrower than it looks

**The v0.3 finding is quoted verbatim before it is answered, because a resolution that paraphrases the problem
it solves cannot be audited:** *"`ITG-FR-017` publishes a retention FLOOR and no CEILING. It reads 'MUST be
retained for **at least** 30 days' … and the gate's failure text is 'unbounded configuration is a specification
hole'."* That finding was **correct**. What was wrong was the disposition.

| Field | Value |
|---|---|
| **Resolution** | **`ITG-FR-017` now reads *"exactly 30 days"*, FIXED for V1 and NOT configurable.** The requirement states, in its own text, that retaining an identity **longer** than 30 days is as non-conformant as retaining it for less |
| **Authority relied on** | `ACCEPTED` **`ADR-0040` §4 L165** assigns the **replay window** element to the **`BC-31` owner**, and §4.4's own closing note states that *"the retention period in `ITG-FR-017` is the **closest true thing** to a window"* — so this value **is** that element. Specifying it is the act `ADR-0040` asks this role to perform; **no new authority is claimed and no ADR is amended** |
| ⚠ **Why this is NOT an invented ceiling** | **Because no number was invented, and none was even written.** The ceiling is the **floor v0.2 had already derived and published**, and that derivation — *the smallest value strictly exceeding both published provider windows (a **24-hour** retry window and a **15-day** replay window)* — determines **exactly one** value. The refused fix at v0.3 was *"and at most **90** days"*, which would have introduced **a second number with no provenance**. Setting the ceiling **equal to the derived floor** introduces **none**. A numeric audit of §4.4 confirms it: the multiset of numeric literals is **identical before and after this change** |
| **Why check 3 is satisfied in the strongest available way** | The gate's failure mode is *"unbounded **configuration**"*. This resolution does not bound a configurable — it establishes that **there is no configurable to bound**. `ITG-FR-017` is a fixed contract value with **no operator setting, no admin control and no `CFG-*` identifier**, consistent with this document's four standing statements that it creates **no configuration identifier** |
| ⚠ **The v0.3 reasoning that was WRONG, disclosed rather than quietly dropped** | v0.3 justified the deferral by asserting that a dedup-retention ceiling *"interacts with **`ID-5`** (retention under legal basis), the Data Privacy runbook, and storage cost"*. **`ID-5` (BC Map L182) governs Account deletion (DSR), `Person` anonymisation, and `StudentRecord` financial and attendance history.** A dedup identity is **none of those**: it is an opaque `(tenant, x-razorpay-event-id)` pair carrying **no personal data, no financial amount and no student reference**. **The v0.3 deferral was therefore a category error** — it treated a transport key's lifetime as a data-retention-policy subject, and so routed to an owner who was never needed. *A deferral to the wrong owner is not caution; it is a stalled decision wearing caution's clothing.* |
| **What this closure does NOT do** | It sets **no** payment-record, financial-record, evidence-record or audit-record retention. `ITG-INV-004` names **three** distinct record types — dedup, evidence, audit — and **`ITG-FR-017` binds only the first**. Retention of financial records remains `BC-05`'s under `PRD-008`, **byte-unchanged and not consulted for a value**. No Data Privacy runbook, no `ID-5` reading and no storage policy is written, amended or implied |
| **Effect on `ITG-AC-017`** | **Updated, because leaving it would have left the gate green and the criterion toothless.** It now asserts the record is **absent at `T + 31 days`** in addition to present at `T + 30 days`. ⚠ **Under the v0.3 wording, an implementation that retained identities FOREVER satisfied the criterion** — the floor-only assertion could not fail on the unbounded case, and *a criterion that cannot fail is not a test*. This is the **only** acceptance criterion changed by this pass; the other **53 are byte-identical** |
| **`ITG-GAP-*` impact** | **NONE.** `ITG-GAP-001`, `ITG-GAP-002` and `ITG-GAP-003` are **all still OPEN** and were not touched, narrowed or closed. This change is about a retention bound; **none of the three concerns retention**, and closing an unrelated finding is not licence to tidy them away |

⚠ **The two other tunable values, recorded so the closure is not read as sweeping:** `ITG-FR-020`'s
...
(*shared secret across modes*) are specified to **fail validation** by `ITG-AC-004`. **All three tunable values
are now bounded**, and one of them — `ITG-FR-017` — is bounded by being **fixed**.

⚠ **What check 3 passing does NOT confer, stated because this is the likeliest misreading of §6.1:** the Stage 4
**verdict is still NOT CONFERRED**. Six of six checks now **measure** PASS, and `PRD_LIFECYCLE.md` §6 still
assigns the verdict to the **requirements reviewer**, a role this pass does not hold. **The last substantive
finding is closed; the procedural obstacle is untouched** — and it was always the larger of the two.
...
| — | **CONFER the Stage 4 verdict.** ⚠ **This item had TWO limbs at v0.3 and now has ONE.** The second limb — *"resolve the check 3 finding: `ITG-FR-017` publishes a retention floor with no ceiling"* — is ✅ **CLOSED at v0.4** by fixing the value at **exactly 30 days** under existing authority (`ADR-0040` **L165**; §6.2), with **no new number** and **no retention policy set**. **The remaining limb is untouched by that closure**: all six checks now measure PASS, but `PRD_LIFECYCLE.md` §6 assigns the Stage 4 **verdict** to the **requirements reviewer**, and this pass still does not hold that role. ⚠ **6 of 6 measured is not 6 of 6 conferred** — closing the last finding removes the *substantive* obstacle and leaves the *procedural* one exactly where it was | **Requirements reviewer** | ⛔ **OPEN — measurement complete, conferral outstanding** (§6, §6.1) |
...
| Version | Date | Change |
|---|---|---|
| **v0.4** | 2026-08-16 | **`ITG-FR-017` BECOMES A FIXED 30-DAY RETENTION, AND STAGE 4 CHECK 3 CLOSES — one word changed, and no number changed anywhere in this document.** *"Processed event identities **MUST** be retained for **at least** 30 days"* becomes *"…for **exactly 30 days**"*, **FIXED for V1 and NOT configurable**: no operator setting, no admin control, **no `CFG-*` identifier**, and an implementation that retains an identity **longer** than 30 days is now as non-conformant as one that retains it for less. ⚠ **THE ONE THING A READER MUST NOT CONCLUDE IS THAT A CEILING WAS INVENTED.** The refused fix at v0.3 was *"and at most **90** days"* — a **second number with no provenance**. What is written here sets the ceiling **equal to the floor v0.2 had already derived and published**, from a derivation that determines **exactly one** value: *the smallest value strictly exceeding both published provider windows*, a **24-hour** retry window and a **15-day** replay window. **A numeric audit confirms the multiset of numeric literals in §4.4 is IDENTICAL before and after.** **Authority: `ACCEPTED` `ADR-0040` §4 L165**, which assigns the **replay window** element to the **`BC-31` owner** — and §4.4's own note already stated that `ITG-FR-017`'s period **is** that element. **No ADR is amended, no new authority claimed, no provider value added.** ⚠ **A DEFECT IN THIS DOCUMENT'S OWN v0.3 REASONING IS DISCLOSED RATHER THAN QUIETLY DROPPED, and it is the substantive content of this entry.** v0.3 deferred the ceiling on the ground that it *"interacts with **`ID-5`** (retention under legal basis), the Data Privacy runbook, and storage cost"*. **`ID-5` (BC Map L182) governs Account deletion (DSR), `Person` anonymisation and `StudentRecord` financial and attendance history — and a dedup identity is none of those.** It is an opaque `(tenant, x-razorpay-event-id)` pair holding **no personal data, no financial amount and no student reference**. **The v0.3 deferral was a category error**: it treated a transport key's lifetime as a data-retention-policy subject and routed it to an owner who was never needed. *A deferral to the wrong owner is not caution; it is a stalled decision wearing caution's clothing.* The v0.3 finding text and its 5-of-6 verdict are **quoted verbatim** in §6.2 and the §6 Stage 4 row rather than overwritten. **`ITG-AC-017` IS THE ONLY ACCEPTANCE CRITERION CHANGED — the other 53 are byte-identical.** It now asserts the record is **absent at `T + 31 days`** as well as present at `T + 30 days`, ⚠ **because under the v0.3 floor-only wording an implementation that retained identities FOREVER satisfied it** — and *a criterion that cannot fail is not a test*. **Stage 4 is RE-RUN: 6 of 6 checks now measure PASS** (§6.1), check 3 satisfied in the strongest available way — **not by bounding a configurable but by establishing there is no configurable to bound**. ⛔ **THE STAGE 4 VERDICT IS STILL NOT CONFERRED, AND THAT IS THE POINT MOST AT RISK OF BEING MISREAD.** `PRD_LIFECYCLE.md` §6 assigns the verdict to the **requirements reviewer**, a role this pass does not hold; **6 of 6 measured is not 6 of 6 conferred**. Closing the last substantive finding removes the *substantive* obstacle and leaves the *procedural* one exactly where it stood — and it was always the larger of the two. **Stage 3 is unchanged at 6/6 measured, also NOT CONFERRED.** ⛔ **`ITG-GAP-001`, `ITG-GAP-002` AND `ITG-GAP-003` ARE ALL STILL OPEN** — none was touched, narrowed or closed; **none of the three concerns retention**, and closing an unrelated finding is not licence to tidy them away. **SCOPE OF THE RULE IS NARROWED EXPLICITLY, not left to inference:** `ITG-FR-017` binds **only** the dedup identity. `ITG-INV-004` names **three** record types — dedup, **evidence**, **audit** — and the latter two are **out of scope**; **payment and financial-record retention remain `BC-05`'s under `PRD-008`**, which is **byte-unchanged (`d24832f33e600518`), not frozen, and was not consulted for a value**. **No Data Privacy runbook, `ID-5` reading or storage policy is written, amended or implied.** **NOTHING WAS CREATED:** identifier count stays **111** (28+11+5+10 obligations + 54 `ITG-AC-*` + 3 `ITG-GAP-*`), context count stays **31**, and no `E-*` edge, port, permission, configuration identifier, API architecture, provider value or numbered `IMPL-nnn` was added. **No Dart source changed.** **NOT reviewed, NOT approved, NOT baselined, NOT frozen** — no `DOCUMENTATION_BASELINE.md` §3 row is written or requested, and Rank 3 precedence is still **not held**. |
