<!--
  ...
  v0.4 supersedes v0.3. v0.3 is retained on disk and in git history; it is NOT
  ...
  Lifecycle     : No stage advanced. The 7-stage finalisation and freeze happen
                  only after A1-A8 are all complete. A3-A8 do not exist.
  Identifiers   : Every LCF-* identifier in this document is PROPOSED and
                  UNRATIFIED. The LCF- prefix itself is not conferred; ratifying
                  it is a Governance Owner act (LCF-GAP-001).
  ...
  Reported but deliberately NOT changed (see section 34):
    R1  LCF-XC-017 excludes "comments, reactions, threads, mentions" while the
        body specifies comments and reactions. Reconciling it is a scope
        decision, not an integrity fix, so it is reported and left intact.
...
⚠ **The `LCF-` prefix used below is proposed, not conferred.** Every repository PRD carries a registered prefix (`MM-`, `SM-`, `FIL-`, `TSF-`, `CNF-`, `ENT-`, `LMD-`, `LCM-`). Ratifying `LCF-` is a **Governance Owner** act — `LCF-GAP-001`.
...
`LCF-XC-017` | Comments, reactions, threads, mentions | **Deferred.** A2 specifies the **post**; comment surfaces are A2-successor scope (`LCF-GAP-004`) |
...
15 | Tenant context | `BC-19`+, ambient via `E-18` | — | Conflict 5 — unresolved for `BC-15`
...
`Community Analytics (V3)` (EA L972–973) are out of scope (`LCF-XC-016`, `LCF-XC-017`).
...
`tenantField` | ⛔ **UNRESOLVED** | Conflict 5 / Conflict 6. Not specified here.
...
`kind` | Closed set | `LCF-BR-010`; not client-extensible
`createdAt` | Timestamp | —
...
`LCF-API-013` | Search within community | ⛔ **NOT PROVIDED** | Depends on the tenant-partitioned index class (`AR-3`), which cannot be specified until Conflict 5 resolves
...
⏸ `LCF-AC-035` | Cross-tenant isolation of community posts is asserted per query | **DEFERRED.** Depends on Conflict 5 (`LCF-ADR-005`). Also blocked because `integration_test/` does not exist in the repository.
...
--- | --- | --- | ---
`LCF-RSK-001` | Conflict 5 (`ID-2` vs library-private community) resolves against Option 1, invalidating the `communityId` model | **High** | §20.2 records Options 2 and 3 so the fallback path is pre-described. Owner: Architecture Owner.
`LCF-RSK-002` | Conflict 6 (L450 vs L488) resolves such that `BC-15` events cannot be emitted at all | **High** | §24 leaves the tenant field UNRESOLVED rather than assuming. Owner: Architecture Owner.
...
`LCF-RSK-014` | The `LCF-` prefix collides with a future ratified prefix | Low | `LCF-GAP-001`. The prefix is proposed, not ratified.
...
`ADR-INDEX.md` records 65 ADR files (52 accepted, 13 proposed) with `ADR-0066`…`ADR-0073`
unopened, so the next free number is `ADR-0066` — recorded here as an observation only.
...
`LCF-ADR-005` | Resolve `ID-2` vs library-private community scope | Architecture Owner | §20, `LCF-API-013`, `LCF-AC-035`
`LCF-ADR-006` | Resolve BC Map L450 (`tenantId` mandatory on every event) against L488 (`BC-11`→`BC-17` never hold one) | Architecture Owner | §24 tenant field
...
20 | Ratify the `LCF-` prefix in the identifier registry | `LCF-GAP-001`
...
10 | Does it mint an `ADR-` number? | ✅ No — `ADR-0066` recorded as an observation only
...
`LCF-RSK-*` | 18 | `001`–`018` | Dense
`LCF-GAP-*` | 11 | `001`, `004`–`013` | `001`, `004`–`011` defined in §33.2; `012`, `013` defined in §34 as reported-not-fixed defects. Positions `002`, `003` unused
`LCF-ADR-*` | 6 | `001`–`006` | Requirements only; **no ADR created**
`LCF-DEC-*` | 7 | `001`–`003`, `003a`, `004`–`006` | Includes `LCF-DEC-003a`
`LCF-DEP-*` | 5 | `001`–`005` | Dense

**Total: 316 identifiers across 16 registers.**

Every one of them is **PROPOSED and UNRATIFIED**. The `LCF-` prefix itself is unratified
(`LCF-GAP-001`).
...
--- | --- | --- | ---
`LCF-GAP-001` | The `LCF-` prefix is not registered. Every other PRD carries a conferred prefix (`MM-`, `SM-`, `FIL-`, `TSF-`, `CNF-`, `ENT-`, `LMD-`, `LCM-`); `LCF-` is proposed only | **Governance Owner** | §0, §30 (`LCF-RSK-014`), §32 item 20
`LCF-GAP-004` | No PRD exists for `BC-14` Content Sharing, so `SharedContentRef` has no ratified shape | Architecture Owner / `BC-14` PRD author | §4 (`LCF-XC-017`), §5.3 row 7, §30 (`LCF-RSK-010`), §32 item 9, `LCF-DEP-003`
`LCF-GAP-005` | A3 does not exist on disk, so the ranking seam is specified but unexercised | A3 author | §5.3 row 11, §16
...
v0.4 was authorised to fix **objectively verified integrity defects only**. The two items
below are **content/scope inconsistencies**, not integrity defects: resolving either one
requires deciding what the document should *say*, which is a Product Owner or Architecture
Owner act. They are reported here and left intact.

ID | Defect | Why it was not fixed | Owner
--- | --- | --- | ---
`LCF-GAP-012` | `LCF-XC-017` (§4) excludes "Comments, reactions, threads, mentions" as A2-successor scope, but §17, §22 and §24 of this same document specify `CommunityComment`, `CommunityReaction`, `LCF-API-008`–`011`, `LCF-EVT-005` and `LCF-EVT-006`. The exclusion and the body contradict each other | Reconciling it means either deleting the comment/reaction specification or narrowing the exclusion. Both are scope decisions. Deleting specified behaviour would violate "invent, remove, merge, split no requirement" | **Product Owner**
`LCF-GAP-013` | `LCF-FR-026` cites `LCF-XC-016` and `LCF-XC-017` as the exclusions covering `Moderation (V3)` and `Community Analytics (V3)`, but `LCF-XC-016` is rate-limit policy authorship and `LCF-XC-017` is comments/reactions. The V3 subjects are actually covered by `LCF-XC-006` (moderation) and `LCF-XC-029` (analytics) | Repointing the citation is plausible but not certain — the original intent is ambiguous, and guessing it would be a silent architectural decision | **Architecture Owner**

Neither defect affects any architectural determination in this document: `BC-15` ownership,
the `BC-14` attachment route, the V2 classification, the A1 dependency and all eight recorded
conflicts are unaffected either way.
...
5 ⭐ | `BC-15` is in the **Global** band and may never hold a `tenantId`, yet a library community is tenant-private | BC Map **L488**: `BC-11→17 Student Network \| Global. No tenantId. Keyed on PersonId. \| Must never receive a StudentRecordId or tenantId (rule ID-2)`. Also L486, and `LIBOORA_MODULE_DEPENDENCY_MATRIX.md` L354 (`X-05`) | **Option 1** — `BC-15` holds an opaque `communityId`; tenant reasoning stays in A1 on the existing `E-13` upward-only ACL pattern (BC Map L307). ⚠ Objection recorded in §20.2: a 1:1 `communityId` may be a `tenantId` renamed. Options 2 and 3 recorded as fallbacks | ⛔ **Architecture Owner** — `LCF-ADR-005` / `LCF-DEC-003`
6 ⭐ | `tenantId` is mandatory on **every** domain event, but `BC-15` may never hold one | BC Map **L450** vs BC Map **L488** | A rule stating which of L450 / L488 governs events emitted from the Global band. **Pre-existing** — arises for any `BC-11`…`BC-17` event, surfaced by this part, not created by it | ⛔ **Architecture Owner** — `LCF-ADR-006` / `LCF-DEC-003a`
...
`LCF-DEC-003` | Choose Option 1, 2 or 3 for tenant scoping (Conflict 5) | Architecture Owner | The data model's `communityId` semantics stay provisional
`LCF-DEC-003a` | Rule on L450 vs L488 for Global-band events (Conflict 6) | Architecture Owner | No `BC-15` event can be lawfully emitted
...
- It creates **no** ADR and mints **no** ADR number. `ADR-0066` is named only as the observed
  next free number in `ADR-INDEX.md`.
