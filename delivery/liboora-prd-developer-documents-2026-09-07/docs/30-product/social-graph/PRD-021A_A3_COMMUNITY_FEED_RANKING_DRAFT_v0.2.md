...
Supersedes    : A3 v0.1 (correction pass). v0.1 retained in history.
This revision : Stage-3 architecture-alignment integration (see §0.1).
...
--- | --- | --- | ---
`LCR-FR-*` | 102 | `001`–`101` | Dense, plus `065a` (Stage-3 integration)
...
`LCR-DEC-*` | 6 | `001`–`007` | `005` **RETIRED** at v0.2; requirements only; **no ADR created**
`LCR-RSK-*` | 11 | `001`–`011` | Dense
`LCR-FND-*` | 12 | `001`–`012` | §37 findings referred upstream — **not requirements, not decisions**

**Total: 230 live identifiers across 12 registers** — 218 requirement-class identifiers across 11 registers, plus 12 `LCR-FND-*` findings (§37), which are observations referred upstream and carry no normative force. **232 identifier rows are physically present**; the two-row difference is exactly the retired `LCR-GAP-005` and `LCR-DEC-005`, excluded from `Count` per the rule stated above.
...
`LCR-FND-012` | **A3 impact: none.** `LCR-RS-003` and `LCR-RS-004` remain `⏸ DEFERRED` under `LCR-GAP-003` / `LCR-DEC-002` exactly as in v0.1. F-2 **narrows** the D2 question by adding a second condition on it; it does not answer it.

---

*End of PRD-021A Part A3 v0.2 (DRAFT). Correction pass (v0.1) retained in full; Stage-3 integration applied. **9 gaps and 6 decision requirements** remain open and routed, not resolved. Two findings (§37) are referred upstream. v0.1 is preserved in repository history.*
