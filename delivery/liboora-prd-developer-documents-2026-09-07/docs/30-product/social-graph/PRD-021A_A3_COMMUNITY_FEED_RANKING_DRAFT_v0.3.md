...
Readiness     : NOT READY. Blockers remain open (§31, §32).
...
Predecessors  : A1 v0.2 (Community Foundation), A2 v0.5 (Community Feed & Content)
Supersedes    : A3 v0.2. v0.1 and v0.2 retained in history.
This revision : D2-P scope-resolution integration (see §0.2).
...
`LCR-RS-003` | **Helpfulness** | ⏸ **DEFERRED** | Comments/reactions now **ship** (`LCF-GAP-012` RESOLVED), but helpfulness needs a **specific reaction kind** meaning "helpful". A2 `LCF-BR-010` requires the kind set to be defined in §22; §22 declares only `Closed set` and enumerates **nothing** — A2 v0.5 `LCF-GAP-014`. → `LCR-GAP-003` (narrowed)
`LCR-RS-004` | **Legitimate engagement** | 🟡 **ELIGIBLE — substrate authoritative, weight not assigned** | ✅ Substrate exists and is published: A2 v0.5 **`LCF-FR-061`** carries "comment count, reaction summary" per post in the feed response; **`LCF-FR-084`** defines the summary as an aggregate count per kind; `LCF-EVT-005`/`006` emit the changes. Activation additionally requires a **weight**, which `LCR-FR-034` reserves to the activation owner. → `LCR-DEC-008`
...
`LCR-RS-004` Legitimate engagement | 🟡 ELIGIBLE | *not yet assigned* | ⛔ No — 0 (weight pending, `LCR-DEC-008`)
...
⛔ **Still blocked — on a narrower gap after the D2-P decision.** Comments and reactions now
**ship** (`LCF-GAP-012` RESOLVED), so the *surface* exists. What does not exist is a reaction
**kind** that means "helpful": A2 `LCF-BR-010` requires the kind set to be "defined in this
part's data model (§22)", yet §22 declares only `Closed set` and enumerates no member. A2 v0.5
records this as **`LCF-GAP-014`** (Product Owner). Selecting or naming a kind here would be
inventing a requirement inside `BC-15`'s aggregate.

`LCR-FR-044` | A3 SHALL NOT infer helpfulness from any proxy — including reaction volume, comment volume or any aggregate count. A helpful signal requires an **explicit** helpful indication. → `LCR-GAP-003`

### 14.3 Legitimate engagement — DEFERRED

Reserved intent: a small, capped contribution from validated community activity.

✅ **Substrate now authoritative — weight outstanding.** After the D2-P decision the engagement
surface is real and, critically, **already published in the shape A3 needs**:
...
counts A2 already publishes. The signal is marked **🟡 ELIGIBLE**, not ACTIVE, because
`LCR-FR-034` reserves the **weight value** to the activation owner and no weight has been
assigned. → **`LCR-DEC-008`**.
...
`LCR-BR-010` | *Reserved, binding on activation:* engagement SHALL be capped such that it can never independently determine ordering, SHALL exclude self-engagement, and SHALL be deduplicated per actor. Now that the substrate is authoritative, this cap is a **precondition of activation**, not a note for later.

`LCR-BR-010a` | On activation, engagement SHALL be derived **only** from the aggregate counts A2 publishes (`LCF-FR-061`, `LCF-FR-084`). A3 SHALL NOT enumerate reacting or commenting members, and SHALL NOT retain any per-actor engagement record.
...
`LCR-AC-022` | ⏸ Given an engagement-bearing post, … | **DEFERRED** — substrate now authoritative (A2 v0.5 `LCF-FR-061`/`084`); criterion remains unwritable until a weight is assigned (`LCR-DEC-008`)
...
`LCR-GAP-003` | ⭐ **NARROWED at v0.3.** The original gap — "do comments/reactions exist in this generation?" — is **ANSWERED** (they ship; `LCF-GAP-012` RESOLVED). What remains is strictly narrower: **which reaction kind denotes "helpful"**. A2 `LCF-BR-010` requires the kind set to be defined in A2 §22, but it is never enumerated — A2 v0.5 **`LCF-GAP-014`**. Affects `LCR-RS-003` only; `LCR-RS-004` no longer depends on this gap | **Product Owner** | §11 (`LCR-RS-003`), §14.2, §30
...
`LCR-DEC-008` | ⭐ **New at v0.3.** Assign the weight for `LCR-RS-004` Legitimate engagement and renormalise the active set to exactly `100` (`LCR-FR-033`, `LCR-FR-035`). Its substrate is authoritative; only the **value** is missing, and `LCR-FR-034` reserves values to the activation owner | **Product Owner** | `LCR-RS-004` activation, `LCR-AC-022`
...
Does it resolve a conflict it does not own? | ✅ No — 9 gaps and 7 decision requirements; the only one **resolved** (`LCR-DEC-002`) was decided by its named **Product Owner** upstream, not by this document
Does it invent a weight for a newly available signal? | ✅ No — `LCR-RS-004` is `ELIGIBLE`, weight withheld to `LCR-DEC-008` (`LCR-FR-034`)
...
`LCR-BR-*` | 16 | `001`–`015` | Dense, plus `010a` (v0.3 — aggregate-only engagement); `009`–`012`, `015` reserved-on-activation
`LCR-INV-*` | 11 | `001`–`011` | Dense
`LCR-RS-*` | 9 | `001`–`009` | 1 active · **1 eligible (`004`)** · 6 deferred · 1 withdrawn
...
`LCR-GAP-*` | 9 | `001`–`010` | `005` **RETIRED** at v0.2 (not a gap); the other 9 routed to named owners
`LCR-DEC-*` | 7 | `001`–`008` | `005` **RETIRED** at v0.2; `002` ✅ RESOLVED at v0.3 (retained, not retired); `008` new at v0.3; requirements only; **no ADR created**
`LCR-RSK-*` | 11 | `001`–`011` | Dense
`LCR-FND-*` | 13 | `001`–`013` | §37 findings referred upstream — **not requirements, not decisions**; `013` records F-2's post-decision status

**Total: 235 live identifiers across 12 registers** — 222 requirement-class identifiers across 11 registers, plus 13 `LCR-FND-*` findings (§37), which are observations referred upstream and carry no normative force. **237 identifier rows are physically present**; the two-row difference is exactly the retired `LCR-GAP-005` and `LCR-DEC-005`, excluded from `Count` per the rule stated above.

v0.3 added exactly **four** identifiers — `LCR-FR-028a`, `LCR-BR-010a`, `LCR-DEC-008` and (in §37.2) `LCR-FND-013` — and removed none. `LCR-DEC-002` is **RESOLVED but retained**: resolution is not retirement, so it keeps its position and stays inside `Count`.
...
*End of PRD-021A Part A3 v0.3 (DRAFT). Correction pass (v0.1) and Stage-3 integration (v0.2) retained in full; D2-P scope resolution integrated. **9 gaps and 7 decision requirements exist; 6 decisions remain open** — `LCR-DEC-002` was resolved upstream by its named Product Owner. `LCR-RS-004` is **ELIGIBLE** (substrate authoritative, weight withheld to `LCR-DEC-008`); `LCR-RS-003` remains **DEFERRED** on A2's new `LCF-GAP-014`. Two findings (§37) remain referred upstream. **Status: NOT READY — NOT FROZEN.** v0.1 and v0.2 are preserved in repository history.*
