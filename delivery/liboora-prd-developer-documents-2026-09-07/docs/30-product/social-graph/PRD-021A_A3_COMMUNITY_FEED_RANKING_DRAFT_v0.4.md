...
Predecessors  : A1 v0.2 (Community Foundation), A2 v0.6 (Community Feed & Content)
Supersedes    : A3 v0.3. v0.1, v0.2 and v0.3 retained in history.
This revision : Owner-ruling integration (see section 0.3). Four explicit rulings
                were received and are integrated: the reaction kind set, the
                helpfulness designation, the ranking weight split, and the
                community-scope tenancy determination.
...
`LCR-DEC-009` | ⭐ **New at v0.4 — recorded, not decided.** Assign a weight for `LCR-RS-003` Helpfulness and renormalise the active set to exactly `100` under `LCR-FR-035`. Its substrate is authoritative **and designated** (`HELPFUL`), so unlike every other deferred signal there is no remaining substrate barrier — only a value. A3 SHALL NOT carve this weight out of the ruled `70`/`30` split | **Product Owner** | `LCR-RS-003` activation; no AC exists yet
...
Does it resolve a conflict it does not own? | ✅ No — 9 gaps and 9 decision requirements. Four are now **resolved**, every one by its named owner upstream: `LCR-DEC-002` and `LCR-DEC-008` by the **Product Owner**, `LCR-GAP-003` by the **Product Owner**, and `LCR-GAP-009` by the **Architecture Owner** via `ACCEPTED` `ADR-0078`. This document records rulings; it issues none
...
Does it weight a designated-but-unruled signal? | ✅ No — `LCR-RS-003` is designated on `HELPFUL` yet carries **weight 0** as `ELIGIBLE`, because no weight was ruled (`LCR-FR-028a`); the outstanding value is routed as `LCR-DEC-009`
Does it claim the EA enumeration was performed? | ✅ No — section 37.2 records `LCR-FND-013` as **BLOCKED**, not closed: `ADR-0075` is accepted in principle but its execution is blocked (`ADR-0075` section 8)
...
`LCR-GAP-*` | 9 | `001`–`010` | `005` **RETIRED** at v0.2 (not a gap). Of the 9 counted: **2 RESOLVED at v0.4** (`003` by Product Owner ruling, `009` by `ACCEPTED` `ADR-0078`) and **7 remain open**, each routed to a named owner
`LCR-DEC-*` | **8** | `001`–`009` | `005` **RETIRED** at v0.2. Of the 8 counted: **2 RESOLVED** (`002` at v0.3, `008` at v0.4 — both retained, not retired) and **6 remain open**. `009` is new at v0.4 (helpfulness weight). Requirements only; **A3 creates no ADR and mints no ADR number**
...
*End of PRD-021A Part A3 v0.4 (DRAFT). Correction pass (v0.1), Stage-3 integration (v0.2) and D2-P scope resolution (v0.3) retained in full; four received owner rulings integrated at v0.4.*
