...
Predecessors  : A1 v0.2 (Community Foundation), A2 v0.7 (Community Feed & Content)
Supersedes    : A3 v0.4. v0.1, v0.2, v0.3 and v0.4 retained in history.
This revision : Governance-and-architecture integration (see section 0.4). Three
                ACCEPTED ADRs are integrated: ADR-0079 (the EA enumeration was
                EXECUTED by append-only section), ADR-0080 (the Governance Owner
                role is constituted, so LCR-GAP-001 has an author) and ADR-0081
                (LCR-DEC-009 is REFUSED as undecidable, not resolved).
                ⭐ NO weight is changed. NO identifier is minted.
...
`LCR-DEC-009` | ⛔ **OPEN at v0.5 — formally REFUSED as undecidable, and expressly NOT resolved** (`ACCEPTED` `ADR-0081` section 2). Assign a weight for `LCR-RS-003` Helpfulness and renormalise the active set to exactly `100` under `LCR-FR-035`. Its substrate is authoritative **and designated** (`HELPFUL`), so unlike every other deferred signal there is no remaining substrate barrier — only a value. A3 SHALL NOT carve this weight out of the ruled `70`/`30` split. ⭐ **Why it stays open after an integration pass that closed three other items:** `ADR-0081` ran six determinability probes and every one returned nothing — no `Accepted` ADR assigns a helpfulness weight; the Decision Record states at **L303–304** that *"§7.3 resolves which kind means helpful; it does **not** assign a weight"*; the Owner Decision Request states at **L365** that it *"Proposes no reaction kind, no weight … no value of any kind"*; and no Rank 1–5 document supplies one. Of the three conceivable resolutions, **two are prohibited** — publishing a number is invention, and carving it from the ruled `70`/`30` overwrites a decision the Product Owner **did** make. Refusal with the measurement recorded is therefore the only honest disposition | **Product Owner** — ⛔ still required | `LCR-RS-003` activation; no AC exists yet
