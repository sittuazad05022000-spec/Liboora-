...
| Architecture alignment | ✅ **CLEAN as of 2026-08-21, for the `E-22` question only.** `FIL-GAP-012` is closed in **both** halves — architecture by `ADR-0055` (BC Map **v1.8** L331 lists `BC-12`) and implementation by `ADR-0059` (a `files` port whose refusal of any non-listed context is held by a mutation-proven test), and blocker **`B-11` is RESOLVED IN FULL** (§4). **`B-2` is also RESOLVED** — all seven Matrix §10.3 architecture tests exist and pass; the suite measures **+230, 0 `[E]`**. *This cell read* ⛔ ***"NOT clean. `FIL-GAP-012` and blocker `B-11` OPEN"*** *and is corrected because it is a statement about the repository that the repository now contradicts.* ⛔ **"Clean" here means the architecture is lawful and enforced — it does NOT mean `PRD-017` is `VERIFIED`.** **0 of 96 `FIL-AC-*` are proven against real storage**, and `FIL-GAP-008`, `FIL-GAP-013` and `FIL-GAP-016` remain **OPEN by deliberate refusal**. ⚠ **The corresponding row inside the FROZEN PRD is deliberately NOT edited** — see §4 |
...
| **`FIL-GAP-012` implementation half** — ✅ **CLOSED 2026-08-21** by `ACCEPTED` [`ADR-0059`](../../00-governance/adr/ADR-0059-e22-consumer-list-enforced-in-code.md). Same shape as `B-7`/`ADR-0012`, in the manifest's own words at **L649**: the `files` port was **declared** and *"only the interfaces are missing"*. `FIL-FR-006` is now enforced by a port that **refuses** an unlisted context, pinned to BC Map **L331** by a map-parsing architecture test | ✅ `IMPL-1230`…`1236` are unblocked in **implementation** as well as architecture; `IMPL-1258`…`1260` implement the guard and its test. ⚠ **`GCP-23` is made harmless at the code boundary, NOT repaired** — manifest **L242** remains module-grained and was deliberately not edited. ⚠⚠ **THE FROZEN PRD'S OWN `FIL-GAP-012` ROW IS DELIBERATELY NOT EDITED, AND THE REASONING IS RECORDED HERE RATHER THAN ACTED ON.** `PRD-017` **L1731** still reads *"⛔ **Implementation half remains OPEN** — no `lib/` code implements `FIL-FR-006`'s consumer check and `B-2` records that all seven Matrix §10.3 architecture tests are missing"*, and **L1693** still reads *"15 OPEN; 1 … PARTIALLY CLOSED"*. **Both clauses are now false in fact**, and it was verified rather than assumed: `grep` returns `isPermittedConsumer` at `services.dart` **L273** and `e22ConsumerContexts` at `file_access.dart` **L79**, and `test/architecture/` holds **10 files** passing **+230** with the seven §10.3 tests among them. **The temptation was therefore to correct two demonstrably false sentences in a frozen document, and it was refused.** `PRD_LIFECYCLE.md` **L177-180** is unambiguous and answers this exact case in advance: *"**A frozen PRD is never silently modified.** Not for an obvious correction, and not for one that is certainly right… If a silent edit is acceptable when the editor is confident, the freeze protects nothing, **because every editor is confident**."* `PGA-01` and `PGA-02` are two confirmed defects left unfixed on that precedent. The authority to amend was also checked and is absent: **`ADR-0059` L11 declares `Amends: Nothing. No PRD, no BC Map, no matrix, no manifest, no EA`** and **§4 item 8** repeats *"No document is amended"*. An ADR cannot confer an authority it explicitly disclaims, and `ADR-0056`'s precedent shows the correct route — a **new** ADR written **before** the edit, then version, changelog, baseline, in that order. **A gap register row is a requirement-bearing statement, not a typo**, so lifecycle **L170-174** puts it on the ADR path rather than the "just fix it" path. ⚠ **The checker was consulted and does NOT ask for the edit**: `prd017_stage5.py` emits *"section 2M claims NO architecture alignment while `FIL-GAP-012` is OPEN"* **inside a `PASS` block** — it is verifying that two records agree, not demanding a status change, and **EXIT=0**. *Editing the PRD to satisfy a line of PASS output I had misread as a failure would have been the exact inversion of "never weaken a checker to obtain PASS" — changing the subject to match the instrument.* **Routed to the Product Owner / Architecture Owner as the amendment that closes `FIL-GAP-012` on the record; disclosed, not done** | `IMPL-1258` … `IMPL-1260` — **released** |
...
> statement about *lawfulness*, not about progress.
>
> ✅ **UPDATED 2026-08-21 — `FIL-GAP-012`'s implementation half is now CLOSED too**, by `ACCEPTED`
> [`ADR-0059`](../../00-governance/adr/ADR-0059-e22-consumer-list-enforced-in-code.md): the four-context list
> is transcribed once (`e22ConsumerContexts`, `file_access.dart` **L79**) and enforced by a port that refuses
> every other caller, proven by a suite in which **deleting either guard fails a test**. The sentence above
> reading *"`FIL-GAP-012`'s implementation half is still OPEN"* was true when written and is retained so the
> sequence stays legible. ⚠ **This still does not mark one task done.** `IMPL-1230`…`1236` are `Not started`.
> ⛔ **`B-2` is ALSO RESOLVED** — the seven §10.3 tests exist and pass (**+230, 0 `[E]`**); the claim below
> that they "remain missing" is superseded and kept only as the record of what blocked them. **What has NOT
> changed is the thing that matters for these seven tasks: 0 of 96 `FIL-AC-*` are proven against real
> storage**, so completion of these seven
