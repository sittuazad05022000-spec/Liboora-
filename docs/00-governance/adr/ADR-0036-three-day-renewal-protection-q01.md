...
| **new** — window boundary (inclusive vs following-day) | Whether the window starts on `endDate` or the day after | **Product Owner** | ✅ **CLOSED at v2.1 (2026-08-31) — `W₀ = D+1`**, the day **after** `endDate`; window = `D+1`, `D+2`, `D+3`; `endDate` itself is **outside** it. Ruled by the **Product Owner** (`PO-1`), recorded in **§8.2a**. *(Was ⛔ **STILL OPEN at v2.0** — *"the conferral said `3-day` and that does not choose a first day"* — from 2026-08-05 to 2026-08-31; §6.5's refusal to infer it was correct for that whole period and is **discharged, not overturned**.)* Still deliberately carries **no** `MM-GAP-*` identifier (§8.3): being answered is not a licence to number it |
