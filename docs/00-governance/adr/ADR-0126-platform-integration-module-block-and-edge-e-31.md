...
| **Status** | ⚠⚠ **Proposed** — 2026-09-09. ⛔ **NOT Accepted. NOT executed.** The migration in §7 is **recorded and deliberately NOT performed** |
| **Date** | 2026-09-09 |
...
| **Supersedes** | — |
| **Amends** | ⛔ **NOTHING YET.** As `Proposed`, this ADR amends no artefact. On acceptance it would amend `tool/module_dependencies.yaml` (Rank 4) **by adding one absent block** and `LIBOORA_BOUNDED_CONTEXT_MAP.md` (Rank 4) **by APPEND only** as a new **§19** — see §7 |
| **Amended by** | — |
...
| **Closes** | ⛔ **Nothing, while `Proposed`.** On acceptance: `IMPL-020` **Gap A** (for `platform/integration` only) and **Gap B** |
...
| **`E-31`** occurrences anywhere | **0** — free |
...
re-verified at execution**, not assumed.
...
## 7. Migration requirements — ⛔ **RECORDED, NOT EXECUTED**

⚠⚠ **Status is `Proposed`. Not one row below has been performed. Every target is byte-unchanged.**

| # | Change | Document | Rank | State |
|---|---|---|---|---|
| **M1** | Add the `platform/integration:` block of §2.1, at a placement whose citation cost is **re-measured immediately before the write** | `tool/module_dependencies.yaml` | **4** | ⛔ **NOT DONE** |
| **M2** | Append **§19** declaring `E-31` per §2.2; update the header `Context Count` cell (edges **29 → 30**) and the `Version` cell; add one changelog row | `LIBOORA_BOUNDED_CONTEXT_MAP.md` | **4** | ⛔ **NOT DONE** |
| **M3** | Register this ADR | `ADR-INDEX.md` | — | ✅ done as **`Proposed`** |
| **M4** | Record `IMPL-020` Gap A (narrow) + Gap B closed; record §5.1 as a **new** OPEN item | `IMPL-020`, `IMPLEMENTATION_BLOCKER_REGISTER.md` | Unranked | ⛔ **NOT DONE** |
