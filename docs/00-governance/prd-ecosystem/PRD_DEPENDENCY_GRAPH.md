...
| **D-14** | `PRD-008`/`PRD-020` | `PRD-019` `BC-31` Integration | `API` | ⚠ **`E-25` citation REMOVED 2026-08-15** — executing `ADR-0035` `D-3`, *authorised* by conferral of **Architecture Owner** authority on 2026-08-05 and deliberately left unexecuted there because *"a Rank 5 edit belongs in its own commit"*. **`E-25` is `BC-20` → `BC-31`** (BC Map **L334**) and is the **only** `BC-31` edge in §7; `PRD-008` owns `BC-05` and `PRD-020` owns `BC-13`, so **neither** may be a party to it. **`D-3` named only the `PRD-008` half; measurement shows the `PRD-020` half is mis-cited on the same evidence** (`BC-13` has exactly one edge, `E-14`, to `BC-11`/`BC-12`/`BC-14`/`BC-15`) — recorded rather than silently widened, because the authorisation covered the defect class and the extra instance is the *same* citation. **`BC-05`’s lawful route is the already-declared port `business.payment_intent`** (Dependency Matrix **L196**, `X-03` **L352**, `ADR-0012` **L86**, ratified as `ADR-0035` `D-1`) — **a port, not a numbered edge**; per `ADR-0033`, BC Map L292 *"governs edges … it does not state that every cross-context read must be an edge."* **`PRD-020`’s route is NOT decided here** and no port is asserted for it — that belongs to the **Architecture Owner** with the `BC-13` owner. **No edge was added, renamed or created; this removes a false citation and asserts no replacement edge.** | Both unwritten |
