...
| **AUTHENTICATION** | Auth **v3.0** / `ADR-0129` authoritative. ⛔ No V1 mobile OTP, ⛔ no password, ⛔ no email-as-identity, ⛔ no email recovery, ⛔ **no Auth v4.0**. ⭐ **v1.3 — `DEVELOPER_HANDOFF.md` and the 6 implementation documents were ALIGNED to Auth v3.0** at commit **`a6dd2b2`**, recorded by `Accepted` [`ADR-0136`](../adr/ADR-0136-b-6-disposition-recorded-implementation-documentation-aligned-stale-boundary-statement-corrected.md) (**`B-6` DISPOSED**). ⚠ **4 of `DEVELOPER_HANDOFF`'s 8 references were deliberately PRESERVED as correct** — a byte-exact BC Map **L127** quotation (**L61**), a live code defect (**L91**/**L93**/**L96**, `TASK-D10` Surface 3), `LIB-21.5` verbatim (**L191**) and a code-surface inventory (**L213**). ⭐ v2.0 citations are **retained for traceability**, annotated with the v3.0 successor. ⛔⛔ **An author MUST still prefer Auth v3.0 over any implementation document.** *(Prior text, correct until v1.3: "⚠ `DEVELOPER_HANDOFF.md` and 6 implementation documents are **stale** — prefer Auth v3.0".)* |
...
| ⭐⭐ **v1.3** | 2026-09-11 | ⭐⭐ **§7A.9's `AUTHENTICATION` ROW IS CORRECTED: the "stale documentation" clause was a statement of FACT that had become FALSE.** Applies `Accepted` [`ADR-0136`](../adr/ADR-0136-b-6-disposition-recorded-implementation-documentation-aligned-stale-boundary-statement-corrected.md), under a one-act **Governance Owner** conferral. ⭐ `DEVELOPER_HANDOFF.md` and the 6 `docs/40-implementation/` documents were **ALIGNED to Auth v3.0** at commit **`a6dd2b2`** (7 files, 13 insertions / 13 deletions, every edit strictly in place) — so ⭐ **`B-6` is DISPOSED**, yet this binding clause still told every future specification author they were *"stale"*. ⚠ **4 of `DEVELOPER_HANDOFF`'s 8 references were DELIBERATELY PRESERVED as correct** — a byte-exact BC Map **L127** quotation, a live code defect (`TASK-D10` Surface 3), `LIB-21.5` verbatim, and a code-surface inventory — and ⭐ **the prior text is retained verbatim in the cell**. ⛔⛔ **ONE ROW EDITED, STRICTLY IN PLACE — the §7A.9 table gains NO row, so ZERO of the 62 citations into this file shift; L403 (PAYMENT, cited by `ADR-0134`) and L407 (API) are byte-verified unchanged.** ⛔ No stage added, renamed, renumbered or re-gated; ⛔ **no entry (`E-1`…`E-4`) or exit (`X-1`…`X-6`) criterion altered**; ⛔ §1–§7, §7A.1–§7A.8, §7A.10, §8 and §9 **byte-unchanged**. ⛔⛔ **The prohibitions are UNWEAKENED** — no V1 mobile OTP, no password, no email-as-identity, no email recovery, no Auth v4.0 — and ⛔ *"an author MUST still prefer Auth v3.0 over any implementation document"* is **retained in force**. ⛔ **`Q-04` STILL OPEN, V1 schema freeze STILL BLOCKED, `API-GAP-001` STILL OPEN, Stage-6A STILL NOT ENTERABLE.** |
