...
| **Version** | ⭐⭐ **v2.2** *(advanced 2026-09-18 by the `PRD-005` v1.4 → v1.5 successor amendment under `Accepted` `ADR-0149` — `PRD_LIFECYCLE.md` §4 step 7. ⚠️ **Prior: v2.1**, which the changelog itself records as **deliberately not incremented** for two earlier passes; ⭐ that practice ends here and the header and changelog move together, ⛔ but the historical rows are **not** retro-numbered.)* |
...
`PRD-004` (v1.2, `ADR-0018`), ⭐ **`PRD-005` (v1.5, `ADR-0019` at v1.4, amended to v1.5 by `ADR-0149`)**, `PRD-007` (v1.0, `ADR-0020`) and **`PRD-006` (v1.9, `ADR-0034`)**.
...
| `PRD-005` | Membership Management | **`BC-02` Membership** | `[CORE]` | V1 | **`FROZEN`** — [`membership-management/PRD-MEMBERSHIP-MANAGEMENT.md`](../../30-product/membership-management/PRD-MEMBERSHIP-MANAGEMENT.md) ⭐⭐ **v1.5**, admitted by `ADR-0019` (`BASELINE-2026-08-04-D`) at v1.4 and ⭐ **amended v1.4 → v1.5 on 2026-09-18** by `Accepted` [`ADR-0149`](../adr/ADR-0149-membership-plan-and-configuration-change-request-workflow-authorization.md) under **`BASELINE-2026-09-18-A`** — a [`PRD_LIFECYCLE.md`](PRD_LIFECYCLE.md) **§4 successor amendment**, ⛔ **NOT a re-baseline and NOT a new PRD**. ⭐ **L67 barred the alternative in terms** (*"Does another PRD already own the context? **It is an amendment to that PRD, not a new one**"*), and **`ADR-0134` L75** fixes that a *"successor amendment"* **is** a version increment of the same document. ⛔⛔ **THE STATUS CELL IS UNCHANGED — `FROZEN` at Rank 3**: §4 is the governed route for amending a frozen PRD, and **L177** bars only **silent** modification. Adds the **membership change-request workflow** (§2.4a, §16.3a, §17.1a, §23.2); **333 → 366 identifiers**; ⛔ **`MM-EVT` still 7, `MM-PO` still 11, `MM-CFG` still 9 — every closed register preserved** | `PRD-006`, `PRD-007`, `PRD-008` |
