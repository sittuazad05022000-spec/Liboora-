...
| **Version** | ⭐⭐ **v2.3** *(advanced 2026-09-19 by the `PRD-005` v1.5 → v1.6 successor amendment under `Accepted` `ADR-0150` — `PRD_LIFECYCLE.md` §4 step 7. ⛔ **A version change, NOT a status change**: `FROZEN` count stays **14**. ⚠️ **Prior: v2.2.**)* ⭐ *(prior v2.2 note: advanced 2026-09-18 by the `PRD-005` v1.4 → v1.5 successor amendment under `Accepted` `ADR-0149` — `PRD_LIFECYCLE.md` §4 step 7. ⚠️ **Prior: v2.1**, which the changelog itself records as **deliberately not incremented** for two earlier passes; ⭐ that practice ends here and the header and changelog move together, ⛔ but the historical rows are **not** retro-numbered.)* |
...
`PRD-004` (v1.2, `ADR-0018`), ⭐ **`PRD-005` (v1.6, `ADR-0019` at v1.4, amended to v1.5 by `ADR-0149` and to v1.6 by `ADR-0150`)**, `PRD-007` (v1.0, `ADR-0020`) and **`PRD-006` (v1.9, `ADR-0034`)**.
...
| `PRD-005` | Membership Management | **`BC-02` Membership** | `[CORE]` | V1 | **`FROZEN`** — [`membership-management/PRD-MEMBERSHIP-MANAGEMENT.md`](../../30-product/membership-management/PRD-MEMBERSHIP-MANAGEMENT.md) ⭐⭐ **v1.6**, admitted by `ADR-0019` (`BASELINE-2026-08-04-D`) at v1.4 and ⭐ **amended v1.4 → v1.5 on 2026-09-18** by `Accepted` [`ADR-0149`](../adr/ADR-0149-membership-plan-and-configuration-change-request-workflow-authorization.md) under **`BASELINE-2026-09-18-A`** — a [`PRD_LIFECYCLE.md`](PRD_LIFECYCLE.md) **§4 successor amendment**, ⛔ **NOT a re-baseline and NOT a new PRD**. ⭐ **L67 barred the alternative in terms** (*"Does another PRD already own the context? **It is an amendment to that PRD, not a new one**"*), and **`ADR-0134` L75** fixes that a *"successor amendment"* **is** a version increment of the same document. ⛔⛔ **THE STATUS CELL IS UNCHANGED — `FROZEN` at Rank 3**: §4 is the governed route for amending a frozen PRD, and **L177** bars only **silent** modification. Adds the **membership change-request workflow** (§2.4a, §16.3a, §17.1a, §23.2); **333 → 366 identifiers**; ⛔ **`MM-EVT` still 7, `MM-PO` still 11, `MM-CFG` still 9 — every closed register preserved** | `PRD-006`, `PRD-007`, `PRD-008` |
...
| `PRD-015` | Search Indexing | **`BC-23`** | `[GENERIC]` | V1 | ⭐⭐ **`FROZEN`** — admitted to `DOCUMENTATION_BASELINE.md` §3.3 at **Rank 3** on 2026-09-05 by **`Accepted`** [`ADR-0106`](../adr/ADR-0106-governance-owner-conferred-for-prd-015-stage-7-admission-act-only.md), the **admitting ADR**, recorded **before** the baseline change per baseline §7 rule 1; entry condition 10 closed by **`Accepted`** [`ADR-0105`](../adr/ADR-0105-product-owner-names-the-owning-role-for-srch-gap-008.md) naming `SRCH-GAP-008`'s owner. ⚠ **The clause *"⛔ Stage 3 NOT conferred"* below was TRUE WHEN WRITTEN and is now superseded** — Stage 3 was conferred at **16/16**, Stage 4 at **6/6**, Stage 5 and Stage 6 gates PASS, Stage 7 at **11/11**; the clause is **retained rather than deleted**, per §8 rule 5's audit discipline. ⛔ **Stage 8 NOT entered — 0 of 78 acceptance criteria proven**; ⛔ `READY`, `IMPLEMENTING`, `VERIFIED` not conferred. *Prior status:* **`DRAFT`** — [`search/PRD-015_SEARCH_INDEXING.md`](../../30-product/search/PRD-015_SEARCH_INDEXING.md) **v0.1**, 2026-09-03; status corrected `PLANNED` → `DRAFT` on 2026-09-04 by **§16** under §8 rule 5 (*"If this register disagrees with a PRD, fix this register"*). ⛔ **Stage 3 NOT conferred** | Permission-aware, tenant-partitioned search |
...
| Version | Date | Change |
|---|---|---|
| ⭐⭐ **v2.2** | 2026-09-18 | ⭐⭐ **`PRD-005` AMENDED v1.4 → v1.5 — `PRD_LIFECYCLE.md` §4 step 7, the registry limb of the frozen-PRD amendment route.** Executed under `Accepted` [`ADR-0149`](../adr/ADR-0149-membership-plan-and-configuration-change-request-workflow-authorization.md), which authorizes the **membership plan and configuration change-request workflow**. ⛔⛔ **THIS IS A VERSION CHANGE, NOT A STATUS CHANGE — `PRD-005` WAS `FROZEN` BEFORE AND IS `FROZEN` AFTER**, so ⛔ **the `FROZEN` count stays at 14, the `DRAFT` count at 3, the `PLANNED` count at 6 and the registered total at 24**; ⭐ **every one of those was re-checked rather than assumed**, because this register's own history records the opposite error twice — `PRD-008` counted as `DRAFT` while `FROZEN`, and the *"None was ever written"* prose surviving three freezes (`GCP-08`). ⭐⭐ **A FROZEN PRD CAN BE AMENDED WITHOUT BEING THAWED, AND THAT IS THE ROUTE TAKEN:** `PRD_LIFECYCLE.md` **§4** prescribes seven steps for exactly this — ADR first, Accept, increment the version, changelog in the same commit, `DOCUMENTATION_BASELINE` in the same commit, `TRACEABILITY_MATRIX` if identifiers changed, then this register — and **L177**'s prohibition is on a frozen PRD being *"never **silently** modified"*, ⭐ which a seven-artifact governed route is the antithesis of. ⭐⭐ **A NEW PRD WAS BARRED, NOT MERELY DISFAVOURED:** **L67** asks *"Does another PRD already own the context?"* and answers *"**It is an amendment to that PRD, not a new one**"* — `PRD-005` owns `BC-02`, so ⛔ **no `PRD-005a`, no successor identity, and NO NEW REGISTRY ROW**; the register still holds **24** identities. ⚠️ **AN EARLIER FRAMING OF MINE IS CORRECTED RATHER THAN REPEATED:** the instrument choice was first put as *"in-place versus successor"*, but **`ADR-0134` L75** — *"a `PRD-008` **v1.8 successor amendment**, not a rewrite, not a re-baseline"* — establishes that a **successor amendment IS a version increment of the same document**; the two were never alternatives. ⭐ **What v1.5 adds:** §2.4a (lifecycle), §16.3a (authorization), §17.1a (auditability), §23.2 (acceptance) — all `a`-suffixed or appended, ⛔ **renumbering no existing section**. **333 → 366 identifiers** (`MM-FR` **119 → 143**, `MM-AC` **97 → 106**). ⛔⛔ **EVERY CLOSED REGISTER PRESERVED:** `MM-EVT` **7** (⛔ no eighth event — the workflow publishes none), `MM-PO` **11** (⛔ the closed protected-operation list is **not** extended; approval attaches to the existing `MM-PO-002`/`MM-PO-011`), `MM-CFG` **9**. ⚠️ **Coverage FELL 60.1% → 59.1%** and is published as measured, with the 12 unverified new obligations **named** rather than papered over. ⭐ **The commercial split is `A-2` and is DERIVED, not chosen** — `TR-2` Manager may approve **activate/deactivate**, `TR-1` Owner approval required for **edits** — on `MM-FR-019`'s frozen *"MUST NOT alter any price snapshot"* against `MM-FR-023`'s economically-significant fields, which ⛔ **exclude `isActive`**; ⛔ **no commercial field set was invented or inferred**, and `A-3`/`A-4` were rejected on **FROZEN `PRD-023` L956** (`TR-2` is *"a partial writer — the restriction is by **subject**, not by scope"*). ⚠️⚠️ **THREE OFFICES — Product Owner, Architecture Owner and Domain Owner (Library Domain) — EXERCISED BY ONE PRINCIPAL**, each a one-act conferral ⛔ **reverting on completion** (`ADR-0033` §7.1); ⛔ **no independent review, ARB quorum or external Security audit is claimed**. ⭐ The Domain Owner conferral is recorded **in the v1.5 header**, because a measurement of **91** conferral-bearing ADRs found **0** standalone conferral records — ⭐ this repository records a conferral **in the instrument that exercises it**, and inventing a new artifact class was declined. ⭐ **This document is navigational and administrative** — ⛔ it **confers nothing**; the baseline advance to **`BASELINE-2026-09-18-A`** is `DOCUMENTATION_BASELINE.md` §7 rule 4's act. ⚠️ **A PRE-EXISTING DEFECT IN THIS VERY TABLE IS DISCLOSED AND NOT ABSORBED:** the row below is labelled **v2.1** *"(deliberately NOT incremented — same pre-existing defect as the entry below)"*, so the header version and the changelog have been out of step; ⭐ **this row DOES increment to v2.2** and the header is moved with it in the same edit, ⛔ but the historical rows are **not** retro-numbered. ⛔ **0 lines of code**; ⛔ `PRD-004`, `PRD-006`, `PRD-007`, `PRD-023`, `MASTER_PRD`, the BC Map and `ADR-0132` byte-unchanged; ⛔ **`AUTH-7.22` still closed at ZERO enumerated permissions**. |
