...
| **BC-30** | **Offline Sync** | `[GENERIC]` | Platform Services | V1 | Owns the client mutation queue, replay and conflict detection — **the execution mechanism only**. ⛔ **SCOPED TO ATTENDANCE ONLY** (`ADR-0114`): the offline **business capability**, including the **conflict-resolution policy**, is owned by **`BC-03` Attendance** (`ATT-PO-011`), which `BC-30` executes over **`E-24`**. **`BC-03` is its only authorised consumer.** ⛔ **NOT** a platform-wide generic synchronisation mechanism; **NOT** authorised for fees/payments/ledger (`BC-05`, also barred by `ADR-0037`), membership, library operations or any other context. Any further consumer requires a new edge and its own ADR (§7 preamble). |
...
| E-21 | BC-01, BC-10, BC-19 | BC-23 Search Indexing | `PL` | Event | `*Created/Updated/Deleted` → index. Search never reads domain tables. `BC-19` added by [`ADR-0093`](../00-governance/adr/ADR-0093-e21-source-list-includes-bc-19.md) — it owns the tenant (library organisation) record (**L128**) that is indexed for public discovery per `AR-1` and `MASTER_PRD.md` **L171**, required by `PRD-021C` C2 `LSD-FR-001`…`LSD-FR-008`; the **contract** it publishes is `Library_PRD_v1.md` **§14A** per [`ADR-0094`](../00-governance/adr/ADR-0094-section-14a-is-the-v1-bc-23-discovery-contract.md), whose §2.1 rules the index class as **Platform Public Discovery Index** (`AR-3`). ⛔ **`BC-15` is deliberately NOT a source** — `ADR-0083` §4.5 **AO-9** refused community search |
| E-22 | BC-01, BC-10, BC-12, BC-14, BC-19, BC-25 | BC-29 File & Media | `CF` | Sync port | Domain holds a `FileRef`, never bytes or a raw storage path. `BC-10` added by [`ADR-0016`](../00-governance/adr/ADR-0016-e22-consumer-list-includes-bc-10.md) — required by `SID-4.35`. `BC-12` added by [`ADR-0055`](../00-governance/adr/ADR-0055-e22-consumer-list-includes-bc-12.md) — required by `FIL-FR-075`…`FIL-FR-082`, V1 student-to-student file sharing. **`BC-19` and `BC-25` added by [`ADR-0095`](../00-governance/adr/ADR-0095-e22-consumer-list-includes-bc-19-and-bc-25.md)** — necessity tested **separately** per context (§3.1, §3.2): `BC-19` holds the `FileRef` for **library gallery media** (it owns the library organisation record, **L128**; §14A.5 publishes Gallery; `AR-1` **L23** routes *"Gallery → `BC-29`"*), and `BC-25` holds the `FileRef` for **branding media** (**L134** already grants it *"branding values"* and *"secret references"*), required by `PRD-021C` C3 `LPP-FR-011` and `LPP-FR-014`…`LPP-FR-017`. ⛔ `BC-25` gains **no** gallery media and `BC-19` gains **no** branding values — the **L128**/**L134** split is unchanged. **`BC-11` and `BC-13` are deliberately NOT consumers** — `ADR-0055` §3 tested necessity per context: `BC-11` answers eligibility as a boolean and never holds a `FileRef`; `BC-13` reaches this context outbound by `E-14`. ⛔ `BC-23` and `BC-26` were tested and **NOT** admitted — `ADR-0095` §3.4 |
...
| Q-04 | Retention period for `StudentAttendance` after enrollment archival? | Legal + storage cost | ⭐⭐ **v1.16 — REFRAMED AND DISCHARGED FOR `MP-DEP-07` ONLY by `Accepted` [`ADR-0137`](../00-governance/adr/ADR-0137-q-04-reframed-mp-dep-07-discharged-legal-risk-opened-as-lr-01.md). ⛔⛔ THE LEGAL QUESTION IS NOT ANSWERED.** ⭐ The earlier determination — *"REQUIRES LEGAL COUNSEL … NOT resolvable by Product or Architecture"* ([`ADR-0133`](../00-governance/adr/ADR-0133-mp-dep-07-schema-freeze-questions-q01-q03-q06-resolved-q04-remains-open-counsel.md) §5, 2026-09-10) **was correct on its own terms and is NOT disturbed**; `ADR-0137` supersedes **only** §5.4's *"Required authority"* Legal-counsel limb, §6.1's two current-state rows and §6.2's current-state heading, **IN PART and IN EFFECT ONLY**, ⛔ **editing no byte of `ADR-0133`** (`ADR-INDEX` **L206**); ⭐ §5.4's **Architecture-Owner limb STANDS**. ⭐⭐ **Basis:** the named authority is ⛔ **constituted NOWHERE** ([`PRD_OWNERSHIP_MODEL`](../00-governance/prd-ecosystem/PRD_OWNERSHIP_MODEL.md) has **0** occurrences of *"counsel"* across its **nine** offices; `MP-NFR-10`'s *"SECURITY + DATA Governance"* is named at Rank 1 but constituted nowhere), and `MASTER_PRD` **L574** requires *"**Resolution** of open questions"* — ⛔ **not a legal determination**. So the question was **unreachable**, and was **reframed** to *"what may V1 safely do while the law is unresolved?"* — answered by **internal interim product policy** `RET-01`…`RET-13` under **Product Owner + Architecture Owner**. ⛔⛔ **NO RETENTION PERIOD, TTL, PURGE SCHEDULE, CRON, ARCHIVAL RULE OR DELETION PATH IS DECIDED, DEFAULTED OR IMPLIED.** ⚠⚠ **The "default 7 years financial, 2 years attendance" recommendation to the left REMAINS EXPRESSLY NOT ADOPTED** — blocker **7a** reads *"**Do not adopt** '7 years financial / 2 years attendance'"* and `ATT-GAP-005` rejects it as unratified. ⭐ A retention **disposition** is not a retention **period** (`ADR-0123`) — and a **reframing** is not a **resolution**. ⛔ **No legal opinion; no claim of compliance; no legal uncertainty resolved** — thirteen uncertainties `U-1`…`U-13` are **preserved** in `ADR-0137` §6 and carried by `LR-01`. ⛔ *"No automatic deletion"* holds **at the application layer only** — ⚠ `RET-13` discloses that managed-BaaS backup/PITR expiry is **vendor-set, unmeasured and NOT resolved** (`U-13`), and ⛔ it does **NOT** mean indefinite retention is legally permissible (`U-11`). ⭐⭐ **All unresolved legal risk is TRANSFERRED to standing legal-risk item `LR-01` in the Rank 7 [`LEGAL_RISK_REGISTER`](../00-governance/LEGAL_RISK_REGISTER.md), PERMANENTLY OPEN**; ⚠ mandatory review before DPDP §§7–17 commence **~2027-05-13** (**G.S.R. 843(E) cl. (c)** — eighteen months from 2025-11-13; ⛔ **a legal commencement date only, NOT a retention period**). ⭐ **`MP-DEP-07` and the V1 schema freeze are UNBLOCKED on this question**; ⛔ Stage 6A stays blocked **independently** by `API-GAP-001`, **unaltered by this act**. ⛔ Blocker **7a**, `ATT-GAP-005`/`014`/`016` and `AUD-GAP-001` **all remain OPEN and UNAMENDED**. ⭐ *Prior recommendation retained for the record:* *"Define with counsel; default 7 years financial, 2 years attendance"* — ⛔ **not adopted, then or now.** |
...
| Version | Date | Change |
|---|---|---|
| **v1.14** | 2026-09-09 | **ONE edge REGISTERED by APPEND as a new §20. No context, aggregate, event or invariant added, removed or altered — count remains 31 (23 in V1); edges 30 → 31.** ⭐⭐ **This is the M2 execution of `Accepted` [`ADR-0127`](../00-governance/adr/ADR-0127-r1-e-32-bc-22-to-bc-31-push-fcm.md)**, whose §1 approved **`E-32`** while expressly deferring its registration: *"the allocation is approved now, but **BC Map registration/mint is deliberately deferred to M2**."* ⭐ **`E-32` = `BC-22 Notification Delivery` → `BC-31 Integration` (`CF`, Sync port, V1, **Push/FCM egress only**)**, exactly and only as conferred — ⛔ **`ADR-0127` is NOT reopened, amended or superseded**. ⭐⭐ **This REGISTERS AN IDENTIFIER FOR AN ALREADY-AUTHORITATIVE CONTRACT AND CREATES NO CAPABILITY:** Rank-1 `MASTER_PRD` **L229** already records *"Push notifications | `BC-22` **via `BC-31`** Integration | Channel adapter | Firebase Cloud Messaging | Yes — FCM (V1)"*, and `PRD-010` **`NTF-FR-031`** requires Push to reach FCM **through `BC-31`** while **`NTF-INV-005`** keeps provider credentials in `BC-31`. The Rank-4 register was simply silent, which **L292** makes decisive — the `ADR-0016`/`ADR-0055`/`ADR-0095` defect class. ⭐ **Pattern `CF` + Sync port is copied from `E-25`** (`BC-20 Billing → BC-31`), the existing capability→integration edge; ⛔ **no new integration pattern**. ⛔⛔ **PUSH/FCM ONLY — this edge carries NO SMS and NO OTP.** The V1 OTP transport is **`E-33`** `BC-18 → BC-31` (**§19**, `ADR-0128`), and ⛔ **`E-33` is neither changed nor re-pointed here**. **FROZEN `AUTH-10.37`**, **`PRD-010` §5 N7** and **`NTF-FR-030`** are **preserved unweakened** — `BC-22` gains **no** SMS channel and V1 integrated channels remain **exactly** In-App and Push. ⛔⛔ **`E-27` and `E-31` UNTOUCHED:** `E-27` permanently vacant (`ADR-0033`; *"numbers are never reused"*); **`E-31` still reserved** for `PRD-021C` C8 §13's `BC-13 → BC-19` referral transport (`TSF-GAP-009`, blocking `IMPL-1424`) and **absent from this file**. ⚠⚠ **CITATION COST MEASURED BEFORE THE WRITE AND FULLY DISCHARGED:** the **§20 append shifts 0** citations and the **L9 cell edit adds no line**; this changelog row — inserted **newest-first**, preserving the established ordering — shifts exactly **one**: **L653 → L654**, cited by `PRD-021A_STAGE4_AC_REQUIREMENT_MAPPING_2026-09-01.md` **L524**. ⭐ **That citation is repaired to L654 in this same commit**; ⛔ **no stale citation is left behind**. ⚠ **`DOCUMENTATION_BASELINE.md` L136 is updated to v1.14 in this same commit**, discharging §7 **rule 3** — and ⭐ **also clearing the v1.13 omission**, which `ADR-0128` §8 had wrongly recorded as satisfied; that false row is corrected as a factual execution-state fix, ⛔ **without reopening `ADR-0128`'s architecture decision**. ⚠⚠ **TWO PRE-EXISTING DEFECTS ARE DELIBERATELY NOT REPAIRED HERE, and are recorded rather than silently carried:** **(D-a)** §15's prose at **L654** still reads *"28 edges"* — measured wrong since before `E-30`, and the very text `PRD-021A` cites, so repairing it would **re-break** that citation; **(D-b)** `REVIEW_14A.md` **L180** cites BC Map *line 124*, which is **blank** — verified blank before this act. Both are outside M2's conferred scope. ⛔⛔ **REGISTERS AN EDGE, NOT A CAPABILITY:** no provider, credential, template, code or configuration; **0** files under `lib/`, `test/` or `tool/`; the module manifest is **byte-unchanged**. ⛔ **`BLK-01`, `BLK-02`, `IMPL-020`, `TASK-D10`, `MP-DEP-03`, Gate 3, D3 and DLT/H1–H3 are ALL UNCHANGED.** **No baseline re-issue** — Rank 4, §7 rule 4; **`BASELINE-2026-09-03-A` stands**. |
| **v1.13** | 2026-09-09 | **ONE new edge by APPEND as a new §19. No context, aggregate, event or invariant added, removed or altered — count remains 31 (23 in V1); edges 29 → 30.** Applies `Accepted` [`ADR-0128`](../00-governance/adr/ADR-0128-r3-bc-18-to-bc-31-v1-otp-transport-edge-e-33.md) on the joint **Architecture Owner + Product Owner** conferral **AO-1**…**AO-4** and **PO-1**. ⭐⭐ **Declares `E-33` `BC-18 Identity & Access` → `BC-31 Integration` (`CF`, Sync port, V1) as the V1 OTP / possession-challenge transport.** ⭐ **This RATIFIES an existing route rather than creating one:** `tool/module_dependencies.yaml` **L437–439** has declared `platform/identity → platform/integration:sso_provider` as *"the single sanctioned bypass of `platform/communication`, **for possession challenge delivery only**"* since **`a2caa22`**, the commit that created this file — and Matrix grid **L137** already carries `R4 IDENTITY & ACCESS` → `INT` = **`◇`**. ⛔ **`X-03` is NOT weakened**: it bans **`domain → INTEGRATION`** (rank 8), and `BC-18` is `platform/identity` at **rank 4**, whose cell is `◇` — `X-03` never applied. ⭐⭐ **FOUR RANK-1/RANK-3 RULES ARE PRESERVED UNWEAKENED, each tested individually:** **`AUTH-3.5`** (satisfied — Authentication calls a **port**; `BC-31` implements transport, so *"MUST NOT implement message transport"* holds literally), **FROZEN `AUTH-10.37`** (*"the OTP … **MUST NOT** pass through any general notification path"*), **`PRD-010` §5 N7** (OTP SMS excluded from `BC-22`) and **`NTF-FR-030`** (*"V1 integrated channels are **exactly** In-App and Push"*). ⭐ **The prohibition is protective, not formal:** `MP-GBR-35` + `NTF-FR-039` would make quiet hours *"defer, never drop"* an OTP, and `AUTH-3.8` expires the challenge in **5 minutes** — the general path would break the authentication factor. ⛔⛔ **`E-27`, `E-31` and `E-32` are PROHIBITED AND UNTOUCHED:** `E-27` permanently vacant (`ADR-0033`; *"numbers are never reused"*); **`E-31` reserved** for `PRD-021C` C8 §13's `BC-13 → BC-19` referral transport (`TSF-GAP-009`, blocking `IMPL-1424`); **`E-32`** allocated to `BC-22 → BC-31` by `Accepted` **`ADR-0127`**, which ⛔ **is NOT reopened** — ⚠ and **`E-32` is deliberately NOT minted here**, because `ADR-0127` §1 defers its registration to M2 and minting it would widen a scoped conferral. **Neither `E-31` nor `E-32` appears anywhere in this file.** ⚠⚠ **CITATION COST MEASURED BEFORE THE WRITE AND FULLY DISCHARGED:** **22** distinct BC Map line-citations exist repo-wide (L100–L652); the **§19 append shifts 0** of them, the **L9 cell edit adds no line**, and this changelog row — inserted **newest-first**, preserving the established ordering — shifts exactly **one**: **L652 → L653**, cited by `PRD-021A_STAGE4_AC_REQUIREMENT_MAPPING_2026-09-01.md` **L524** for *"28 edges"*. ⭐ **That citation is repaired to L653 in this same commit** (the R2 repair necessitated by this amendment); ⛔ **no stale citation is left behind**. ⛔ **`sso_provider` is accepted AS-IS** — not renamed, no sibling port added, `tool/module_dependencies.yaml` **byte-unchanged**; the naming concern is recorded in `ADR-0128` §2.2 as a future consideration only. ⛔⛔ **Creates NO delivery path:** no SMS provider (D3), no DLT registration (H1–H3), no `AERR-*`, no credential, no code — **0** files under `lib/`, `test/` or `tool/`. ⛔ **`BLK-01`, `BLK-02`, `IMPL-020`, `TASK-D10`, `MP-DEP-03` and Gate 3 are all UNCHANGED** — ⭐ **a route is not a delivery path**. ⛔ **No frozen PRD and no Rank 1–3 requirement is altered**; the only other edits in this commit are the **Unranked** `IMPL-020` correction (its L63/§1.3 route claim, falsified by L437–439) and the authorised `PRD-021A` citation repair. **No baseline re-issue** — Rank 4, §7 rule 4; **`BASELINE-2026-09-03-A` stands**. |
| **v1.12** | 2026-09-07 | **ONE description cell re-scoped. No context, edge, aggregate, event or invariant added, removed or altered — count remains 31 (23 in V1), edges remain 29 with `E-27` permanently vacant.** Applies `Accepted` [`ADR-0114`](../00-governance/adr/ADR-0114-offline-sync-is-a-capability-of-bc-03-attendance.md) on the Product Owner's **Option B** and the Architecture Owner's **`Q-1b`**. **§3 `BC-30` cell (L139)** now records that `BC-30` owns **the execution mechanism only** — client mutation queue, replay, conflict detection — while the offline **business capability and the conflict-resolution policy** are owned by **`BC-03` Attendance**, executed over **`E-24`**, and that **`BC-30` is SCOPED TO ATTENDANCE ONLY** with `BC-03` as its **only authorised consumer**. ⭐ **This RATIFIES an existing separation rather than creating one:** FROZEN Rank-3 `PRD-006` v1.9 already states it at **`ATT-PO-011`** (*"This module **MUST** define the conflict-resolution policy… and **MUST NOT** implement the queue, the replay or the conflict detection itself"*), **`ATT-PO-006`**, **`ATT-AC-181`**, L235, L403 and §27.1 L2429 — and the module manifest and `attendance.dart` already implement it (`sync.enqueue` with **0** queue/replay/detector classes in `BC-03`). ⛔ **`PRD-006` and `PRD-008` are BYTE-UNCHANGED; 0 `ATT-*` or `FEE-*` identifiers touched.** ⛔ **`BC-30` is NOT retired** — retiring it would strip `ATT-PO-011`'s prohibition of its counterparty and leave the queue homeless in the one context expressly forbidden to hold it (`ADR-0114` §8.1); **`Q-1a` was rejected on that measured ground**, and `Q-6` is thereby **discharged, not deferred**. ⛔ **`E-24` is RETAINED UNCHANGED** (`Q-2`) — not withdrawn, re-pointed or re-typed. ⛔ **No `BC-05` → `BC-30` edge is created or implied**; `Accepted` `ADR-0037`'s bar on offline financial write/queue/ledger/sync stands untouched. ⚠ **CITATION COST MEASURED BEFORE THE EDIT:** the **L139 cell edit adds no line and shifts 0 of 2,651** BC Map line-citations; this changelog row inserts at the head of the newest-first table and shifts **18** live citations below it by exactly **+1** (L648, L649, L655, L709, L753, L844, L874, L899) — all re-verified after the write. ⚠ **An earlier assumption in the preparing analysis was FALSIFIED and is recorded:** the changelog is **not** the last section — §15, §16, §17 and §18 follow it — so an append here is **not** free, unlike the §15–§17 extension appends. ⛔ **`PRD-018` is NOT written and remains `PLANNED`**; `PRD_REGISTRY.md` is byte-unchanged because §2's closed nine-value vocabulary admits no terminal status. ⛔ **Confers no lifecycle stage and authorises no implementation:** `OfflineSyncEngine` is untouched — **0 lines** — and its known scaffold defects (in-memory queue, no transport, `SyncState.conflicted` assigned 0 times, 0 tests) are **left unrepaired on purpose**. **No baseline re-issue** — Rank 4, per §7 rule 4 and the `ADR-0016`/`ADR-0055`/`ADR-0083`/`ADR-0085`/`ADR-0096` precedents; the `DOCUMENTATION_BASELINE.md` §3.2 **version cell** is nonetheless updated in this same commit under §7 rule 3, disclosing the pre-existing v1.8→v1.11 drift it inherits. |
| **v1.11** | 2026-09-02 | **One new edge and one new event by APPEND as a new §17, plus TWO one-cell §7.3 amendments. No context added, removed or reclassified — count remains 31 (23 in V1). No aggregate, member or invariant changed; §8 and §15.5 byte-unchanged. §9.1's Event Delivery Contract unvaried.** Executes five `PRD-021C` owner decisions conferred directly by the human principal. **(1) §7.3 `E-21` source cell** `BC-01, BC-10` -> `BC-01, BC-10, BC-19`, applying `Accepted` [`ADR-0093`](../00-governance/adr/ADR-0093-e21-source-list-includes-bc-19.md) on `XPC-OD-001` option **A** — `BC-19` owns the tenant (library organisation) record (**L128**) that `AR-1` and `MASTER_PRD.md` **L171** project through `BC-23`, required by `PRD-021C` C2 `LSD-FR-001`…`LSD-FR-008`. This is the **third** instance of the `ADR-0016` defect class — a PRD depending on an edge participant the Rank 4 register omits, which **L292** rules *"does not exist"*. ⛔ `BC-15` is **still NOT** a source: `ADR-0083` §4.5 **AO-9** refused community search and that refusal stands. **(2) §7.3 `E-22` consumer cell** `BC-01, BC-10, BC-12, BC-14` -> `BC-01, BC-10, BC-12, BC-14, BC-19, BC-25`, applying `Accepted` [`ADR-0095`](../00-governance/adr/ADR-0095-e22-consumer-list-includes-bc-19-and-bc-25.md) on `XPC-OD-003` option **C** — the **third** amendment to this cell after `ADR-0016` (`BC-10`) and `ADR-0055` (`BC-12`), and the **first** to admit two contexts, so necessity was tested **separately per context** exactly as `ADR-0055` §3 did: `BC-19` holds the `FileRef` for **library gallery media** (§14A.5 publishes Gallery; `AR-1` **L23** routes *"Gallery -> `BC-29`"*), `BC-25` holds it for **branding media** (**L134** already grants it *"branding values"* and *"secret references"*, the same reference shape). ⛔ **`BC-11` and `BC-13` remain deliberately NOT consumers**, and `BC-23`/`BC-26` were tested and **NOT** admitted (`ADR-0095` §3.4). ⛔ `BC-25` gains **no** gallery media and `BC-19` gains **no** branding values — the **L128**/**L134** split is unchanged, and **`BC-29` remains the sole media infrastructure owner** (**L138**): a `FileRef` is a reference, never bytes and never a raw storage path, and ⛔ **no duplicate media infrastructure is created** — the amendment *avoids* new infrastructure by reusing the existing edge. **(3) New §17** applies `Accepted` [`ADR-0096`](../00-governance/adr/ADR-0096-profile-view-fact-produced-by-bc-19-over-new-edge-e-30.md) on `XPC-OD-007A` option **C**, declaring **`E-30`** `BC-19 Tenancy` -> `BC-26 Analytics Read Model` (`PL`, Event, **V1**) carrying the new event **`tenancy.LibraryProfileViewed`**, which unblocks `PRD-021C` C3 §9A's owner-only Profile Views readout. ⭐ **The producer was DETERMINED, not assumed:** `BC-19` owns the entity that was viewed (**L128**), already publishes `tenancy.*` events (**L435**) so no new capability is conferred, and owns the `Tenant` aggregate (**L381**) as §9's convention requires. `BC-18` was rejected because the public profile is readable **without authentication** and `BC-18` is not in the path; `BC-26` itself was rejected by its own invariant *"no projection is a system of record"* (**L385**); C3 was rejected as *"NOT a bounded context"*. ⛔ **`BC-24 -> BC-26` was NOT used** — measured, `BC-24` is the source of **0** edges and its `AuditEntry` requires an *"actor of record"* (**L384**) that an anonymous view does not have. ⛔ **`E-26` was NOT reused as telemetry** — wrong producer (`BC-27`), wrong direction (AI *reads from* `BC-26`), wrong mechanism (Sync port). ⛔ **`E-27` IS NOT REUSED** — withdrawn by [`ADR-0033`](../00-governance/adr/ADR-0033-e27-core-cluster-edge-allowlist.md) and `PRD_LIFECYCLE.md` §5 rule 5 reads *"Numbers are never reused, even after withdrawal"*; with `E-28`/`E-29` taken by `ADR-0083`, **`E-30` is the next lawful number** and the set becomes **`E-01`…`E-26`, `E-28`, `E-29`, `E-30` = 29 edges with `E-27` permanently vacant**. ⭐ **The payload boundary IS the privacy control** (§17.2.1): the event carries only the subject library and the occurrence time, and ⛔ **no viewer identity in any form** — no `PersonId`, `AccountId`, device identifier or fingerprint — so `ID-2`, `ID-3` and `X-05` are **preserved, not weakened**, and no context gains an exemption. **(4)** `Accepted` [`ADR-0097`](../00-governance/adr/ADR-0097-profileviews-certified-uniqueviewers-not-certified.md) on `XPC-OD-007B` option **B** certifies **`ProfileViews` ONLY** as the **first `CertifiedMetric` instance named anywhere in the repository** — the class existed at **L385** and no instance had ever been defined — fixing its counting rule, duplicate handling, bot handling, trend period, privacy/suppression rules and authoritative definition under `BC-26`'s existing authority. ⛔ **`UniqueViewers` is NOT certified** and **MUST NOT** be rendered as zero, unknown, a placeholder, *"coming soon"*, disabled, blurred, teased or estimated — ⭐ it is not *computable* from a fact that identifies no one, so the asymmetry is **consistency with §17.2.1, not an omission**. ⛔ **No BC Map cell is amended by `ADR-0097`** — defining a metric *exercises* **L135**/**L385** rather than extending them — and ⛔ **`PRD-009` is NOT created**, remaining `PLANNED` at `PRD_REGISTRY.md` **L246**. **(5)** `Accepted` [`ADR-0094`](../00-governance/adr/ADR-0094-section-14a-is-the-v1-bc-23-discovery-contract.md) on `XPC-OD-002` option **A** establishes `Library_PRD_v1.md` **§14A** as the V1 `BC-23` discovery/query contract and rules the `AR-3` index class as **Platform Public Discovery Index**; it is **declaratory and amends nothing**, and ⛔ **`PRD-015` is NOT authored**, remaining `PLANNED` at **L315**. ⭐ **BC-26 remains the SOLE analytics authority** (**L135**, **L385**) and **`BC-19` gains no analytics capability whatsoever** — it states a fact and cannot count, aggregate, project, define or report; ⛔ **no second search, media or analytics system exists after this version**. ⭐ **Why APPEND for §17, stated as measurement:** §15's preamble measured that inserting a row after `E-26` shifts **1,293** line-citations into this file, *"several inside **FROZEN** Rank 3 documents"*, against **0** for an append — the method [`ADR-0079`](../00-governance/adr/ADR-0079-ea-v2.3-capability-enumeration-addenda.md) §8.5 Option A established and §15/§16 both reused. The two `E-21`/`E-22` cell edits are **in-place because they are cell edits**: they add no row and shift no line, exactly as `ADR-0016` and `ADR-0055` did to this same table. ⚠ **TWO residues are DISCLOSED, NOT resolved.** (1) §7.5's derived statistics (**L352**–**L356**, *"Published Language (`PL`) | 7"*) are now stale — the true totals are **29 edges** and **8** `PL` edges, stated in **§17.3** and ⛔ **deliberately NOT edited** in §7, because silently editing a §7 cell to tidy a derived count would break the byte-identity guarantee that makes the append method safe; §15 left the same cells stale when it minted `E-28`/`E-29`. (2) ⚠⚠ **`GAP-BCMAP-BC26-EDGES`** — measured, `BC-26` appears as a *"Primary consumer"* in ~**20** §9 event rows (e.g. **L405**) **while §7 declares no edge into `BC-26` other than `E-26`**. ⭐ `ADR-0096` §1.2 **refused to use that anomaly as its authority** — it would have resolved a pre-existing unrelated inconsistency as a side effect of a `PRD-021C` decision, and would have left the new fact resting on an *absence* rather than a *declaration*. It is **routed to the Architecture Owner as a separate question**; **§17.4** records it. ⛔ `SM-7.17`'s ruling that `StudentViewed` is *"an audit concern, not a domain event"* is **distinguished on its facts, not overruled** — a staff member viewing a student has a named actor **and** a named data subject; a seeker viewing a public library profile has neither — and `Student_Management_PRD_v1.md` is **byte-unchanged**. ⛔ `BC-15` remains the source of **zero** edges and `LCG-ADR-002` stays **OPEN**. ⛔ `XPC-OD-004`, `XPC-OD-005` and `XPC-OD-006` remain **OPEN**; `XPC-CONF-013` (**Privacy Owner VACANT**) is **not** resolved and ⛔ **no Privacy Owner, Security or ARB review is asserted**. **No baseline re-issue** — Rank 4, per `DOCUMENTATION_BASELINE.md` §7 rule 4 and the `ADR-0016`, `ADR-0055`, `ADR-0083`, `ADR-0085` precedents. ⛔ **Confers no lifecycle stage: `PRD-021C` and every part remain DRAFT at Stage 2, NOT FROZEN, NOT APPROVED, NOT BASELINED; Stage 3 is NOT entered and no alignment record exists.** ⛔ **No `lib/` source file touched; no `IMPL-*` minted** |
...
## 17. ⭐ `ADR-0096` Extension — the profile-view fact, edge `E-30` (V1)

> ⚠⚠ **THIS SECTION IS A NORMATIVE EXTENSION OF §7 AND §9, NOT A COMMENTARY ON THEM.**
> Admitted 2026-09-02 by **`Accepted`** [`ADR-0096`](../00-governance/adr/ADR-0096-profile-view-fact-produced-by-bc-19-over-new-edge-e-30.md)
> under **Architecture Owner** authority, discharging `PRD-021C` decision **`XPC-OD-007A`** (option **C**).
> §7's rule at **L292** — *"If an edge is not in this table, it does not exist and adding it requires an
> ADR"* — is satisfied by this section: the ADR exists, and the row below **is** that table, extended.
>
> ⭐ **Why this is an APPEND and not rows inserted into §7.3 and §9.** This is the method
> [`ADR-0079`](../00-governance/adr/ADR-0079-ea-v2.3-capability-enumeration-addenda.md) §8.5 Option A
> established and that **§15** and **§16** both reused. §15's preamble measured the cost of the alternative:
> inserting a row after `E-26` would have shifted **1,293** line-citations into this file, *"several inside
> **FROZEN** Rank 3 documents"*, where appending shifts **zero**. **§1–§16 are unchanged by this section**
> apart from the header `Version` cell, the header `Context Count` cell, one changelog row, and the two
> §7.3 cells amended by the separate `ADR-0093` and `ADR-0095`.

### 17.1 New integration edge

⛔ **`E-27` IS NOT REUSED AND REMAINS PERMANENTLY VACANT.** It was minted by `ADR-0032`, withdrawn by
[`ADR-0033`](../00-governance/adr/ADR-0033-e27-core-cluster-edge-allowlist.md) in v1.7, and
`PRD_LIFECYCLE.md` §5 rule 5 states *"Numbers are never reused, even after withdrawal."* `E-28` and `E-29`
were taken by `ADR-0083` (**§15.1**). The next lawful number is therefore **`E-30`**, and the edge set
becomes **`E-01`…`E-26`, `E-28`, `E-29`, `E-30`** — **29 edges, with `E-27` a deliberate gap that records a
withdrawal**. A reader who finds no `E-27` has found the correct state.

| # | Upstream (provider) | Downstream (consumer) | Pattern | Mechanism | Release | Contract |
|---|---|---|---|---|---|---|
| **E-30** | BC-19 Tenancy | BC-26 Analytics Read Model | `PL` | Event | **V1** | Carries **`tenancy.LibraryProfileViewed`** (§17.2) — the **fact** that a library's public profile was rendered. `BC-19` is the producer because it *"Owns the tenant (library organisation) record"* (**L128**) that the public profile renders, already publishes `tenancy.*` events (**L435**), and owns the `Tenant` aggregate (**L381**). ⛔ **`BC-19` gains NO analytics capability** — it states a fact and cannot count, aggregate, project, define or report. `BC-26` remains the **sole** owner of the metric/semantic layer (**L135**, **L385**). Delivery is **not** on the synchronous path of the public profile render, following `E-20`'s discipline. Admitted by `ADR-0096` §2.2 |

**Direction.** Inbound to `BC-26`. ⭐ `BC-19` is already the source of an edge-carried event surface
(**L435**), so this row confers **no new producer capability** on it; it names one additional fact.

**Pattern.** `PL` + Event is the **existing** shape for a fact travelling into a capability context —
`E-20` (`→ BC-24`), `E-21` (`→ BC-23`) and `E-23` (`→ BC-22`) are all `PL` + Event. ⭐ **No new integration
pattern is introduced.**

**On §7's "in V1" heading.** **L292** scopes §7 to V1, and both endpoints here are **V1** contexts
(**L128**, **L135**). The row is V1-scoped and raises no V2 question.

### 17.2 New event — a normative extension of §9

§9's naming convention is **binding**: *"`<Context>.<Aggregate><PastTenseVerb>`… Events are facts about the
past. An event named as an imperative (`SendReminder`) is a command… and must not enter the event bus as a
domain event."* The name below is **derived** from that rule, not chosen.

| Producer | Event | Primary consumers | Purpose |
|---|---|---|---|
| BC-19 | `tenancy.LibraryProfileViewed` | BC-26 | The fact that a library's public profile was rendered. Carried by **`E-30`**. ⛔ Carries **no** count, total, trend or comparison — those are `BC-26` derivations |

⛔ **No other event is added, renamed, re-purposed or removed.** `tenancy.TenantProvisioned` and
`tenancy.TenantSuspended` (**L435**) are byte-unchanged.

#### 17.2.1 ⛔⛔ Payload boundary — the privacy control is the payload

| MUST carry | Why |
|---|---|
| The **library/tenant** whose profile was viewed | The subject of the fact. `BC-19` owns this identifier (**L381**) |
| The **occurrence time** | So `BC-26` can bucket into the periods `ADR-0097` defines |
...
- ⛔ **No `StudentRecordId`** and nothing from which one is recoverable — `ID-2` (**L489**) is **preserved,
  not weakened**, and **no** context gains an exemption.
- ⛔ **No social signal** — which friends, followers, group members or peers viewed (`X-05`).
- ⛔ **No count, total, trend, percentage, ranking or comparison** — `BC-26`'s to derive.
...
cannot re-identify a viewer whose identity was never transmitted. This is also why
[`ADR-0097`](../00-governance/adr/ADR-0097-profileviews-certified-uniqueviewers-not-certified.md) certifies
`ProfileViews` and **declines** `UniqueViewers`: uniqueness is not computable from a fact that identifies
no one, and making it computable would require re-introducing exactly what this boundary excludes. ⭐ **The
two decisions are consistent by construction, not by coincidence.**

#### 17.2.2 Tenancy and the two `AR-3` index classes

- `BC-19` sits in the **`BC-19→31` capability tier**: *"Tenant-aware. Carry and propagate `tenantId`, own no
  tenant data of record… Indices, caches, projections… are **all** tenant-partitioned"* (**L490**).
  `BC-26`'s projection of this event **inherits** that rule; ⛔ **no exemption is created.**
- The **public profile read** is served on the `AR-3` **Platform Public Discovery Index** path with **no
  caller tenant context** (§11.1). The event names the **subject** library, which is *published* information.
- The **metric readout** is `AR-3` **Tenant Operational Data**: a read **MUST** carry tenant context and
  **MUST** be refused where it is absent or does not match. ⛔ **`MP-GBR-08`, `SE-1` and `X-13` are
  untouched**, and the metric is **never** placed on the discovery-index path.

**Delivery.** `E-30` is governed by **§9.1**'s Event Delivery Contract exactly as written; ⛔ **no property
of §9.1 is varied, relaxed or extended.**

### 17.3 ⚠ Derived §7 statistics are stale, and are deliberately NOT edited

§7.5's pattern-distribution table (**L352**–**L356**) counts *"Published Language (`PL`) | 7"* and a total
consistent with 26 edges. **`E-30` makes eight `PL` edges and 29 edges in total.**

⛔ **Those cells are deliberately NOT edited**, exactly as §15 left them unedited when `E-28` and `E-29`
were minted. They are §7 statistics, and §15/§16 both declare §1–§14 byte-identical; silently editing a §7
cell to keep a derived count tidy would break the byte-identity guarantee that makes the append method safe.
⭐ **The correct totals are stated here so no reader is misled:** **29 edges**, `E-27` vacant; **8** `PL`
edges.

### 17.4 ⚠⚠ One pre-existing anomaly is DISCLOSED and deliberately NOT resolved

**Measured:** `BC-26` appears as a *"Primary consumer"* in roughly **twenty** §9 event rows — for example
`enrollment.StudentEnrolled | BC-02, BC-23, BC-24, BC-26, BC-22` (**L405**) — **while §7 declares no edge
into `BC-26` other than `E-26`** (`BC-27 AI → BC-26, BC-23`, a *retrieval* port for AI's benefit).

⚠ **`ADR-0096` §1.2 refused to use this as its authority.** It would have been convenient to argue that a
twenty-first `BC-26` consumption needs no edge either. That argument was rejected because (1) it would
resolve a pre-existing, unrelated inconsistency as a side effect of a `PRD-021C` decision, and (2) it would
leave the new fact resting on an *absence* rather than a *declaration*.

⚠ The anomaly is therefore recorded as **`GAP-BCMAP-BC26-EDGES`** — *§9 records `BC-26` as a consumer of
~20 events for which §7 declares no edge* — **Architecture Owner**, and **routed as a separate question**.
⛔ **`E-30` covers ONLY the profile-view fact and resolves nothing else.**

### 17.5 What this section does **not** do

- ⛔ Does **not** add, remove, rename or reclassify a bounded context — **31 before and after, 23 in V1**.
- ⛔ Does **not** reuse `E-27`, which remains permanently vacant.
- ⛔ Does **not** alter `E-26`, or reuse it as a telemetry ingress.
- ⛔ Does **not** create a `BC-24 → BC-26` edge. `BC-24` is the source of **no** edge (measured: 0 rows), and
  its `AuditEntry` requires an *"actor of record"* (**L384**) that an anonymous view does not have.
- ⛔ Does **not** amend `Student_Management_PRD_v1.md`. `SM-7.17`'s ruling that `StudentViewed` is *"an audit
  concern, not a domain event"* is **distinguished on its facts, not overruled** — a staff member viewing a
  student has a named actor **and** a named data subject; a seeker viewing a public library profile has
  neither (`ADR-0096` §3.3).
- ⛔ Does **not** alter any aggregate, member or invariant — **§8 and §15.5 are byte-unchanged**.
- ⛔ Does **not** define, own, certify or version any **metric** — that is
  [`ADR-0097`](../00-governance/adr/ADR-0097-profileviews-certified-uniqueviewers-not-certified.md), and
  `BC-26` remains the sole metric/semantic authority (**L135**, **L385**).
- ⛔ Does **not** create a second analytics system, semantic layer, metric store, projection engine or read
  model, and does **not** make `BC-19` an analytics context.
- ⛔ Does **not** weaken `ID-1`, `ID-2`, `X-05`, `EV-1`…`EV-4`, `MP-GBR-08`, `SE-1` or `X-13`.
- ⛔ Does **not** expose individual viewer identities, view lists or individual view timestamps.
- ⛔ Does **not** make `BC-15` an event producer — `LCG-ADR-002` stays **OPEN**.
- ⛔ Does **not** vary §9.1's Event Delivery Contract.
- ⛔ Does **not** resolve `GAP-BCMAP-BC26-EDGES` (§17.4), `XPC-OD-004`, `XPC-OD-005` or `XPC-OD-006`.
- ⛔ Does **not** re-issue the baseline — this file is **Rank 4** (`DOCUMENTATION_BASELINE.md` §7 rule 4).
...
## 19. ⭐ `ADR-0128` Extension — the V1 OTP transport, edge `E-33` (V1)

> ⚠⚠ **THIS SECTION IS A NORMATIVE EXTENSION OF §7, NOT A COMMENTARY ON IT.**
> Admitted 2026-09-09 by **`Accepted`** [`ADR-0128`](../00-governance/adr/ADR-0128-r3-bc-18-to-bc-31-v1-otp-transport-edge-e-33.md)
> under a **joint Architecture Owner + Product Owner** one-act conferral (**AO-1**…**AO-4**, **PO-1**),
> resolving **R3**, the OTP-SMS transport-ownership question opened by `ADR-0126` §5.1.
> §7's rule at **L292** — *"If an edge is not in this table, it does not exist and adding it requires an
> ADR"* — is satisfied by this section: the ADR exists, and the row below **is** that table, extended.
>
> ⭐ **Why this is an APPEND and not a row inserted into §7.3.** The method
> [`ADR-0079`](../00-governance/adr/ADR-0079-ea-v2.3-capability-enumeration-addenda.md) §8.5 Option A
> established, reused by **§15**, **§16**, **§17** and **§18**. §15's preamble measured the alternative:
> inserting after `E-26` would shift **1,293** line-citations, *"several inside **FROZEN** Rank 3
> documents"*, where appending shifts **zero**. **§1–§18 are unchanged by this section** apart from the
> header `Version` cell, the header `Context Count` cell and one changelog row.

### 19.1 New integration edge

⛔ **`E-27` IS NOT REUSED AND REMAINS PERMANENTLY VACANT** (`ADR-0033`; `PRD_LIFECYCLE.md` §5 rule 5 —
*"Numbers are never reused, even after withdrawal"*). ⛔⛔ **`E-31` AND `E-32` ARE ALSO NOT USED HERE:**
**`E-31`** is **reserved** for `PRD-021C` C8 §13's `BC-13 → BC-19` referral transport (`TSF-GAP-009`,
blocking `IMPL-1424`), and **`E-32`** is **allocated to `BC-22 → BC-31` (Push/FCM) by `Accepted`**
[`ADR-0127`](../00-governance/adr/ADR-0127-r1-e-32-bc-22-to-bc-31-push-fcm.md), whose §1 **defers its
BC Map registration to M2** — so ⛔ **`E-32` is not minted in this file either**, and `ADR-0127` is
**not reopened**. The next lawful number is therefore **`E-33`**, verified **0** occurrences repo-wide
immediately before minting. The edge set becomes **`E-01`…`E-26`, `E-28`, `E-29`, `E-30`, `E-33`** —
**30 edges**, with **`E-27` a deliberate gap** recording a withdrawal and **`E-31`/`E-32` deliberate
absences** recording a reservation and a deferral. A reader who finds none of the three has found the
correct state.

| # | Upstream (provider) | Downstream (consumer) | Pattern | Mechanism | Release | Contract |
|---|---|---|---|---|---|---|
| **E-33** | BC-18 Identity & Access | BC-31 Integration | `CF` | Sync port | **V1** | **The V1 OTP / possession-challenge transport.** `BC-18` *"Owns credentials, sessions, devices, **OTP**"* (**L127**) and requests delivery over a port; `BC-31` *"Owns outbound third-party contracts, credentials, retries, idempotent delivery"* (**L140**) and performs the transport. ⭐ **Ratifies an existing declared route, not a new one:** `tool/module_dependencies.yaml` **L437–439** declares `platform/identity → platform/integration:sso_provider` as *"the single sanctioned bypass of `platform/communication`, **for possession challenge delivery only**"* — present since **`a2caa22`**, the commit that created this file. ⛔ **`BC-18` holds no vendor name, credential or template**; ⛔ **`BC-22` is not on this path**. Admitted by `ADR-0128` §2.1/§2.3 |

**Direction.** Inbound to `BC-31`. ⭐ `BC-31` is already an edge consumer — **`E-25`** (`BC-20 Billing →
BC-31`, **L334**) — so this row confers **no new capability** on it; it names one additional caller.

**Pattern.** `CF` + Sync port is **copied from `E-25`**, the existing and only other edge into `BC-31`
...
### 19.2 ⭐⭐ Why this route, and not `BC-18 → BC-22 → BC-31`

⚠ `IMPL-020` §1.3 assumed the OTP travelled through `BC-22`. **That assumption is superseded**, on four
authorities that outrank it — none of which is weakened by this section:

| Authority | Rank / status | Effect |
|---|---|---|
| **`AUTH-3.5`** | 3, FROZEN | *"Authentication **MUST NOT** implement message transport."* ✅ **Satisfied** — `BC-18` calls a **port**; `BC-31` implements the transport. ⭐ Calling a port is not implementing transport |
| **`AUTH-10.37`** | 3, **FROZEN** | *"The OTP itself MUST NOT be treated as a notification and **MUST NOT pass through any general notification path**."* ✅ **Preserved** |
| **`PRD-010` §5 N7** | 3, baselined | *"OTP generation **and OTP SMS delivery**"* — **excluded** from `BC-22`. ✅ **Preserved and confirmed** |
| **`NTF-FR-030`** | 3, baselined | *"V1 integrated channels are **exactly** In-App and Push."* ✅ **Preserved** — `BC-22` gains no SMS channel |

⭐⭐ **The prohibition is protective, not formal.** `MP-GBR-35` (**Rank 1**) gives `BC-22` quiet hours
over *"every channel"* and `NTF-FR-039` requires them to *"**defer, never drop**"*. A deferred OTP
**expires** — `AUTH-3.8` fixes challenge validity at **5 minutes**. Routing OTP through the general
notification path would **break the authentication factor**, which is precisely what `AUTH-10.37`
prevents.

⛔ **`X-03` is not weakened and never applied.** It forbids **`domain → INTEGRATION`** (Matrix **L352**,
**L167**) — a **rank-8** domain. `BC-18` is `platform/identity` at **rank 4**, and Matrix grid **L137**
already carries `R4 IDENTITY & ACCESS` → `INT` = **`◇`** (port allowed).

**Product classification (`PO-1`).** OTP SMS is **transactional authentication delivery, not general
notification**. ⭐ This **confirms** `PRD-010` N7, `AUTH-10.37` and `NTF-FR-030`; it ⛔ **overrides none
of them**, and **no amendment to any of the three is required or made**.

### 19.3 ⛔ What this section does NOT do

- ⛔ **Creates no delivery path.** ⭐ **A route is not a delivery path** — no SMS provider (**D3**), no
  DLT registration (**H1–H3**), no template, sender ID, credential or `AERR-*` identifier
- ⛔ **`BLK-01`, `BLK-02`, `IMPL-020`, `TASK-D10`, `MP-DEP-03` and Gate 3 are UNCHANGED**
- ⛔ **Renames nothing.** `platform/integration:sso_provider` is **accepted as-is for V1** (`AO-2`); no
  sibling port is added and `tool/module_dependencies.yaml` is **byte-unchanged**. ⚠ The naming
  mismatch — an OTP carried by a port named for SSO — is recorded in `ADR-0128` §2.2 as a **future
  architecture consideration** with ⛔ no owner and no obligation attached
- ⛔ **Mints no other edge**, changes no existing edge, and reclassifies no context
- ⛔ **Amends no frozen PRD and no Rank 1–3 requirement**
- ⛔ **Confers no lifecycle stage** and authorises no implementation

---

## 20. ⭐ `ADR-0127` Extension — the Push/FCM egress edge `E-32` (V1) — **M2 execution**

> ⚠⚠ **THIS SECTION IS A NORMATIVE EXTENSION OF §7, NOT A COMMENTARY ON IT.**
> Registered 2026-09-09 as the **M2 execution** of **`Accepted`**
> [`ADR-0127`](../00-governance/adr/ADR-0127-r1-e-32-bc-22-to-bc-31-push-fcm.md) (Architecture Owner),
> whose §1 approved the allocation while expressly deferring the mint: *"the allocation is approved now,
> but **BC Map registration/mint is deliberately deferred to M2**."* §7's rule at **L292** — *"If an edge
> is not in this table, it does not exist and adding it requires an ADR"* — is satisfied: the ADR exists,
> and the row below **is** that table, extended.
>
> ⛔ **`ADR-0127` IS NOT REOPENED, AMENDED OR SUPERSEDED.** This section executes it verbatim and adds
> nothing to it. ⭐ **Appended, not inserted**, on the **§15**/**§16**/**§17**/**§18**/**§19** precedent.

### 20.1 Registered integration edge

⭐⭐ **This registers an identifier for an ALREADY-AUTHORITATIVE contract; it creates no capability.**
Rank-1 `MASTER_PRD` **L229** already records the Push/FCM route, and `PRD-010` **`NTF-FR-031`** already
requires it. Only the **Rank-4 register** was silent — the `ADR-0016` / `ADR-0055` / `ADR-0095` defect class.

| # | Upstream (provider) | Downstream (consumer) | Pattern | Mechanism | Release | Contract |
|---|---|---|---|---|---|---|
| **E-32** | BC-22 Notification Delivery | BC-31 Integration | `CF` | Sync port | **V1** | ⭐ **Push/FCM egress ONLY.** `BC-22` *"Owns channel selection, templates, deduplication, quiet hours, delivery guarantees, consent/unsubscribe"* (**L131**) and reaches the provider over a port; `BC-31` *"Owns outbound third-party contracts, credentials, retries, idempotent delivery"* (**L140**) and performs the egress. `[EVIDENCE]` Rank-1 `MASTER_PRD` **L229** — *"Push notifications \| `BC-22` **via `BC-31`** Integration \| Channel adapter \| Firebase Cloud Messaging \| Yes — FCM (V1)"* — with `PRD-010` **`NTF-FR-031`** (*"Push **MUST** reach FCM through **`BC-31`**; `BC-22` **MUST NOT** hold FCM credentials"*) and **`NTF-INV-005`**. ⛔⛔ **Carries NO SMS and NO OTP.** ⛔ `BC-22` gains **no** vendor name and **no** credential. Admitted by `ADR-0127` §1 |

**Direction.** Inbound to `BC-31`. ⭐ `BC-31` is already an edge consumer — **`E-25`** (`BC-20 Billing →
BC-31`, **L334**) and **`E-33`** (`BC-18 → BC-31`, **§19.1**) — so this row confers **no new capability**
on it; it names one further caller.

**Pattern.** `CF` + Sync port is **copied from `E-25`** (*"Gateway abstraction; Billing knows no vendor
names"*). ⭐ **No new integration pattern is introduced.**

**On §7's "in V1" heading.** **L292** scopes §7 to V1, and both endpoints are **V1** contexts (**L131**,
**L140**). The row is V1-scoped and raises no V2 question.

### 20.2 ⛔⛔ `E-32` is NOT an OTP path — the separation is load-bearing

⚠ `BC-31` now has **two** inbound edges from platform contexts, and they are **deliberately distinct**:

| Edge | Route | Carries | Authority |
|---|---|---|---|
| **`E-32`** *(this section)* | `BC-22` → `BC-31` | **Push / FCM only** | `ADR-0127` · `MASTER_PRD` **L229** · `NTF-FR-031` |
| **`E-33`** (**§19.1**) | `BC-18` → `BC-31` | **OTP / possession challenge** | `ADR-0128` · manifest **L437–439** |

⭐⭐ **Collapsing them would breach four rules.** **FROZEN `AUTH-10.37`** — *"the OTP … **MUST NOT** pass
through any general notification path"* · **`PRD-010` §5 N7** — OTP SMS **excluded** from `BC-22` ·
**`NTF-FR-030`** — *"V1 integrated channels are **exactly** In-App and Push"* · and the **functional**
bar: `MP-GBR-35` + `NTF-FR-039` make quiet hours *"defer, never drop"*, while `AUTH-3.8` expires the
challenge in **5 minutes**.

⛔ **All four are preserved unweakened by this section**, and ⛔ **`E-33` is neither changed nor re-pointed.**

### 20.3 ⛔ What this section does NOT do

- ⛔ **Registers an edge, not a capability.** No SMS provider (**D3**), no DLT registration (**H1–H3**),
  no FCM project, template, sender ID, credential or `AERR-*` identifier
- ⛔ **`BLK-01`, `BLK-02`, `IMPL-020`, `TASK-D10`, `MP-DEP-03` and Gate 3 are UNCHANGED**
- ⛔ **Mints no other edge.** ⛔ **`E-27`** stays permanently vacant; ⛔ **`E-31`** stays **reserved** for
  `PRD-021C` C8 §13 (`TSF-GAP-009`, blocking `IMPL-1424`) and **absent from this file**
- ⛔ **Amends no frozen PRD and no Rank 1–3 requirement**
...
## 22. ⭐ `ADR-0137` Changelog entry — v1.16: `Q-04` REFRAMED and discharged for `MP-DEP-07` only; `LR-01` opened

> ⚠⚠ **THIS SECTION IS THIS FILE'S v1.16 CHANGELOG ENTRY**, recorded at end of file and ⛔ **NOT as a row in the
> `## Changelog` table above** — the same append discipline §21 established for v1.15, and for the identical measured
> reason: the `## Changelog` table occupies **L611–L627** and ⚠ **§15 … §21 all sit BELOW it**, so the changelog is
> **not** this file's last section. A row inserted **newest-first** would land at **L613** and shift ten live
> citations, including ⭐⭐ **L1108**.
>
> ⛔⛔ **L1108 is cited by `Accepted` [`ADR-0129`](../00-governance/adr/ADR-0129-google-sign-in-v1-primary-authentication-mobile-otp-v2.md) L79**, and `ADR-INDEX` **L206** rules
> *"Never edit an Accepted ADR's decision text."* ⭐ **Appending here shifts ZERO citations** —
> [`ADR-0079`](../00-governance/adr/ADR-0079-ea-community-enumeration-executed-by-append-only-section.md) §8.5 **Option A** ⚠ *(path corrected here: §21 and seven earlier citations point at a filename that does not exist — a **pre-existing** defect, disclosed and ⛔ **not repaired in this act**, which is scoped to `ADR-0137`)*, and the
> `ADR-INDEX` end-of-file addendum precedent. ⭐ The header `Version` cell reads **v1.16**, so the current version is
> unambiguous.
>
> ⭐⭐ **What v1.16 does.** ONE cell amended **strictly in place** — the `Q-04` row at **L543**. `Q-04` is **REFRAMED**
> from a legal question into an internal data-governance question and **discharged for `MP-DEP-07` ONLY**, by
> `Accepted` [`ADR-0137`](../00-governance/adr/ADR-0137-q-04-reframed-mp-dep-07-discharged-legal-risk-opened-as-lr-01.md).
> All unresolved legal risk is **transferred** to standing legal-risk item **`LR-01`** in the new Rank 7
> [`LEGAL_RISK_REGISTER`](../00-governance/LEGAL_RISK_REGISTER.md), which is ⛔ **PERMANENTLY OPEN** and carries
> uncertainties **`U-1`…`U-13`**.
>
> ⛔⛔ **What v1.16 does NOT do.** ⛔ Answers **no** legal question · ⛔ decides **no** retention period, TTL, purge
> schedule, cron, archival rule or deletion path · ⛔ gives **no** legal opinion and **no** legal advice · ⛔ claims
> **no** compliance · ⛔ does **not** claim the policy is legally safe, lawful, defensible or sufficient · ⛔ resolves
> **none** of `U-1`…`U-13` · ⛔ does **not** assume indefinite retention is permissible (`U-11`) · ⛔ does **not**
> assert that no automatic deletion occurs anywhere — `RET-13` discloses that managed-BaaS backup/PITR expiry is
> vendor-set, **unmeasured** and **NOT resolved** (`U-13`) · ⛔ edits **no** byte of `ADR-0133`
> (blob `de29b3d8f543cc11cf1dd3a62656d358a974f929`) · ⛔ constitutes **no** office — neither Legal Counsel nor
> *"SECURITY + DATA Governance"* · ⛔ mints **no** permission (`AUTH-7.22` stays at **0**) · ⛔ adds, removes or alters
> **no** context, edge, aggregate, event or invariant; `E-27` stays vacant · ⛔ amends **no** frozen PRD · ⛔ changes
> **no** file under `lib/`, `test/`, `tool/`, `packages/` · ⛔ **closes no implementation blocker — item 7a remains
> OPEN and UNAMENDED**, and `ATT-GAP-005`/`014`/`016`, `AUD-GAP-001`, `MP-GBR-04` and `MP-NFR-10` are all
> **UNAMENDED** · ⛔ confers **no** lifecycle stage · ⛔ leaves `API-GAP-001` **unaltered** and ⛔ **does NOT
> unblock Stage 6A**.
>
> ⚠ **Baseline:** this file is **Rank 4**, so its version change does ⛔ **not** trigger `DOCUMENTATION_BASELINE.md` §7
> rule 4. ⭐⭐ But **`MASTER_PRD` v1.10 → v1.11 is a Rank 1 version change in this same commit**, which **does** trigger
> it — so the baseline identifier **advances** and §7 rule 3's *same commit* limb is satisfied: **every amendment,
> `ADR-0137`, and the new `LEGAL_RISK_REGISTER` in ONE governed commit.**

---

## 23. ⭐ `ADR-0138` Changelog entry — v1.17: the unsupported identifier `SX-02` is WITHDRAWN; the governed API gap is `API-GAP-001` alone

> ⚠⚠ **THIS SECTION IS THIS FILE'S v1.17 CHANGELOG ENTRY**, recorded at end of file and ⛔ **NOT as a row in the
> `## Changelog` table above** — the same append discipline §21 and §22 established, and for the identical measured
> reason: a newest-first row would land at **L613** and shift ten live citations, including ⭐⭐ **L1108, which is
> cited by `Accepted` [`ADR-0129`](../00-governance/adr/ADR-0129-google-sign-in-v1-primary-authentication-mobile-otp-v2.md) L79** — an edit `ADR-INDEX` **L206** forbids.
> ⭐ **Appending here shifts ZERO citations**, and **L1108 is md5-verified identical** before and after this act.
> ⭐ The header `Version` cell reads **v1.17**, so the current version is unambiguous.
>
> ⭐⭐ **What v1.17 does.** **Two cells amended strictly in place** — the `Q-04` row at **L543** and one clause inside
> **§22** — removing the unsupported co-label `SX-02`, by `Accepted`
> [`ADR-0138`](../00-governance/adr/ADR-0138-sx-02-withdrawn-unsupported-identifier.md).
>
> ⚠⚠ **Why.** **`SX-02` was introduced by commit `9a3d43f`** (the `ADR-0137` act) and was ⛔ **NEVER independently
> defined, authorised, scoped, owned or registered** by any governed act — **0** occurrences before that commit,
> **0** in `Accepted` `ADR-0136` (the only governed `SX-*` enumeration), **0** in the `PRD_REGISTRY` prefix register,
> and **0** lines in which it appeared without `API-GAP-001`. ⭐ **The actual governed API gap is `API-GAP-001`
> alone**, defined at `ADR-0135` **L194–196**.
>
> ⛔⛔ **What v1.17 does NOT do.** ⛔ Assigns `SX-02` **no** meaning, scope, owner, requirement semantics or
> continuing status · ⛔ creates **no** replacement identifier — **no `SX-05`**, no rename, no reassignment · ⛔ does
> **not** register the `SX-*` namespace · ⛔⛔ does **not** resolve, narrow or modify **`API-GAP-001`**, which
> **REMAINS OPEN** and byte-unchanged (`ADR-0135` blob `09b85fad4fd33bc5ef392e1372487b78b1328b9f`; authority
> **Product Owner + Architecture Owner**, ⛔ not conferred) · ⛔ does **not** select the **`API-8`** mechanism, still
> **DEFERRED** · ⛔ **does NOT enter Stage 6A** — `PRD_LIFECYCLE` §7A.4's `E-1`…`E-4` reference **no `SX-*`
> identifier at all**, so no entry criterion changes · ⛔ **`TS-001` NOT allocated**, `docs/50-technical/` **NOT
> created**, **no** `TS-*` authored · ⛔ edits **no** byte of `ADR-0133`, `ADR-0135`, `ADR-0136`, `ADR-0137` or
> Authentication v3 · ⛔ adds, removes or alters **no** context, edge, aggregate, event or invariant; `E-27` stays
> vacant · ⛔ amends **no** frozen PRD · ⛔ **closes no implementation blocker — item 7a remains OPEN and
> UNAMENDED**, and `ATT-GAP-005`/`014`/`016`, `AUD-GAP-001`, `MP-GBR-04` and `MP-NFR-10` are all **UNAMENDED** ·
> ⛔ resolves **no** part of `LR-01` or `U-1`…`U-13` · ⛔ changes **no** file under `lib/`, `test/`, `tool/`,
> `packages/` · ⛔ constitutes **no** office and mints **no** permission (`AUTH-7.22` stays at **0**) · ⛔ confers
> **no** lifecycle stage.
>
> ⭐⭐ **`ADR-0137` is BYTE-UNCHANGED** (`c503bc129696b773fff268ada7be696c1408d297`) and superseded **IN PART and IN
> EFFECT ONLY** in **five** places, **as to identifier existence ONLY** — ⭐ **every substantive decision `D-1`…`D-7`
> STANDS**, including `MP-DEP-07` **DISCHARGED**, `Q-04`'s legal content **UNRESOLVED**, and `LR-01` **PERMANENTLY
> OPEN**. ⚠ The **false citation** of `SX-02` to `ADR-0135` at `ADR-0137` **L219** is **disclosed**.
>
> ⚠ **A GLOBAL `SX-02` COUNT IS NOT A VALID VERIFICATION** of this act (`ADR-0138` §3.4). Five references remain
> readable inside `ADR-0137` (`ADR-INDEX` **L207** — *"the superseded record stays readable"*), and the withdrawal
> instrument, its filename and disclosure entries such as this one **necessarily name the identifier they withdraw**.
> ⭐ The correct test is **location and context**: ⛔ **zero occurrences in any operative position.**
>
