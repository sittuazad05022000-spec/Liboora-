...
| **Context Count** | 31 (23 in V1 scope) — **unchanged by v1.1, v1.2, v1.3, v1.4, v1.5 and v1.6** |
...
| E-27 | BC-03 Attendance | BC-04 Seating | `C/S` + `PL` | **Read projection** | `StudentPresence{studentRecordId, tenant, presenceState, attendedToday, method, asOf}` — Seating **composes** presence onto the Seat Card at read time and **stores none of it**. **Attendance remains the sole owner of presence truth**; Seating renders it. Added by [`ADR-0032`](../00-governance/adr/ADR-0032-attendance-presence-visibility-to-seating.md) — required by `PRD-006` §23.4a. **Distinct from `E-08`**: that edge *pushes* two check-in/check-out **events**; this one is *pulled* on read and carries **no event**. Seating **MUST NOT** read raw Wi-Fi or any network observation, and **MUST NOT** infer a presence state from the *absence* of an event |
...
**Note on E-27 vs E-08 — two edges, same direction, deliberately not merged.** `E-08` is an **event** edge: Attendance *pushes* `StudentCheckedIn`/`StudentCheckedOut` and Seating updates live occupancy. `E-27` is a **read projection**: Seating *pulls* presence when a Seat Card is rendered. They are kept as separate rows because the *Mechanism* column would otherwise be ambiguous, and because a presence state change that is **neither a check-in nor a check-out** — entering an approved grace period, crossing a booked-shift end — has **no event** and must not acquire one by widening `E-08`. `E-08`'s contract is unchanged and still carries exactly those two events. `E-02` and `E-03` are the precedent: a read projection is its own row.
...
are unchanged **by that ruling**. *(Edge `E-27` was added later, by `ADR-0032`, and is unrelated to `AR-6`.)*
