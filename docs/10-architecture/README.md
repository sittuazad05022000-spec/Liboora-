...
| [`../../tool/module_dependencies.yaml`](../../tool/module_dependencies.yaml) | `v1` | **Machine-readable** normative rules, including the `domain/person: 7.5` tier. ⚠ Its consumer `tool/check_module_boundaries.dart` **does not exist** — see Open action items. ⚠ It also **describes the decided architecture, not the current code**: `GlobalStudentProfile` still lives in `domain/social`. Deliberate — see [`../40-implementation/TRACEABILITY_MATRIX.md`](../40-implementation/TRACEABILITY_MATRIX.md) §8A |
...
2. ~~Produce the Bounded Context Map~~ — **done**, `LIBOORA_BOUNDED_CONTEXT_MAP.md` v1.3
3. ~~Produce the Module Dependency Matrix~~ — **done**, `LIBOORA_MODULE_DEPENDENCY_MATRIX.md` v1.1 + `tool/module_dependencies.yaml`
3b. ~~Produce a formal ADR set (governance task `R-3`)~~ — **done**, [`../00-governance/adr/`](../00-governance/adr/), `ADR-0001`…`ADR-0011`
...
5. **Implement `tool/check_module_boundaries.dart`** — the fitness function specified in Dependency Matrix §10.2. **It does not exist.** Until it does, the boundaries are documentation, not enforcement. Tracked as **`IMPL-014`, priority P1**, in [`../40-implementation/IMPLEMENTATION_ROADMAP.md`](../40-implementation/IMPLEMENTATION_ROADMAP.md) §3. **`ADR-0011` raised the cost of its absence**: the manifest now declares a rank-7.5 tier with banned symbols (`StudentRecordId`, `TenantId`, `mobileNumber`, `otp`) and banned imports that nothing checks, and the existing code violates several of them. A declared, unenforced boundary reads as protection and is not (`SID-4.56`).
