...
|---|---|---|---|
| `AccountId` | `BC-18` | Global | ⭐ **v1.8** — Created on **first successful Google authentication**, when the provider identity is bound. Credentials only. ⛔ **NOT derived from the Google `sub` or from the Google email**; permanent and non-reusable. Amended by `Accepted` [`ADR-0129`](../00-governance/adr/ADR-0129-google-sign-in-v1-primary-authentication-mobile-otp-v2.md). *(Prior text, correct until v1.8: "Created on first successful OTP. Credentials only.")* |
...
| **MP-GBR-18** | Attendance check-in is **idempotent** by `(studentRecordId, date, idempotencyKey)`. ⭐ **v1.9 — AMENDED by `Accepted` [`ADR-0130`](../00-governance/adr/ADR-0130-student-payment-v1-cash-only-liboora-platform-charge-and-settlement-v2.md).** ⛔ Payment capture idempotent **by gateway reference is V2**, there being **no V1 payment gateway**; ⭐ **V1 cash capture MUST remain idempotent and server-authoritative** (`ADR-0037`). ⚠ The attendance limb is **UNCHANGED**. *(Prior text, correct until v1.9: "Payment capture is idempotent by gateway reference.")* | **New — recovered.** EA critique #10: *"double-charges and duplicate check-ins are inevitable at scale."* |
...
| ID | Addition |
|---|---|
| **MP-GBR-25** | ⭐ **v1.8 — AMENDED by `Accepted` [`ADR-0129`](../00-governance/adr/ADR-0129-google-sign-in-v1-primary-authentication-mobile-otp-v2.md).** **Google Sign-In (Google OIDC) is the sole required authentication method in V1. There are no passwords.** The immutable Google `sub` is the provider identity key; ⛔ the Google email is **never** the canonical identity, `AccountId`, `PersonId`, uniqueness key or automatic matching key. ⚠ **Mobile OTP is NOT a V1 authentication factor** — the capability is re-scoped to **V2**, and is **not** removed: its V2 purpose remains a future Product + Architecture decision. *(Prior text, correct until v1.8: "**Mobile OTP is the sole authentication factor in V1. There are no passwords.** OTP is single-use with a TTL and a bounded attempt budget." — the OTP challenge properties remain valid for V2.)* |
...
| **MP-CON-11** | ⭐ **v1.8 — AMENDED by `Accepted` [`ADR-0129`](../00-governance/adr/ADR-0129-google-sign-in-v1-primary-authentication-mobile-otp-v2.md).** Google Sign-In is the sole required V1 authentication factor. **No passwords, no demo or guest accounts in any release build.** ⛔ The **no-demo / no-guest limb is UNCHANGED and still binding** — `TASK-D10` and Matrix §10.4 gate 3 depend on it. ⚠ The former *"no social login"* clause is **withdrawn for V1 only**, that being the exact clause `ADR-0129` reverses. *(Prior text, correct until v1.8: "Mobile OTP is the sole V1 authentication factor. No passwords, no social login, no demo or guest accounts in any release build.")* |
...
|---|---|---|
| **MP-ASM-01** | ⭐ **v1.8 — RE-SCOPED by `Accepted` [`ADR-0129`](../00-governance/adr/ADR-0129-google-sign-in-v1-primary-authentication-mobile-otp-v2.md).** Every user has an **eligible Google account**. ⛔ A mobile number is **not** a V1 authentication requirement. | ⚠ **The V1 scope exclusion is ACCEPTED, not mitigated**: users who cannot or do not hold an eligible Google account — including minors who cannot independently satisfy Google account eligibility — are **outside V1 authentication scope** and deferred to V2 (`ADR-0129` `A-14`/`A-16`). ⛔ **No fallback factor is introduced.** *(Prior text, correct until v1.8: assumption "Every user has a mobile number capable of receiving SMS", risk "The sole authentication factor fails; a fallback factor becomes a V1 blocker.")* |
...
| **MP-RSK-04** | ⭐ **v1.9 — RE-SCOPED by `Accepted` [`ADR-0130`](../00-governance/adr/ADR-0130-student-payment-v1-cash-only-liboora-platform-charge-and-settlement-v2.md).** Double seat allocation **(V1, unchanged)**; ⛔ duplicate **digital/gateway** payment capture is a **V2** risk, gateway payment being out of V1 scope; duplicate **cash** capture remains controlled by server-authoritative recording (`ADR-0037`). | High | DB constraints, pessimistic locks, idempotency keys, reconciliation. *(Prior text, correct until v1.9: "Double seat allocation or duplicate payment capture.")* |
...
| **MP-DEP-04** | ⭐ **v1.9 — RE-SCOPED to V2 by `Accepted` [`ADR-0130`](../00-governance/adr/ADR-0130-student-payment-v1-cash-only-liboora-platform-charge-and-settlement-v2.md).** Payment gateway onboarding — ⛔ **NOT a V1 external dependency**; student payment in V1 is **cash only** and no V1 gateway exists. | External | ⚠ **V2** — SaaS Billing, fee collection. *(Prior text, correct until v1.9: "Payment gateway onboarding | External | SaaS Billing, fee collection".)* |
...
| **MP-DEP-07** | Resolution of open questions `Q-01`…`Q-07` | ⭐⭐ **v1.11 — ALL SEVEN NOW DISCHARGED; THIS DEPENDENCY IS SATISFIED.** `Q-01`, `Q-03`, `Q-06` resolved by `Accepted` [`ADR-0133`](../00-governance/adr/ADR-0133-mp-dep-07-schema-freeze-questions-q01-q03-q06-resolved-q04-remains-open-counsel.md); `Q-02`, `Q-05`, `Q-07` already settled/closed. ⭐⭐ **`Q-04` is DISCHARGED FOR THIS GATE ONLY by `Accepted` [`ADR-0137`](../00-governance/adr/ADR-0137-q-04-reframed-mp-dep-07-discharged-legal-risk-opened-as-lr-01.md), which REFRAMED it from a legal question into an internal data-governance question and adopted interim policy `RET-01`…`RET-13`.** ⛔⛔ **`Q-04`'s LEGAL CONTENT IS NOT RESOLVED — it is SUPERSEDED IN EFFECT for this gate and TRANSFERRED to standing legal-risk item `LR-01`** ([`LEGAL_RISK_REGISTER`](../00-governance/LEGAL_RISK_REGISTER.md)), **which remains PERMANENTLY OPEN**. ⛔ No retention period, TTL, purge schedule, cron or deletion path exists or is implied; ⛔ **no legal opinion, legal determination or claim of compliance is made or asserted anywhere in `ADR-0137`**; ⛔ *"no automatic deletion"* — which holds **at the application layer only** (⚠ `RET-13`: managed-BaaS backup/PITR expiry is **disclosed, NOT resolved**, uncertainty `U-13`) — does **NOT** mean indefinite retention is legally permissible: that proposition is **UNRESOLVED** (`U-11`) and is exactly what `LR-01` carries. ⚠ **`LR-01` must be reviewed before DPDP §§7–17 commence (~May 2027, G.S.R. 843(E) cl. (c) — a legal commencement date only, ⛔ NOT a retention period).** ⭐ **SCHEMA FREEZE IS UNBLOCKED ON THIS DEPENDENCY** — ⛔ but Stage 6A remains blocked **independently** by `API-GAP-001`, which this act leaves **unaltered**. *(Prior text, correct until v1.11: "6 of 7 RESOLVED … `Q-04` ALONE REMAINS OPEN (Legal counsel required — not conferred) … SCHEMA FREEZE REMAINS BLOCKED".)* *(Prior text, correct until v1.10: "Internal — partially resolved (`Q-02`, `Q-07` settled)".)* | Schema freeze |
...
| # | Question | Status |
|---|---|---|
| `Q-01` | Does an expired membership release the seat immediately, at end-of-day, or after a grace period? | ⭐ **RESOLVED — v1.10** by `Accepted` [`ADR-0133`](../00-governance/adr/ADR-0133-mp-dep-07-schema-freeze-questions-q01-q03-q06-resolved-q04-remains-open-counsel.md), propagating `Accepted` [`ADR-0036`](../00-governance/adr/ADR-0036-three-day-renewal-protection-q01.md) §7/§8: **after a grace period of 3 calendar days**, window starting **`W₀ = D+1`**, ⭐ **FIXED at platform level and mandatory V1** — ⛔ not tenant-configurable, ⛔ no `SEAT-CFG-*`/`BC-25` identifier granted. ⚠ `ADR-0036`'s **release-actor** residual (automatic vs human at window end, `SEAT-FR-217`) remains ⛔ **OPEN** with the `BC-04` owner. *(Prior text, correct until v1.10: "Open — recommendation: configurable, default 24h". ⭐ The 24h figure is retained here because frozen `PRD-005` **L816** rules that "a recommendation inside an open question is not a decision" — it never bound, and is superseded rather than erased.)* |
| `Q-02` | Is `Branch` first-class in V1? | **Settled** — model `branchId` in V1, default single branch (`MP-CON-13`) |
| `Q-03` | Entitlement fail-open or fail-closed on timeout? | ⭐ **RESOLVED — v1.10** by `Accepted` [`ADR-0133`](../00-governance/adr/ADR-0133-mp-dep-07-schema-freeze-questions-q01-q03-q06-resolved-q04-remains-open-counsel.md): the disposition is **per feature, resolved from an external configured value** owned by **`BC-25`** — ⛔ **NO platform-wide default**, and ⛔ no module may derive one by generalising another context's conduct. Taken verbatim from **FROZEN `ENT-FR-018`** and Rank 4 **`E-17`** (BC Map **L326**). ⚠ **Policy SHAPE only:** ⛔ the configured **values do not exist** — **`ENT-GAP-002`** remains **OPEN** (Governance Owner + `BC-25`/`PRD-023`), so `ENT-FR-018` stays satisfiable only after it closes; ⛔ no `ENT-CFG-*` minted. *(Prior text, correct until v1.10: "Open — recommendation: per-gate; paid features fail-closed".)* |
| `Q-04` | Retention period for attendance after enrollment archival? | ⭐⭐ **v1.11 — REFRAMED AND DISCHARGED FOR `MP-DEP-07` ONLY by `Accepted` [`ADR-0137`](../00-governance/adr/ADR-0137-q-04-reframed-mp-dep-07-discharged-legal-risk-opened-as-lr-01.md).** ⛔⛔ **THE LEGAL QUESTION IS NOT ANSWERED.** `ADR-0137` determined that the authority earlier records waited for — *"Legal counsel"*, and `MP-NFR-10`'s *"SECURITY + DATA Governance"* — is ⛔ **constituted NOWHERE** in [`PRD_OWNERSHIP_MODEL`](../00-governance/prd-ecosystem/PRD_OWNERSHIP_MODEL.md) (**0** occurrences of *"counsel"* among its **nine** offices), so the question was **unreachable**, not merely unanswered; and that `MP-DEP-07` requires *"**Resolution** of open questions"* — ⛔ **not a legal determination, and the word "counsel" appears nowhere in that dependency.** ⭐ The question was therefore **reframed** to *"what may V1 safely do while the law is unresolved?"* and answered by **internal interim product policy** `RET-01`…`RET-13` under **Product Owner + Architecture Owner** authority. ⛔⛔ **NO RETENTION PERIOD, TTL, PURGE SCHEDULE, CRON OR DELETION PATH IS DECIDED, DEFAULTED OR IMPLIED; the "7 years financial / 2 years attendance" note REMAINS EXPRESSLY NOT ADOPTED** (blocker **7a** *"Do not adopt"*; `ATT-GAP-005` rejects it as unratified). ⛔ **No legal opinion and no compliance claim is made.** ⛔ *"No automatic deletion"* holds **at the application layer only** — ⚠ `RET-13` discloses that managed-BaaS backup/PITR expiry is **vendor-set, unmeasured and NOT resolved** (`U-13`). ⭐⭐ **All unresolved legal risk — including whether indefinite retention is permissible at all (`U-11`) — is TRANSFERRED to standing item `LR-01` in [`LEGAL_RISK_REGISTER`](../00-governance/LEGAL_RISK_REGISTER.md), PERMANENTLY OPEN**, mandatory review before **~May 2027** *(a legal commencement date only — ⛔ NOT a retention period)*. ⛔ `ATT-GAP-005`, `ATT-GAP-014`, `ATT-GAP-016` and `AUD-GAP-001` **all remain OPEN and UNAMENDED**; ⛔ blocker **7a** remains **OPEN and UNAMENDED**. ⭐ **Six** governed sources plus `Accepted` `ADR-0051` named **Legal counsel**; ⭐ that naming is **preserved as the historical record** and is ⛔ **not deleted** — `ADR-0137` determines only that the named authority is **unreachable within this governance system**, not that the naming was wrong. *(Prior text, correct until v1.11: "STILL OPEN — needs counsel … Legal counsel is the missing authority and cannot be substituted by Product or Architecture".)* |
...
| `Q-06` | Who owns proration arithmetic? | ⭐ **RESOLVED — v1.10** by `Accepted` [`ADR-0133`](../00-governance/adr/ADR-0133-mp-dep-07-schema-freeze-questions-q01-q03-q06-resolved-q04-remains-open-counsel.md): **`BC-02` Membership owns the *entitlement delta*** — the price difference and remaining day count, published on `MM-EVT-004` — and **money execution belongs to the Business Platform** and is ⛔ **V2**. Already frozen and implemented by **`MM-FR-099`**/**`MM-FR-100`** (*"MUST NOT compute a prorated monetary credit … MUST NOT compute a refund"*). ⛔⛔ **Money proration is NOT brought into V1** — `MM-XC-012` and `PRD-008` `NG-6` keep it **V2**, unchanged; ⛔ **no `liboora_contracts` contract created**. ⚠ **`MM-GAP-002`/`ENT-GAP-003` discharged as to OWNERSHIP only** — ⛔ NOT closed as to the V2 mechanism. *(Prior text, correct until v1.10: "Open — recommendation: `BC-02` computes entitlement delta, Business executes money".)* |
...
| Version | Change |
|---|---|
| ⭐ **v1.12** | 2026-09-11. ⭐ **THE UNSUPPORTED IDENTIFIER `SX-02` IS WITHDRAWN FROM THIS DOCUMENT; THE GOVERNED API GAP IS `API-GAP-001` ALONE.** By `Accepted` [`ADR-0138`](../00-governance/adr/ADR-0138-sx-02-withdrawn-unsupported-identifier.md). ⚠⚠ **`SX-02` was introduced by commit `9a3d43f` (the `ADR-0137` act) and was NEVER independently defined, authorised, scoped, owned or registered by any governed act** — **0** occurrences before that commit, **0** in `Accepted` `ADR-0136` (the only governed `SX-*` enumeration), **0** in the `PRD_REGISTRY` prefix register, **0** lines where it appeared without `API-GAP-001`. ⛔⛔ **NO meaning, scope, owner, requirement semantics or continuing status is assigned; NO replacement identifier; NO `SX-05`; NO rename.** **Two cells amended strictly in place** — **L574** (`MP-DEP-07`) and this changelog's **v1.11** row; ⭐ **0 citations shifted** (**0** `#L` anchors exist into this file). ⭐⭐ **NOTHING SUBSTANTIVE CHANGES:** ⛔ **`API-GAP-001` REMAINS OPEN, UNRESOLVED AND BYTE-UNCHANGED** (`ADR-0135` blob `09b85fad4fd33bc5ef392e1372487b78b1328b9f`; authority **Product Owner + Architecture Owner**, ⛔ not conferred); ⛔ **`API-8` still DEFERRED**; ⛔ **Stage 6A NOT entered** (§7A.4 `E-1`…`E-4` reference no `SX-*` identifier at all); ⛔ **`TS-001` NOT allocated** and `docs/50-technical/` **NOT created**. ⛔ `ADR-0137` is **byte-unchanged** (`c503bc129696b773fff268ada7be696c1408d297`) and superseded **IN PART and IN EFFECT ONLY** in **five** places, **as to identifier existence ONLY** — ⭐ **all of its substantive decisions `D-1`…`D-7` STAND**, including `MP-DEP-07` **DISCHARGED**, `Q-04`'s legal content **UNRESOLVED**, `LR-01` **PERMANENTLY OPEN** with `U-1`…`U-13`, and `RET-01`…`RET-13`. ⚠ The **false citation** of `SX-02` to `ADR-0135` at `ADR-0137` **L219** is **disclosed**. ⚠ **A GLOBAL `SX-02` COUNT IS NOT A VALID VERIFICATION** of that act (`ADR-0138` §3.4): five references remain readable inside `ADR-0137` (`ADR-INDEX` **L207**), and the withdrawal instrument, its filename and this disclosure row necessarily name the identifier they withdraw — the correct test is **location and context**, ⛔ **zero operative use**. ⛔ Blocker **7a**, `ATT-GAP-005`/`014`/`016`, `AUD-GAP-001`, `MP-GBR-04` and `MP-NFR-10` **all UNAMENDED**; ⛔ **no office constituted, no permission minted, no role created, no retention period adopted, 0 code files, no push**. |
| ⭐⭐ **v1.11** | 2026-09-11. ⭐⭐ **`MP-DEP-07` IS FULLY DISCHARGED AND THE V1 SCHEMA FREEZE IS UNBLOCKED ON THAT DEPENDENCY — BY REFRAMING `Q-04`, NOT BY ANSWERING IT.** By `Accepted` [`ADR-0137`](../00-governance/adr/ADR-0137-q-04-reframed-mp-dep-07-discharged-legal-risk-opened-as-lr-01.md). ⛔⛔ **NO retention period, TTL, purge schedule, cron or deletion deadline is decided; NO legal opinion is given; NO compliance is claimed; NO legal uncertainty is resolved; NO office is constituted; NO permission is minted.** `Q-04`'s legal content is **superseded in effect for this gate only** and **transferred to standing legal-risk item `LR-01`** in the new Rank 7 [`LEGAL_RISK_REGISTER`](../00-governance/LEGAL_RISK_REGISTER.md) (**PERMANENTLY OPEN**, carrying uncertainties `U-1`…`U-13`, review before ~May 2027 — a commencement date only, ⛔ not a retention period). Two cells amended **in place** — **L574** (`MP-DEP-07`) and **L676** (`Q-04`); ⭐ **0 citations shifted**. `ADR-0133` §5.4's *"Required authority"* Legal-counsel limb, §6.1's two current-state rows and §6.2's current-state heading **superseded IN PART and IN EFFECT ONLY** — ⛔ `ADR-0133` is **byte-unchanged** (`ADR-INDEX` L206). ⛔ Blocker **7a**, `ATT-GAP-005`/`014`/`016`, `AUD-GAP-001`, `MP-GBR-04` and `MP-NFR-10` **all UNAMENDED**. ⛔ Stage 6A **still blocked independently** by `API-GAP-001`, **unaltered**. |
| ⭐ **v1.10** | 2026-09-10. ⭐⭐ **`MP-DEP-07` SCHEMA-FREEZE QUESTIONS: `Q-01`, `Q-03` and `Q-06` RESOLVED; `Q-04` STOPPED AND LEFT OPEN**, by `Accepted` [`ADR-0133`](../00-governance/adr/ADR-0133-mp-dep-07-schema-freeze-questions-q01-q03-q06-resolved-q04-remains-open-counsel.md), decided **jointly** under a one-act conferral by the **Product Owner** and **Architecture Owner** (sitting as **ARB** and as **`BC-04` owner**); ⛔ both offices **revert on completion** (`ADR-0033` §7.1). ⭐ **Exactly SIX cells amended, each STRICTLY IN PLACE**: **L673** (`Q-01`), **L675** (`Q-03`), **L676** (`Q-04`), **L678** (`Q-06`), **L574** (`MP-DEP-07`) and the header `Version` cell. ⚠⚠ **CITATION COST: ZERO SHIFTED — measured before the write and verified after.** **416 citations across 125 documents** point into this file; ⚠ **four of the six amended cells ARE CITED** — **L673** by 3 documents, **L675** by **9**, **L678** by 4, and ⭐ **L678 is the HIGHEST cited line in the entire file** — which is precisely why every amendment is in place and **not one line is displaced**. The file is **735 → 736 lines**, the **+1 being this changelog row**, appended at the foot **below every cited line** per `ADR-0079` §8.5. ⭐⭐ **`Q-01` RESOLVED BY RECOGNITION — it was already answered, and nobody had propagated it.** `ADR-0036` is **`ACCEPTED`** and records *"✅ **ANSWERED at v2.0 — a 3-calendar-day protection window, mandatory V1**"*, with its §8 answering all three of its own questions: duration **3 calendar days**, boundary **`W₀ = D+1`** (2026-08-31), scope **FIXED, not tenant-configurable**. ⛔ Yet **L673** and Rank 4 BC Map **L540** still read *"Open — configurable, default 24h"*, a contradiction in which the two highest-ranked registers disagreed with an Accepted ADR for nine days. ⭐ This act **propagates** that answer, which `ADR-0036` §7 had expressly declined to do. ⭐ **The 24h vs 3-day conflict is resolved BY RANK and the record PRESERVED** — frozen `PRD-005` **L816**: *"A recommendation inside an open question is **not a decision**"*, so the 24h figure ⛔ **never had binding force** and is ⛔ **NOT deleted**, being retained verbatim as prior text. ⚠ `ADR-0036`'s **release-actor** residual stays ⛔ **OPEN** — *when* is now known; *who performs the release* is not. ⭐ **`Q-03` RESOLVED — per feature, from an external `BC-25`-owned configured value; ⛔ NO platform-wide default.** Taken verbatim from **FROZEN `ENT-FR-018`** and Rank 4 **`E-17`**, so ⛔ **nothing is invented**. ⚠ **The distinction is stated, not blurred:** the policy **SHAPE** is settled while ⛔⛔ **the configured VALUES do not exist** — **`ENT-GAP-002`** (*"no entitlement configurable exists at any rank"*; `ENT-CFG-*` empty by finding) ⛔ **REMAINS OPEN** with **Governance Owner + `BC-25`/`PRD-023`**, so `ENT-FR-018` is still *satisfiable only after `ENT-GAP-002` closes* and ⛔ **no entitlement gate can be implemented from this resolution alone**. ⛔ No `ENT-CFG-*` minted, ⛔ no default named, ⛔ `PRD-023` untouched, ⛔ `MM-BR-013`/`SEAT-FR-296` **neither generalised nor touched** (`ENT-XC-007`). ⭐ **`Q-06` RESOLVED — `BC-02` owns the entitlement delta; money execution is the Business Platform's and is V2.** The `BC-02` half was **already frozen and already implemented** by **`MM-FR-099`**/**`MM-FR-100`**, and `PRD-005`'s own note confirms it *"publishes the entitlement delta … deliberately stops short of money"*. ⛔⛔ **Money proration is NOT brought into V1** — `MM-XC-012` and `PRD-008` `NG-6` keep it **V2**, both unchanged; ⛔ **no `liboora_contracts` contract, class, method or interface created**. ⚠ **`MM-GAP-002`/`ENT-GAP-003` discharged as to OWNERSHIP only**, ⛔ **NOT closed** as to the V2 mechanism, and ⛔ `ENT-XC-005`'s *"undetermined owner"* cell is **not rewritten**. ⛔⛔ **`Q-04` STOPPED — REMAINS OPEN, and DELIBERATELY NOT GUESSED.** **Six independent governed sources plus `Accepted` `ADR-0051` all name Legal counsel** — BC Map **L543** (*"Define with counsel"*), **L676** (*"needs counsel"*), **`MP-NFR-10`** (SECURITY + DATA Governance), blocker **7a** (*"Legal counsel + Architecture Owner"*), `ATT-GAP-005`, `AUD-GAP-001` — and ⛔ **Legal counsel is NOT conferred and cannot be substituted by Product or Architecture authority**. ⚠⚠ **THE TRAP IS NAMED:** blocker **7a** states *"**Do not adopt** '7 years financial / 2 years attendance'"* and **`ATT-GAP-005` rejects that note as unratified** — figures that sit in a *recommendation* column, look authoritative, and are ⛔ **NOT adopted, NOT softened into a provisional default, and NOT cited as a starting point**. ⛔ **No retention period, TTL, purge schedule, archival rule or deletion path is created**; `ATT-GAP-005`, `ATT-GAP-014`, `ATT-GAP-016`, `AUD-GAP-001` and blocker **7a** ⛔ **all remain OPEN**. ⭐ **L676 is amended only to RECORD the stop** — so a later reader can tell the question was examined and refused, not overlooked. ⛔⛔ **SCHEMA FREEZE REMAINS BLOCKED, stated without softening.** `MP-DEP-07` requires **all seven** questions; ⛔ `Q-04` is open; therefore the dependency is ⛔ **NOT discharged**. ⭐ What changed is that the blocker narrowed from **four open questions to one**, with a **single named external prerequisite** — which is progress and ⛔ **NOT a freeze authorisation**. ⛔⛔ **NOTHING ELSE EXECUTED:** ⛔ **EVERY FROZEN PRD IS UNTOUCHED** — `PRD-005`, `PRD-006`, `PRD-007`, `PRD-008`, `PRD-014`, `PRD-016`, `PRD-023`; ⭐ possible only because frozen `SEAT-BR-024` was drafted *"to be correct under any resolution"* of `Q-01` · ⛔ `PRD-019`/`PRD-022` **not modified** · ⛔ `ADR-0036`, `ADR-0011`, `ADR-0051`, `ADR-0052`, `ADR-0130`, `ADR-0131`, `ADR-0132` **byte-unchanged** · ⛔ Authentication **v2.0/v3.0** byte-unchanged, ⛔ **no v4.0** · ⛔ **no permission, no `PERM-*`** — `AUTH-7.22` remains **closed and empty** per `ADR-0132` · ⛔ **no requirement, configuration, event, aggregate, edge, port, state or bounded context created** — count remains **31**, highest edge **`E-33`**, ⛔ **`E-34` NOT allocated** · ⛔ **BC Map `E-17` wording at L326 UNCHANGED** — it was already correct · ⛔ **no Technical Specification**, ⛔ **`TS-001` NOT allocated**, ⛔ **`docs/50-technical/` NOT created** · ⛔ **no API, endpoint, OpenAPI, SQL, schema, table, column, index, migration or Supabase artefact** · ⛔ **`Q-02`'s `branchId` settlement neither re-opened nor implemented** · ⛔ **no code, no test, no CI change** · ⛔ **blockers `B-4`, `B-5`, `B-6` ALL REMAIN OPEN** · ⛔ **`SAAS-GAP-003`/`004` remain OPEN and V2-deferred** · ⛔ **no `GCP-*` closed, no blocker discharged** · ⛔ **`ADR-0129`'s register row still MISSING** · ⛔ **`A-9` not executed** · ⛔ **NOT pushed** — `github/main` remains `267f46e5c9b191da874553ffdfaf70dbfd3720cb`. |
| ⭐ **v1.9** | 2026-09-10. ⭐⭐ **V1 STUDENT PAYMENT IS CASH ONLY; the LIBOORA PLATFORM CHARGE and LIBRARY → LIBOORA SETTLEMENT are DEFERRED TO V2**, by `Accepted` [`ADR-0130`](../00-governance/adr/ADR-0130-student-payment-v1-cash-only-liboora-platform-charge-and-settlement-v2.md), decided **jointly** under a one-act conferral by the **Product Owner**, **Architecture Owner**, **`BC-20` Owner**, **Authorization (`BC-18`/`PRD-001`)** and **Governance Owner**; ⛔ every office **reverts on completion** (`ADR-0033` §7.1). ⭐ **Exactly THREE rule cells amended, each STRICTLY IN PLACE**: **L359** (`MP-GBR-18`), **L554** (`MP-RSK-04`) and **L571** (`MP-DEP-04`) — plus the header `Version` cell. ⚠⚠ **CITATION COST: ZERO SHIFTED — measured before the write and verified after.** The file is **734 → 735 lines**, the **+1 being this changelog row itself**; **416 line-number citations across 125 documents** point into this file, the highest at **L678**, and ⭐ **none of L359, L554 or L571 is cited** (measured **0**), so every citation still resolves to its intended rule. This row is **appended newest-first at the head of the version-history table at the foot of the document, below every cited line**, per `ADR-0079` §8.5. ⭐ **V1: student cash payment YES; ⛔ UPI NO · ⛔ card NO · ⛔ online payment NO · ⛔ student payment gateway NO.** ⭐ **V2 (DEFERRED, not designed): UPI · card · online payment · gateway · gateway provider.** ⭐⭐ **The LIBOORA platform/extra charge and library → LIBOORA settlement are OUT OF V1 SCOPE** — ⛔ no V1 settlement instrument, rail, authorization or architecture is required, and their absence is ⛔ **NOT a V1 blocker**. ⚠ *"NO" in V1 means **deliberately out of V1 scope**, not broken or awaiting V1 implementation.* ⛔⛔ **`MP-GBR-24` is PRESERVED UNCHANGED** — student→library (`BC-05`) and library→LIBOORA (`BC-20`) *"must never share a model, a table or a metric"*; the two flows are distinct, which is precisely why the absence of the second does not invalidate the first. ⭐ **`ADR-0046` is SUPERSEDED for its V1 limb ONLY and is BYTE-UNCHANGED** (`ADR-INDEX` L206 — *"never edit an Accepted ADR's decision text"*); ⭐ **Razorpay WAS properly selected as the V1 gateway on 2026-08-15 and that history is preserved, not denied** — it is superseded because its subject left V1 scope, and ⛔ **no V2 provider is selected** (`Q-B31` stays OPEN). ⚠ **This is the repository's FIRST ADR supersession** (measured 117 ADRs, 0 previously superseded). ⭐ **`SAAS-GAP-003`** (outbound settlement architecture) and **`SAAS-GAP-004`** (settlement authorization) are **re-classified V2/DEFERRED — NOT claimed resolved**; the architecture determination is recorded that **`ADR-0035`'s `O-3` is scoped verbatim to *student → library* and does NOT cover library → LIBOORA**, and that **`E-25` is a gateway abstraction** unusable under cash-only V1 — ⛔ so `ADR-0035` is **not extended by interpretation**, `E-25` is **preserved**, and ⛔ **no edge is minted (`E-34` NOT allocated)**. ⛔ **`AUTH-7.22`'s closed permission catalogue is PRESERVED INTACT**: ⛔ no permission created, ⛔ no `PERM-*` identifier (measured: enumerated permissions **0**, `PERM-*` repo-wide **2**, both withdrawal records — **no convention exists to extend**), ⛔ **no Authentication PRD v4.0**, and ⛔ Authentication PRD **v2.0/v3.0 and `prd-v2/`/`prd-v3/` are BYTE-UNCHANGED**. ⛔ **`PRD-008` v1.7 is FROZEN and NOT edited** — its §6.1 **L209**/**L210**/**L211**/**L213**/**L214** V1 rows (UPI, card, online, verification, webhook) **contradict this decision and are superseded IN EFFECT by rank** (baseline §4: Rank 2 ADR > Rank 3 PRD), ⚠ a **governed and temporary divergence** whose lawful repair is a **`PRD-008` successor — a SEPARATE act, NOT performed here**; ⛔ its stale self-described "DRAFT" header is **NOT rewritten**. ⛔⛔ **NOTHING ELSE EXECUTED:** BC Map, Dependency Matrix, `tool/module_dependencies.yaml`, the EA, `ADR-0035`, `ADR-0037`, `ADR-0043`, `ADR-0045` and all code, schema, API, UI, test, migration and deployment files are **unchanged**; ⛔ **`ADR-0002`/`ADR-0005` supersessions still OUTSTANDING** (`ADR-0129` §7); ⛔ **`SAAS-GAP-001`/`002`/`005`/`006`/`007` remain OPEN**; ⛔ **no blocker closed** — `MP-DEP-03`, `BLK-01`, `BLK-02`, `TASK-D10`, gate 3, `H1`–`H4`, `D3` stand, and ⚠ **`FEE-GAP-017` is NOT closed** but **re-classified as V2 work**, its question concerning a charge that does not exist in V1; ⛔ **`A-9` not executed**; ⛔ **no provider, instrument, mechanism or implementation created**; ⛔ **not pushed** — `github/main` remains `267f46e5c9b191da874553ffdfaf70dbfd3720cb`. |
| **v1.8** | 2026-09-10. ⭐⭐ **V1 AUTHENTICATION CHANGED: Google Sign-In replaces Mobile OTP as the sole required V1 authentication method**, by `Accepted` [`ADR-0129`](../00-governance/adr/ADR-0129-google-sign-in-v1-primary-authentication-mobile-otp-v2.md), decided **jointly** by the Architecture Owner and Product Owner with **Privacy Owner sign-off**, under a one-act conferral. ⭐ **Exactly FOUR rule cells amended, each STRICTLY IN PLACE**: **L309** (`AccountId` now created on first successful **Google authentication**; ⛔ never derived from `sub` or email), **L379** (**`MP-GBR-25`**), **L522** (**`MP-CON-11`**) and **L535** (**`MP-ASM-01`**) — plus the header `Version` cell. ⚠⚠ **CITATION COST: ZERO SHIFTED — measured before the write and verified after.** ⭐ **v1.8 CORRECTION (D-1):** the file is **733 → 734 lines** — the four rule cells and the header cell are amended **strictly in place and contribute no line change**, and the **+1 line is this changelog row itself**, appended at the foot of the document. *(An earlier v1.8 changelog draft stated "733 → 733 lines"; that figure was **incorrect** and is corrected here on independent re-measurement.)* ⭐ **v1.8 CORRECTION (D-2) — citation metric restated with a reproducible methodology.** *(An earlier v1.8 changelog draft stated "375 line-number citations across 110 documents"; that figure could **not be reproduced** by independent review and is **withdrawn as unverifiable** — it is replaced, not re-estimated.)* **Method, so any reviewer can reproduce the figure exactly:** search scope `docs/` + `tool/` + `.github/`, text files only; a **citation** is one occurrence of the literal `MASTER_PRD.md`, an optional closing backtick, up to two spaces, optional `**`, then `L` followed by 1–4 digits — POSIX BRE `MASTER_PRD\.md`\{0,1\} \{0,2\}\*\{0,2\}L[0-9]\{1,4\}`; a **citing document** is any file containing at least one such match; ⛔ excluded as **false positives** are line numbers belonging to other documents that merely co-occur in the same sentence (notably Enterprise-Architecture lines such as `L1405`, `L1534`, `L1821`), which is precisely why an unanchored search over-counts. **Result on this methodology: 416 citations across 125 documents**, the **highest cited line being L678** — verified stable across two independent runs. All four amended lines are edited **in place** and every citation therefore **still resolves to its intended rule**; the amended lines are cited only from `ADR-0129` and `ADR-INDEX.md`, both of which cite them **as amended**. This changelog row is **inserted newest-first inside the version-history table at the very foot of the document, below every cited line**, per the `ADR-0079` §8.5 append-not-insert doctrine. ⭐ **`U-1 = B`: the verified mobile number is NO LONGER the V1 identity anchor.** V1 requires **no** mobile number, mobile verification, Mobile OTP or SMS OTP. ⛔⛔ **Mobile OTP is NOT deleted — it is RE-SCOPED to V2**, and its V2 purpose, optionality or eventual removal remains a **future Product + Architecture decision**. ⭐ **Google identity rules:** the immutable Google **`sub`** is the provider identity key; ⛔ the Google **email** is **never** the canonical identity, `AccountId`, `PersonId`, payment identity, uniqueness key or automatic matching/re-link key; ⛔ a Google **recovery Gmail** is Google-side recovery only and is **never** a Liboora recovery identity. ⭐ **`AccountId` remains the canonical internal Liboora identity — permanent and non-reusable.** ⛔⛔ **PRESERVED UNCHANGED:** **`MP-CON-11`'s no-demo / no-guest limb** (`TASK-D10` and gate 3 depend on it) · **`MP-GBR-27`** (credentials only inside `BC-18`) · **`MP-GBR-34`** (no event carries a mobile number) · **`MP-GBR-01`/`02`/`04`/`05`** (identity triad, financial-history survival, guardian consent) · `BC-10` `PersonId` ownership and atomicity (`ADR-0011`) · the tenant/role/membership model · the payment identity boundary · `GuardianLink` as an **enrollment** mechanism (⛔ never delegated authentication, ⛔ no parent/proxy login) · Reception and `PR-1`/`PR-2` authenticate identically with **no role exception and no second V1 factor** (existing elevation/session/audit controls remain **layered** — `ADR-0063`). ⛔ **Non-authentication OTP/SMS references are DELIBERATELY UNCHANGED** and were classified individually, not swept: **L97** (`MP-SCOPE-09`), **L368** (§18 security principles), **L452**/**L453**/**L457** (§22 notification channels and `MP-CFL-03`), **L536** (`MP-ASM-02`), **L570** (**`MP-DEP-03`**), **L604**, **L708**. ⛔⛔ **NOTHING ELSE IS EXECUTED BY THIS AMENDMENT:** Authentication PRD v2.0, the BC Map, `tool/module_dependencies.yaml`, `DOCUMENTATION_BASELINE.md`, `ADR-0002`, `ADR-0005`, `ADR-0011`, `ADR-0128` and all code, schema, API, UI and test files are **byte-unchanged**; ⛔ **no BC Map edge minted (`E-34` NOT allocated)**; ⛔ **no `identity_provider` port created**; ⛔ **`ADR-0002` and `ADR-0005` NOT superseded** (both required, both outstanding — `ADR-0129` §7); ⛔ **no blocker closed** — **`MP-DEP-03`**, `BLK-01`, `BLK-02`, `TASK-D10`, Gate 3, **H1**–**H4** and **D3** all unchanged; ⛔ **`A-9` secret custody remains outside the conferral**. ⚠ **Consequential work still outstanding, each requiring its own governed act:** **Authentication PRD v3.0 re-baseline** (588 requirements; `AUTH-3.22` removal, `AUTH-4.1` re-keying, the `Unlinked` state, and privacy conditions **C-1**/**C-2**/**C-3** incl. `AC-11.18` and `AUTH-11.31`) · BC Map identity edge · the `identity_provider` port · the two supersessions · **and the new baseline identifier**, since §7 rule 4 moves it for a **Rank 1–3** version change — ⚠ **`DOCUMENTATION_BASELINE.md` is NOT updated by this act and its §3 declaration is therefore knowingly stale until that act runs.** |
