...
⚠ **Provenance disclosure.** This record's 230 lines were committed to the repository by an automated
`genspark auto-backup` commit (**`9426ec7`**) that fired while the authoring commit was being prepared,
**not** by a deliberate commit of the agent. ⭐ Verified before proceeding: the backup captured **exactly
this file and nothing else** (`git show --stat`: *1 file changed, 230 insertions*), the working tree
content is **byte-identical** to the commit, and all three pre-commit citation corrections (§11 v1.0)
survived intact. ⛔ **The auto-backup was NOT amended or rewritten** — history is preserved and the
provenance is disclosed here instead, consistent with the append-only doctrine.
