...
⚠ **Provenance disclosure.** This record, `ADR-0112` and the `ADR-INDEX.md` row were committed by an
automated `genspark auto-backup` commit (**`0f032e2`**) that fired while the authoring commit was being
prepared, **not** by a deliberate commit of the agent. ⭐ Verified before proceeding: the backup captured
**exactly the three intended files and nothing else** (`git show --stat`: *3 files changed, 346
insertions, 1 deletion*), and the working tree is **byte-identical** to the commit. ⛔ **The auto-backup
was NOT amended or rewritten** — history is preserved and the provenance is disclosed here, consistent
with the append-only doctrine. ⚠ This is the **third** such capture in this engagement, each disclosed
the same way.
