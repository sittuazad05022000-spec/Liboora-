...
⚠ **Provenance disclosure.** This record was committed by an automated `genspark auto-backup` commit (**`fe341de`**), and the accompanying instrument fix by **`2a7b03f`**, both firing while the authoring commits were being prepared — **not** by deliberate commits of mine. ⭐ Verified before proceeding: between them they captured **exactly** this record and `prd009_task_coverage.py`, and nothing else. ⛔ **Neither was amended or rewritten.** This is the **sixth** such capture in this engagement, each disclosed the same way.

⚠⚠ **A regression this record caused in my own instrument, and how it was fixed.** Writing this file broke `prd009_task_coverage.py`: it failed with *"REUSED identifiers: ['IMPL-2000']"*, taking the suite from **33/6** to **32/7**. ⛔ **That was my instrument being too crude, not a real collision** — condition 5 below legitimately **cites** the range `IMPL-2000`…`2031` as evidence, exactly as `PRD-010`'s readiness record cites its own `IMPL-1900`…`1929`. ⭐ The checker was corrected to exclude `PRD-009`'s **own** artefacts while still scanning every other document, and the fix was **mutation-tested in both directions**: an own-artefact citation now passes, and a genuine foreign clash planted in a scratch file still **FAILS** with *"REUSED identifiers: ['IMPL-2005']"*. ⛔ The record was **not** weakened to make the checker pass, and the checker was **not** weakened to make the record pass. Suite restored to **33/6**. ⚠ A stale hardcoded *"13 tasks are blocked"* string in the checker's own output was also corrected — the measured figure is **8**.
