...
### 23.4a Seat Card live presence — the five-state requirement, and why it is specified but NOT yet implementable

**Product requirement.** The Seat Card must show **current student presence** alongside **seat allocation**, and the
two **MUST NOT** be collapsed into one field. Five presence renderings are requested:
...
**Part 1 — the frozen contract is BINARY; the requirement needs FIVE states.** `SEAT-FR-103` says
...
**Consequently the Seat Card five-state live presence display is SPECIFIED and NOT IMPLEMENTABLE in V1 as the
...
> 3 resolved, 1 narrowed, 19 open.** Where a row's status changed, §32.1 states the source that changed it. The
...
| **`ATT-GAP-002b`** | 🔴 **OPEN — and it blocks a Product-Owner-required display** | **A new question, opened rather than answered, and the blocker was proved rather than predicted.** The Seat Card must show current student presence beside seat allocation (§23.4a). **The separation itself needs nothing** — frozen `SEAT-FR-041` already states *"`Occupied` means allocated, not physically present"*, and `SEAT-FR-103`/`104`/`105`/`106` already require the card to compose such values at read time, label them as-of, and degrade visibly. **What is missing is fidelity and transport.** `SEAT-FR-103`'s occupancy field is **two-valued**; the requirement needs **five** states, and widening a frozen Rank 3 requirement is the Architecture Owner's act. And `E-08` carries exactly two binary transitions, so entering grace, crossing the booked-shift end and crossing tolerance into overstay have **no carrier at all**. A fifth `attendance.*` event was injected and the Stage 5 gate re-run: it failed **five** ways, including *"section 2F registers ATT-EVT-\* as 4 (001..004); computed 5"* and an `ATT-NFR-010` coverage failure — and `ATT-AC-117` independently asserts *"exactly four … and no other"*. The probe was reverted before anything was written. **Three shortcuts were named and refused**: a raw Wi-Fi → Seat read (`ATT-BR-045`, `SEAT-BR-020`, L292); `BC-04` inferring grace from event *absence* (the *"last known network event"* fabrication §10A.5 prohibits, merely relocated); and overloading `StudentCheckedOut` to mean *entered grace* (it would emit a check-out that did not happen). A **suffixed successor of `ATT-GAP-002`**. Framed by **`ADR-0032`** (`Proposed`); the Architecture owner must decide. |
...
**Result over the 22 rows: 3 resolved (`ATT-GAP-001`, `ATT-GAP-002`, `ATT-GAP-017`), 1 narrowed
(`ATT-GAP-011`), 18 open** —
...
| **Wi-Fi Presence — Seat Card live presence (§23.4a)** | **V1 SCOPE — specified, NOT implementable.** ⛔ The **separation** of allocation from presence is already guaranteed by frozen `SEAT-FR-041`/`103`, but the frozen occupancy field is **binary** and `E-08` carries only two check transitions, so grace / outside-window / overstay / no-booking have **no carrier** — `ATT-GAP-002b`, `ADR-0032`. A raw Wi-Fi → Seat read is **refused** |
