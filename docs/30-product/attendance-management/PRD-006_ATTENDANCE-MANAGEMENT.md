...
| **Version** | **v1.9 — FROZEN** |
| **Status** | **`FROZEN`** — [`PRD_LIFECYCLE.md`](../../00-governance/prd-ecosystem/PRD_LIFECYCLE.md). **Admitted to the baseline at Rank 3 by [`ADR-0034`](../../00-governance/adr/ADR-0034-attendance-management-prd-v1.9-baseline.md) under `BASELINE-2026-08-05-A`, and recorded in [`DOCUMENTATION_BASELINE.md`](../../00-governance/DOCUMENTATION_BASELINE.md) §3.3.** *"Freeze is **conferred, not claimed**"* — this row states a fact conferred elsewhere and creates nothing; if it ever disagrees with §3.3 of that declaration, **the declaration is right and this row is a defect**. **Stage 3 ✅ ALIGNED**, 6 of 6 lifecycle checks, re-run for v1.9 *after* the `E-27` dependency decision ([`PRD-006_ARCHITECTURE_ALIGNMENT_V19_SUPPLEMENT.md`](./PRD-006_ARCHITECTURE_ALIGNMENT_V19_SUPPLEMENT.md) v1.1, whose superseded **⛔ NOT ALIGNED** v1.0 verdict is retained beside the new one). **Stage 4 ✅ verdict A**, 6 of 6 ([`PRD-006_STAGE4_REREVIEW_V19.md`](./PRD-006_STAGE4_REREVIEW_V19.md)). **Stage 5/6 gates exit 0** — 525 identifiers across nine registers, all contiguous, 0 duplicates, 0 dangling, 0 orphan criteria, **0 collisions**, and **285/285 = 100.0%** of obligations both verified and task-claimed across 179 tasks. **Two earlier freeze attempts were REFUSED and both records are preserved** — the first for want of an admitting ADR and a baseline row, the second because freeze-readiness verification found a **Rank 4 conflict**: `E-27` sat outside `LIBOORA_MODULE_DEPENDENCY_MATRIX.md`'s intra-cluster allow-list `E-01`…`E-10`. **That blocker was resolved by withdrawing the edge, not by widening the allow-list** — `ADR-0033`, option `O-C`, accepted under **Architecture Owner** authority **conferred by the human principal** and recorded as a conferral, **not** as an ARB meeting; BC Map → v1.7 and the Dependency Matrix was **not amended**. **`ADR-0032` was never demoted** and remains `Accepted`; only its transport changed. **FROZEN is not VERIFIED**: **0 of 179 tasks and 0 of 222 acceptance criteria are proven by a test**, and `SID-4.56` holds that *"a rule that cannot be checked SHALL be treated as unmet"*. **18 gap-ledger entries remain OPEN with named owners and are NOT ratified by admission**; `ADR-0021`…`ADR-0031` remain **`Proposed`** — *"Under consideration; not binding"* — and `ADR-0031` in particular stays `Proposed` because **no Product Owner has approved a range** for the presence tolerance, so **a twenty-fifth configuration row was refused rather than invented** and the tolerance remains a fixed product rule (§10A.7a). Freezing this document changed **none** of that |
| **Date** | 2026-08-04 · **frozen 2026-08-05** |
| **Release** | **V1** |
| **Baseline** | **`BASELINE-2026-08-05-A`** — **admitted to it.** *Was written against `BASELINE-2026-08-04-E` and explicitly **not** admitted to it; the new identifier was issued **because** this Rank 3 document entered the baseline (`DOCUMENTATION_BASELINE.md` §7 step 4)* |
| **Rank** | **Rank 3** — authoritative for **everything inside `BC-03` Attendance**, conferred by [`DOCUMENTATION_BASELINE.md`](../../00-governance/DOCUMENTATION_BASELINE.md) §3.3 and §4. Rank 3 now holds **seven** module baselines and they do not overlap: this document owns whether a student is **present**; `PRD-007` v1.0 owns whether a seat is **allocated**. **Seat allocation is not presence and presence is not allocation**, and neither document may decide the other's question. Outranked by Rank 1 (`MASTER_PRD.md` global rules) and Rank 2 (Accepted ADRs) |
...
| **Authorities applied** | Master PRD v1.7 (Rank 1) · `ADR-0001`, `ADR-0011`, `ADR-0012`, `ADR-0017`, **`ADR-0032`**, **`ADR-0033`**, **`ADR-0034`** (Rank 2) · Authentication PRD v2.0, Library PRD v1.1, `PRD-003` v1.0, `PRD-004` v1.2 **FROZEN**, `PRD-005` v1.4 **FROZEN**, `PRD-007` v1.0 **FROZEN** (Rank 3) · **BC Map v1.7**, Module Dependency Matrix v1.3 (Rank 4) · `ARCHITECTURE_RULINGS.md` v1.2 (Rank 5) · Enterprise Architecture v2.1 (Rank 6, **descriptive only**). *The BC Map reference was **v1.5** while this document was DRAFT; it is **v1.7** now because `ADR-0032` added edge `E-27` (v1.6) and `ADR-0033` withdrew it again (v1.7) — so the **net edge set is unchanged** at `E-01`…`E-26`, and `ADR-0031` is deliberately absent from this list because it is still `Proposed` and binds nothing* |
...
| Version | Date | Change |
|---|---|---|
| **v1.9 — FROZEN** | 2026-08-05 | **Metadata only. FROZEN and admitted to the baseline at Rank 3 — not one requirement was touched.** The version number is deliberately **not** incremented and the specification is **not** renumbered: Stage 7 *confers* status, and `PRD-004` (v1.2), `PRD-005` (v1.4) and `PRD-007` (v1.0) were each admitted at the version they carried. **Six header rows changed and nothing else**: *Version*, *Status*, *Date*, *Baseline*, *Rank* and *Authorities applied*. **This was verified mechanically rather than asserted** — the md5 of every line of the document below the header block was measured before and after the edit and is **identical** (`8fb479c2387d1098a47824eb8b9b69a9`), and the body is therefore untouched. **Correction, recorded rather than quietly amended:** an earlier draft of this row claimed *“the total line count is unchanged at 3215”*. That claim was **false and is withdrawn** — the document is **3216** lines, because **this changelog row is itself the one added line**. The body md5 above is the load-bearing evidence and it holds; the line count never could, since adding a changelog row necessarily adds a line. Logged as `GCP-12` in [`DOCUMENTATION_BASELINE.md`](../../00-governance/DOCUMENTATION_BASELINE.md) §6. So **no requirement, business rule, invariant, event, exclusion, protected operation, configurable, non-functional requirement, acceptance criterion, gap-ledger row or scope statement was added, removed, renumbered or reworded**, and the register totals stand exactly as the gates measured them: **525 identifiers**, **285 obligation-bearing**, **222 acceptance criteria**, **four** domain events closed at four, **24** configuration rows each keeping a Default *and* a Range. **The row immediately below is preserved verbatim**: a frozen document's history is part of the record, and the v1.9 DRAFT entry is what shows the specification was complete **before** anyone conferred a status on it. **Admitted by `ADR-0034`** under **`BASELINE-2026-08-05-A`**, recorded in `DOCUMENTATION_BASELINE.md` §3.3, §4 and §8 and in `PRD_REGISTRY.md` v1.8. The freeze became possible only because the last genuine blocker was resolved: **`ADR-0033` withdrew edge `E-27`** rather than widening the Rank 4 intra-cluster allow-list, so the Dependency Matrix was **not amended**, the BC Map returned to **26 edges**, and `ADR-0032` **kept its `Accepted` status throughout**. **What this freeze did NOT do**: it closed **no** gap — all **18** remain OPEN with named owners — it accepted **no** `Proposed` ADR beyond the two the Architecture Owner actually decided, it did **not** create the twenty-fifth configuration row that `ADR-0031` still lacks a Product-Owner-approved range for, it weakened **no** gate, and it touched **no** code. **FROZEN is not VERIFIED**: **0 of 179 tasks and 0 of 222 acceptance criteria** are proven by a test. |
