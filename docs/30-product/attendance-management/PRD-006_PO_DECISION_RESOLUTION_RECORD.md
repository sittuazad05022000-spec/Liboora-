...
| **`R-19`** | **Amendment of §2F's `ATT-CFG` and `ATT-AC` counts, so `D-18`'s tolerance can be registered** | Architecture Owner | 🟡 **HALF CLOSED — and the half that is open is open by REFUSAL, not by neglect.** **The `ATT-AC` half is DONE**: nine criteria were registered (`ATT-AC-214`…`ATT-AC-222`, PRD-006 §30.20), of which **`ATT-AC-221` verifies the 30-minute tolerance behaviourally** — 05:50 → 10:10 against a 06:00–10:00 booking must yield **exactly 4 hours**. So this row's disclosed consequence, *"the only open row that currently prevents an approved numeric value from being verifiable"*, **no longer holds: the value is now verifiable.** **The `ATT-CFG` half is REFUSED and stays open.** A twenty-fifth `ATT-CFG` row would need a **default and an allowed range** (`PRD_LIFECYCLE.md` Stage 4 check 3: *"Unbounded configuration is a specification hole"*), and **no range has ever been approved by the Product Owner**. Inventing one would have produced a register row that **passes every gate while corrupting the specification**. **`ADR-0031` therefore remains `Proposed`**, and the tolerance stands as a **fixed product rule** rather than a configurable. *(The twenty-fifth row's number is deliberately not written out: PRD-006's traceability gate reads a well-formed identifier as a reference, and it correctly failed exactly that citation when it was written — the prose was changed, the check was not.)* **Still owned by the Architecture Owner + Product Owner jointly; still not claimed as closed.** |
...
| **`R-21`** | **How does authoritative student presence reach the Seat Card at six-state fidelity?** | Architecture Owner + Seat Management product owner | 🟢 **RESOLVED by `ADR-0032` (`Accepted`, option `O-5`) — decided by the named owner, and the blocker was DISSOLVED rather than paid for.** Presence is **composed at read time** by `BC-04` over **one new read edge `E-27`** (`BC-03` → `BC-04`, `C/S` + `PL`, read projection — BC Map **v1.6** §7.1), and frozen `SEAT-FR-103`'s presence field was **minimally widened** to four states (`PRD-007` **v1.1**, **one field cell**, executed **ADR-first and non-silently**, on the `ADR-0016` one-cell precedent). **Both measured blockers closed, neither by weakening a rule.** *Fidelity*: the field list was re-inspected and contains **no** *"only"*, *"exactly"* or *"no other"* — unlike `SEAT-FR-107`, which does — so the enumeration was never closed. *Transport*: because a read is a **pull**, a presence change that is neither a check-in nor a check-out needs **no carrier at all**, so the measured **five-way** cost of a fifth event was **avoided, not paid** — the event surface stays at **exactly four**. **Authority is recorded honestly**: Architecture Owner authority **conferred by the human principal**, written down in `ADR-0032` §5.2 as a conferral with an explicit *"what is NOT claimed"* table — **no ARB meeting was held, minuted or attended**. **What this does NOT close:** the **six-state** ask is met only **four states deep**; the two shift-relative renderings (`OUTSIDE SHIFT WINDOW`, `SHIFT OVERSTAY`) still need a **booked shift** `BC-03` cannot read — **`R-14`/`ATT-GAP-002a`, `ADR-0029` (`Proposed`), OPEN**. Overstay is therefore specified as **two axes** (🟢 PRESENT **plus** a separate overstay badge) and is **not yet evaluable**. `SEAT-GAP-009` also stays **OPEN** — it asks the **opposite** direction (`BC-04` → `BC-03`), and closing it on `E-27` would be a **false closure**. |

**Four rows are now green, one is half green, and each closed for a different reason — the reasons are not interchangeable.** `R-11`
...
(`D-20`); and **`R-21` closed because the Architecture Owner exercised conferred authority and, in doing so, found an option
that removed the cost rather than paying it** (`ADR-0032`, `O-5`). Only the first is evidence about the system; the others are
exercises of authority, and recording them as though they were the same kind of fact would overstate how much has been
*discovered*. **`R-19` is deliberately marked half green rather than green**: half of it was performed and half was **refused
on a stated ground**, and colouring the whole row green would claim work that was consciously not done.

**Seven rows remain fully open, and `R-19` remains half open.** **`R-19` is no longer the load-bearing one, and the reason
is worth stating precisely**: it was load-bearing because it was *"the only open row that currently prevents an approved
numeric value from being verifiable"* — and **`ATT-AC-221` now verifies that value**, so that specific consequence is
discharged. What remains of `R-19` blocks only the tolerance's **configurability**, which no requirement asks for. **The
load-bearing open row is now `R-14`/`ATT-GAP-002a`** — the missing booked-shift input, which blocks the two shift-relative
Seat Card renderings and the whole `SCHEDULE MISMATCH` axis. `R-15` has been **narrowed** by `D-21`
