...
| **Version** | **v0.1** |
| **Status** | **`FROZEN`** — conferred 2026-08-20 by `ACCEPTED` [`ADR-0053`](../../00-governance/adr/ADR-0053-configuration-prd-v0.1-baseline.md) and by the row in [`DOCUMENTATION_BASELINE.md`](../../00-governance/DOCUMENTATION_BASELINE.md) §3.3/§4 under **`BASELINE-2026-08-20-A`**. **This document still confers no status on itself** — the status is read *off the baseline*, and this cell is a transcription of that conferral, not a claim by the document. ⛔ **`FROZEN` is not `VERIFIED`:** 0 of 59 `CNF-AC-*` proven by a test, 0 of 30 `IMPL-1100`…`1129` tasks implemented, 8 `CNF-GAP-*` OPEN. *Superseded text retained verbatim:* **`DRAFT`** — [`PRD_LIFECYCLE.md`](../../00-governance/prd-ecosystem/PRD_LIFECYCLE.md) **Stage 2 (Draft)**. Stages 3–7 not yet run. **This document confers no status on itself** |
...
| **Lifecycle stage** | **Stage 7 — Freeze & Governance, PASSED.** Stage 1 **PASS** (`PRD-023_STAGE1_DISCOVERY.md` §10) · Stage 2 complete · Stage 3 **PASS / unconditional** ([`PRD-023_ARCHITECTURE_ALIGNMENT.md`](PRD-023_ARCHITECTURE_ALIGNMENT.md) §9, 6/6) · Stage 4 **PASS 6/6** ([`PRD-023_STAGE4_REQUIREMENTS_REVIEW.md`](PRD-023_STAGE4_REQUIREMENTS_REVIEW.md)) · Stage 5 **GATE SATISFIED** ([`PRD-023_STAGE5_CONFERRAL.md`](PRD-023_STAGE5_CONFERRAL.md), inventory frozen at 180 identifiers / 113 Class A) · Stage 6 **PASS** ([`../../40-implementation/configuration/PRD-023_IMPLEMENTATION_TASKS.md`](../../40-implementation/configuration/PRD-023_IMPLEMENTATION_TASKS.md), `IMPL-1100`…`1129`, 113/113) · Stage 7 **PASS** ([`PRD-023_STAGE7_FREEZE.md`](PRD-023_STAGE7_FREEZE.md)). **Stage 8 not begun** |
...
| **Registry status** | **`FROZEN`** in [`PRD_REGISTRY.md`](../../00-governance/prd-ecosystem/PRD_REGISTRY.md) §4.3 **L327**, moved `PLANNED` → `FROZEN` on 2026-08-20 by the `ADR-0053` commit. **Still not changed *by* this document** — the register is written by the governance act, and this cell reports it. *Superseded text retained verbatim:* **`PLANNED`** … §4.3 **L327**. Not changed by this document |
| **Rank** | **Rank 3** — conferred 2026-08-20 by [`DOCUMENTATION_BASELINE.md`](../../00-governance/DOCUMENTATION_BASELINE.md) §3.3 and §4, **not claimed here**. With `BC-18` (Authentication) and `BC-19` (`PRD-013`) already at Rank 3, **`BC-25` completes the BC Map L271 FOUNDATIONAL band at 3 of 3**. *Superseded text retained verbatim:* **None yet.** Rank 3 is conferred by … §3.3/§4 at Stage 7, **not claimed here** |
| **Baseline** | **Admitted under `BASELINE-2026-08-20-A`**, issued because a Rank 3 document entered the baseline (baseline §7 rule 4). *Superseded text retained verbatim:* Written against **`BASELINE-2026-08-19-C`** — the baseline current when the requirement text below was authored, and the one every Stage 3–6 measurement was taken against |
| **Date** | 2026-08-20 |

> ⚠ **This header changed at Stage 7 and the version did NOT, and both facts are stated rather than left to inference.**
> Five cells above — `Status`, `Lifecycle stage`, `Registry status`, `Rank`, `Baseline` — were edited on 2026-08-20 to
> transcribe a conferral made elsewhere. **`Version` remains `v0.1`**, per `ADR-0020` §4 item 2 (*"freeze confers
> status; it does not renumber"*) and its §5, which expressly rejects *"renumber to v1.0 to mark the freeze"*.
>
> ⚠ **The file's sha256 therefore changed, and that change is disclosed, not hidden.** Stages 3, 4, 5 and 6 were all
> measured against `e97496083a76bfb0f45be9acae754050c374561b64734df5738dac7ff6399326`. **No requirement, rule,
> invariant, exclusion, acceptance criterion, gap or section body below this header was altered** — the edit is confined
> to freeze metadata, exactly as [`../entitlement/PRD-014_STAGE7_FREEZE.md`](../entitlement/PRD-014_STAGE7_FREEZE.md) §3
> records for `PRD-014`. The 180 identifiers, 8 registers, 113 Class A obligations and 59 acceptance criteria those
> stages measured are byte-identical; [`PRD-023_STAGE7_FREEZE.md`](PRD-023_STAGE7_FREEZE.md) §3 records the post-edit
> hash and the diff scope so the substitution can be checked rather than trusted.
...
| Version | Date | Change |
|---|---|---|
| **v0.1** | 2026-08-20 | **Stage 7 — FROZEN at Rank 3, and the version deliberately NOT moved.** Admitted by `ACCEPTED` [`ADR-0053`](../../00-governance/adr/ADR-0053-configuration-prd-v0.1-baseline.md) under **`BASELINE-2026-08-20-A`**, with the Rank 3 row written into `DOCUMENTATION_BASELINE.md` §3.3/§4 — which is the gate `PRD_LIFECYCLE.md` **L155-175** actually names. **No requirement text changed.** The edit is confined to five header cells (`Status`, `Lifecycle stage`, `Registry status`, `Rank`, `Baseline`) plus this row; the 180 identifiers, 8 registers, 113 Class A obligations and 59 acceptance criteria measured at Stages 3–6 are byte-identical, and the resulting sha256 change is disclosed in [`PRD-023_STAGE7_FREEZE.md`](PRD-023_STAGE7_FREEZE.md) §3 rather than left for a reader to discover. **`Version` stays `v0.1`** per `ADR-0020` §4 item 2 and §5 — the fourth v0.x admission, after `PRD-013`, `PRD-016` and `PRD-014`. ⛔ **What the freeze does NOT assert:** **0 of 59** `CNF-AC-*` are proven by a test, **0 of 30** `IMPL-1100`…`1129` tasks are implemented, acceptance coverage is **76 of 113 = 67.3%** — which is **not** the 100.0% *task* coverage figure and must not be quoted as one — all **8 `CNF-GAP-*` remain OPEN** (including `CNF-GAP-001`, the unresolved aggregate ownership, admitted **as an open conflict** rather than settled to make this pass), the **7** architecture tests required by `TRACEABILITY_MATRIX.md` §10.3 are still **missing**, and **`S4-D-01` is disclosed, not repaired** — §12's uncovered set names 40 where 37 are uncovered, an error that *understates* coverage and whose repair would have changed the hash every Stage 3–6 measurement is anchored to. ⚠ **Two governance defects are carried into this freeze unrepaired and named:** **`GCP-20`** (baseline §4 is missing the `PRD-014` Entitlement Rank 3 row, so after this admission §4 holds 11 rows against §3.3's 12 — the arithmetic gap **survives by design**) and **`GCP-21`** (baseline §3.2 records `ARCHITECTURE_RULINGS.md` at v1.2 while that file's L6 reads v1.1; which record is wrong is **not established**, so neither value was written). ⚠ **No file under `lib/`, `packages/`, `test/` or `web/` was touched and no checker was modified**, so no gate was taught about this document in the act that admitted it. |
...
*End of `PRD-023 — Settings & Configuration` **v0.1 (Stage 7 — FROZEN at Rank 3)**. The version is unchanged from the Stage 2 draft **on purpose**: `ADR-0020` §4 item 2 — freeze confers status, it does not renumber.*
