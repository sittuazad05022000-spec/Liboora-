...
`LDR-FR-023` — *(deliberately not allocated — see the note below)*

> ⚠ **This identifier is intentionally absent, and the absence is recorded rather
...
⚠ **One identifier slot was deliberately not allocated** — `LDR-FR-023`, the
withdrawn fallback requirement (§4.8). The register stops at **022** and remains
contiguous; the withdrawal is recorded, not hidden.
