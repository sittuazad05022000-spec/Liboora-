...
| **Version** | ⭐⭐ **v1.6** (Startup MVP) — successor amendment under `Accepted` [`ADR-0150`](../../00-governance/adr/ADR-0150-bc-02-membership-complete-operation-role-authorization-allocation.md), adding the **complete eleven-operation authorization matrix** (§16.3b); **v1.5** text carried forward unchanged except as §26's v1.6 row records. *(v1.5 was the change-request workflow under `Accepted` `ADR-0149`.)* |
...
|---|---|---|---|---|
| `MM-FR-n` | Functional requirement | ⭐⭐ **150** | `MM-FR-001` … `MM-FR-144`, plus the suffixed `MM-FR-057a`, `MM-FR-119a`, `MM-FR-121a`, `MM-FR-123a`, `MM-FR-127a`, `MM-FR-131a` | 2–11, 19–20; §2.4a, §16.3a, §17.1a (v1.5); ⭐ **§16.3b (v1.6)** |
...
| `MM-PO-n` | Protected operation (**closed list**) | **11** ⛔ *unchanged at v1.5 **and** v1.6* | `MM-PO-001` … `MM-PO-011` | §16.3; allocation in **§16.3a** *(two ops)* and ⭐ **§16.3b** *(all eleven)* |
| `MM-CFG-n` | Configurable | **9** | `MM-CFG-001` … `MM-CFG-009` | §13.4 |
| `MM-GAP-n` | Proposed gap / open question — **not a requirement** | ⭐ **14** | `MM-GAP-001` … ⭐ **`MM-GAP-011`**, plus the suffixed successors `MM-GAP-006a`, `MM-GAP-007a` and `MM-GAP-010a` | §25.2 |

⭐⭐ **Total: 374 identifiers** *(v1.6; was **366** at v1.5 and **333** at v1.4)*, of which ⭐ **360 carry
obligation** and 14 are `MM-GAP-*` open questions, which are not obligations. Of the 360, ⭐ **254 are
obligations that an acceptance criterion can verify** (all registers except `MM-AC-*` itself).
...
| All criterion-verifiable obligations | **146** | ⭐ **254** | ⚠️ **57.5%** *(v1.5: 146 / 247 = 59.1%; v1.4: 134 / 223 = 60.1%)* |

Per register: `MM-BR-*` 31/35 · `MM-INV-*` 12/12 · `MM-EVT-*` 7/7 · `MM-XC-*` 16/16 · `MM-PO-*` **11/11** · ⭐ `MM-FR-*`
**60/150** · `MM-NFR-*` 7/14 · `MM-CFG-*` 2/9.

> ⚠️⚠️ **COVERAGE FELL AGAIN AT v1.6 — 59.1% → 57.5% — AND IS PUBLISHED AS MEASURED.** v1.6 adds **7**
> `MM-FR-*` obligations (`MM-FR-138`…`MM-FR-144`) and ⛔ **0** new acceptance criteria, so the denominator rose
> while the numerator did not. ⭐ **Writing criteria to order was again DECLINED.** ⛔ The 7 uncovered new
> obligations are **named**: `MM-FR-138`, `MM-FR-139`, `MM-FR-140`, `MM-FR-141`, `MM-FR-142`, `MM-FR-143`,
> `MM-FR-144`. ⭐⭐ **`MM-PO-*` stays 11/11** — the *rule* registers are unaffected at **77/81 = 95.1%**, and it
> is the **authorization allocation itself**, not its verification, that v1.6 delivers.
...
### 16.3b Complete operation×role authorization — all eleven protected operations

⭐⭐ **Added at v1.6 under `Accepted` [`ADR-0150`](../../00-governance/adr/ADR-0150-bc-02-membership-complete-operation-role-authorization-allocation.md)**, which records the **Product Owner**'s explicit allocation for the nine operations §16.3a did not reach, formalised by the **Authorization (`BC-18` / `PRD-001`) Owner**. ⛔ **`MM-PO-002` and `MM-PO-011` are governed by `ADR-0149` and are reproduced here for completeness only — their allocation is UNCHANGED.**

⭐ **Expressed as a derived capability** (`ADR-0132` §5.3), evaluated at request time through
`identity.policy_decision`. ⛔ **Zero `PERM-*` identifiers**; `AUTH-7.22` remains **closed at zero enumerated
permissions**.

| Operation *(protected operation cited, **not redefined**)* | Action | `TR-1` Owner | `TR-2` Manager | `TR-3` Reception | `TR-4` Student | `TR-5` Parent |
|---|---|---|---|---|---|---|
| Create membership plan (`MM-PO-001`) | **Create** | ✅ `Tenant` | ⛔ | ⛔ | ⛔ | ⛔ |
| Edit / activate / deactivate plan (`MM-PO-002`) | **Approve** / **Update** | *see §16.3a — `ADR-0149`, unchanged* | | | ⛔ | ⛔ |
| Create membership (`MM-PO-003`) | **Create** | ✅ `Tenant` | ✅ `Tenant` | ✅ `Tenant` | ⛔ | ⛔ |
| **Manually activate** a `PendingPayment` membership (`MM-PO-004`) | **Update** | ✅ `Tenant` | ✅ `Tenant` | ✅ `Tenant` | ⛔ | ⛔ |
| Void a pre-activation membership (`MM-PO-005`) | **Update** | ✅ `Tenant` | ✅ `Tenant` | ✅ `Tenant` | ⛔ | ⛔ |
| Renew membership (`MM-PO-006`) | **Create** | ✅ `Tenant` | ✅ `Tenant` | ✅ `Tenant` | ⛔ | ⛔ |
| Upgrade membership (`MM-PO-007`) | **Create** | ✅ `Tenant` | ✅ `Tenant` | ✅ `Tenant` | ⛔ | ⛔ |
| View membership detail *(includes price snapshot)* (`MM-PO-008`) | **Read** | ✅ `Tenant` | ✅ `Tenant` | ⛔⛔ **DENY** | ✅ `Self` | ✅ `Linked` |
| List operational membership views (`MM-PO-009`) | **Read** | ✅ `Tenant` | ✅ `Tenant` | ✅ `Tenant` | ⛔ | ⛔ |
| Query validity for a student (`MM-PO-010`) | **Read** | ✅ `Tenant` | ✅ `Tenant` | ✅ `Tenant` | ✅ `Self` | ✅ `Linked` |
| Change a membership configurable (`MM-PO-011`) | **Approve** / **Update** | *see §16.3a — `ADR-0149`, unchanged* | | | ⛔ | ⛔ |

⭐ **Every cell is stated explicitly.** ⛔ **No authority is inherited, implied or derived from any other cell**,
from role naming, or from any ordering of roles.

`MM-FR-138` — Authorization for every `MM-PO-*` protected operation **MUST** be expressed through the
**derived-capability** mechanism and evaluated by `BC-18` at request time. ⛔ This module **MUST NOT** define,
store, cache or enumerate a permission (`MM-XC-016`).

`MM-FR-139` — Each role's treatment **MUST** be stated **explicitly** for every operation. ⛔ Authority **MUST
NOT** be derived from role naming, hierarchy, seniority or similarity to another grant (`AP-4` / `AUTH-7.7`),
and permissions **MUST NOT** form a hierarchy (`AUTH-7.28`).

> ⭐⭐ **Why the prose Notes of §16.3 could not simply be read as this matrix, recorded because it is the reason
> this section exists.** §16.3's Notes carried *"Owner/Manager class"*, *"Reception class **and above**"* and
> *"Elevated"*. ⛔ *"And above"* presupposes a **role ordering**, and `AP-4`/`AUTH-7.7` bars deriving a permission
> from *"hierarchy, seniority"*; ⛔ *"Elevated"* has **zero** definitions as a role set. ⭐ [`ADR-0019`](../../00-governance/adr/ADR-0019-membership-management-prd-v1.4-baseline.md)
> **L103** already disclaimed those cells — `MM-PO-001`…`011` *"name the protected operations **without modelling
> a role**"*. ⭐ **So this matrix is DECIDED, not derived**, and §16.3's Notes remain **descriptive prose that
> confers nothing**.

`MM-FR-140` — `TR-3` Reception **MUST NOT** hold `MM-PO-008`. A membership detail includes the **price
snapshot** (`MM-FR-040`), and `AC-7.8` denies a Reception holder financial permissions *"regardless of other
roles held anywhere"*. ⛔ The denial **MUST NOT** be replaced by a field-restricted read.

> ⭐⭐ **`G-8` is closed here, in the negative.** [`ADR-0149`](../../00-governance/adr/ADR-0149-membership-plan-and-configuration-change-request-workflow-authorization.md)
> §9 raised the `MM-PO-008` / `AC-7.8` tension as an independent, non-blocking gap owned by the **Product Owner +
> Authorization Owner**. ⭐ It is resolved **by upholding `AC-7.8` literally**, ⛔ **not by qualifying it**: the
> Product Owner expressly refused to invent a field restriction, and the existing `D-7` restriction
> (`MM-FR-131`) is scoped to **change requests**, ⛔ **not to membership detail**.

`MM-FR-141` — `TR-2` Manager **MUST NOT** hold `MM-PO-001`. A membership plan carries `price`, `durationDays`
and `eligibilityRule` (`MM-FR-023`), which are **commercial configuration**, and `TR-2`'s authority is *"entire
library, **excluding commercial configuration**"*. ⛔ This preserves the same boundary `MM-FR-135` applies to
plan **edits**.

`MM-FR-142` — `TR-5` Parent's authority in this module **MUST** be **read-only** and **MUST NOT** exceed
`Linked` scope. ⛔ `TR-5` **MUST NOT** hold **Create**, **Update** or **Approve** on any `MM-PO-*`.

`MM-FR-143` — `TR-4` Student's authority in this module **MUST NOT** exceed `Self` scope, and **MUST** be
**Read** only. ⛔ `TR-4` **MUST NOT** hold `MM-PO-001`, `MM-PO-003`, `MM-PO-005`, `MM-PO-006` or `MM-PO-007` in
V1 — ⚠️ recorded as **`MM-GAP-011`**, a **NON-BLOCKING** deferral, ⛔ **not** a decision that student
self-service is unwanted.

`MM-FR-144` — An operation not granted to a role in the table above **MUST** be refused. ⛔ Absence of a grant
is refusal, never permission (`AP-3` deny-by-default, `AP-9` fail-closed).
...
| `MM-GAP-011` | ⭐⭐ **Student (`TR-4`) self-service / request authority for `MM-PO-001`, `MM-PO-003`, `MM-PO-005`, `MM-PO-006` and `MM-PO-007`.** V1 is **DENY** (`MM-FR-143`) because the closed change-request workflow admits only `MM-PO-002` and `MM-PO-011` as target operations (`MM-FR-119`), and ⛔ **no `Initiate` action exists** in the closed action vocabulary — so there is **no lawful representation** for student-initiated renewal, upgrade, void or membership creation in V1. ⚠️ **This is a DEFERRAL, not a product position that student self-service is unwanted.** Whether V2 should widen `MM-FR-119`'s target set, and for which operations, is undecided. ⛔ **Opened by `ADR-0150` §5; NOT resolved by it**, and ⛔ **no request type, action, permission or workflow was invented to avoid opening it** | **Product Owner** + **Authorization (`BC-18`) Owner** | ⛔ **No** — `AP-3` deny-by-default already makes V1 behaviour safe and fully defined |
...
| ⭐⭐ **v1.6** | 2026-09-19 | ⭐⭐⭐ **SUCCESSOR AMENDMENT — THE COMPLETE ELEVEN-OPERATION AUTHORIZATION MATRIX IS ESTABLISHED (§16.3b), implementing `Accepted` [`ADR-0150`](../../00-governance/adr/ADR-0150-bc-02-membership-complete-operation-role-authorization-allocation.md).** Authored under one-act **Product Owner** *(the substantive allocation)* and **Authorization (`BC-18`/`PRD-001`) Owner** *(formalisation)* conferrals, ⛔ **reverting on completion** (`ADR-0033` §7.1); ⚠️⚠️ **exercised by ONE principal**, so ⛔ **no independent review, ARB quorum or external Security audit is claimed**. ⭐⭐ **THE NINE PREVIOUSLY UNCOVERED OPERATIONS ARE NOW ALLOCATED — AND THE ALLOCATION IS *DECIDED*, NOT *DERIVED*, WHICH IS THE WHOLE POINT.** §16.3's prose Notes (*"Owner/Manager class"*, *"Reception class **and above**"*, *"Elevated"*) ⛔ **could NOT lawfully be read into role sets**: *"and above"* presupposes a **role ordering**, and `AP-4`/`AUTH-7.7` bars deriving a permission from *"hierarchy, seniority"* while `AUTH-7.28` bars permission hierarchies; *"Elevated"* has **0** definitions as a role set repository-wide. ⭐ `ADR-0019` **L103** had already disclaimed those cells as naming operations *"without modelling a role"*. ⚠️⚠️ **THREE CONFLICTS WERE RAISED AGAINST FROZEN TEXT BEFORE ANYTHING WAS WRITTEN, RETURNED TO THE PRODUCT OWNER RATHER THAN REINTERPRETED, AND ALL THREE RESOLVED RESTRICTIVELY:** ⭐ **(1) `MM-PO-008` `TR-3` = DENY** (`MM-FR-140`) — `AC-7.8` is upheld **literally**, ⛔ **not** softened by inventing a field restriction; the existing `D-7`/`MM-FR-131` restriction is scoped to **change requests**, ⛔ **not** to membership detail, so extending it would have been a new product decision disguised as a preservation. ⭐⭐ **This CLOSES `G-8`** (`ADR-0149` §9) **in the negative.** ⭐ **(2) `MM-PO-001` `TR-2`/`TR-3` = DENY** (`MM-FR-141`) — a plan carries `price`/`durationDays`/`eligibilityRule` (`MM-FR-023`), which is **commercial configuration**, and `prd-v2/02` §2.4.1 scopes `TR-2` to *"entire library, **excluding commercial configuration**"*; ⭐ this keeps v1.6 consistent with `ADR-0149` `D-2`, which denied `TR-2` even the **approval** of a plan edit. ⭐ **(3) *"INITIATE REQUEST"* WITHDRAWN** — ⛔ **no `Initiate` action was invented** (`AUTH-7.24`) and ⛔ **`MM-FR-119`'s closed target set was NOT widened**; `TR-4` is **DENY** on `MM-PO-001`/`003`/`005`/`006`/`007` (`MM-FR-143`), and the consequence is recorded as ⭐ **`MM-GAP-011`**, **NON-BLOCKING**, ⛔ **a deferral rather than a position that student self-service is unwanted**. ⚠️ **TWO OPERATIONS ENDED MORE RESTRICTED THAN THE DECISION'S FIRST DRAFT**, and that draft is disclosed at `ADR-0150` §2.3 so the trail shows the decision was **challenged, not transcribed**. ⛔⛔ **EVERY CLOSED REGISTER PRESERVED, RE-MEASURED MECHANICALLY: `MM-PO-*` STILL 11** — ⭐ **the closed protected-operation list is NOT extended; v1.6 allocates the existing eleven and mints none** — **`MM-EVT-*` still 7** (⛔ no event published by an authorization statement), **`MM-CFG-*` still 9**. ⭐ **Counts: 366 → 374** (`MM-FR` **143 → 150** via `MM-FR-138`…`MM-FR-144`; `MM-GAP` **13 → 14** via `MM-GAP-011`; the other eight registers unchanged). ⚠️⚠️ **COVERAGE FELL 59.1% → 57.5% AND IS PUBLISHED AS MEASURED, NOT SOFTENED** — 7 new obligations, ⛔ **0** new criteria, each uncovered obligation **named** in §0.2; ⭐ **writing criteria to order was DECLINED**, and the *rule* registers are unaffected at **77/81 = 95.1%** with **`MM-PO` 11/11**. ⚠️⚠️ **A REAL STRUCTURAL DEFECT IN THIS AMENDMENT WAS CAUGHT BY THE COMMITTED CHECKER, NOT BY REVIEW, AND IS RECORDED RATHER THAN QUIETLY FIXED:** §16.3b was first written with rows keyed `| \`MM-PO-nnn\` | …`, which [`tool/docs_check/prd005_traceability.py`](../../../tool/docs_check/prd005_traceability.py) correctly reported as **`DUPLICATE DEFINITIONS: 11`** — the table was **re-defining** the closed register rather than **citing** it. ⭐ **The document was corrected** (rows re-keyed on the operation name, the identifier cited **inside** the cell) — ⛔ **the checker was NOT loosened** — and duplicates returned to **0**. ⭐ A second checker finding, `MM-GAP-011` not counting toward its register, was the same class: the row had been written `| ⭐ **\`MM-GAP-011\`** |` against the `TABLE_DEF` convention of a **bare** leading identifier; corrected, and the total reconciled to **374** against the checker's independent measurement. ⛔⛔ **`AUTH-7.22` REMAINS CLOSED AT ZERO ENUMERATED PERMISSIONS · 0 `PERM-*` MINTED · NO new action class, scope class or role · `ADR-0132` NOT reopened and NONE of its §7 prerequisites engaged** — ⭐ §16.3b **allocates** existing operations to existing roles through the **derived-capability** mechanism (`ADR-0132` §5.3); ⛔ **allocation is not minting**. ⛔ `ADR-0149`'s `MM-PO-002`/`MM-PO-011` allocation is **preserved unchanged** and reproduced in §16.3b **for completeness only**. ⛔ **No requirement renumbered, no section renamed, no existing `MM-PO-*` semantics altered, no `MM-GAP-*` closed, 0 lines of code.** ⭐ `FROZEN` and **Rank 3** retained — `PRD_LIFECYCLE.md` §4 amends a frozen PRD through a governed route, and **L177** bars only ***silent*** modification. |
