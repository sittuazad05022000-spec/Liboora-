...
| **`NTF-CFG-004`** | **Retry attempt bound** (`NTF-FR-049`) | Bounds the retry loop so a permanently failing delivery reaches a terminal state rather than retrying forever | **Architecture Owner** | ⭐ **UNIT AUTHORITATIVE · VALUE OWED** — the unit is **total attempts including the first**, fixed by the **rank-0 `JobRuntime` port contract** (`job_runtime.dart` **L104-106**) with a **≥ 1** floor enforced executably (`services.dart` **L152-159**, citing `INV-21`). ⛔ **Number owed** — `NTF-GAP-018`. See §20.3 |
...
**regenerable***. ⭐⭐ **Notification delivery has the opposite property**: a notification is
**not regenerable** and has **no second copy** — a lost membership-expiry notice is simply not sent.
⇒ The two domains have **inverted recovery models**, so `3` is not merely unauthorised for `PRD-010`,
it is **derived from an assumption that does not hold here**. `ADR-0033` **§7.1** forbids the borrow;
this measurement shows the borrow would also be **wrong on the merits**.
...
`NTF-GAP-*` (**25**) · `NTF-CFG-*` (**6**). ⚠ **Counts are provisional; the AC register is
