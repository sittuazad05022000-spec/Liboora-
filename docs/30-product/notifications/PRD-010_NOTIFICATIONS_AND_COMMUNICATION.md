...
| **`NTF-CFG-003`** | **Category classification register** — each catalogue entry's **mandatory / optional** class (`NTF-FR-038`, `NTF-FR-040`) | Adding a catalogue entry must not change the contract | **Product Owner** | ⚠ **PARTIAL — allowed set RATIFIED, per-entry defaults OWED.** ⭐ Allowed set **{`mandatory`, `optional`}** — **`[AUTHORITATIVE]`** by Product Owner act, `ADR-0108` §7.2. ⭐⭐ **Frozen constraint recorded:** notices discharging `AUTH-8.52` / `AUTH-9.29` / `AUTH-9.43` / `AUTH-9.76` MUST be `mandatory` and MUST NOT be opt-outable. ⛔ **Per-entry defaults `[OWED — Product Owner]`** (`NTF-GAP-015`) |
...
| ⭐ **`NTF-CFG-007`** | **Transport / attempt-run deadline** — the bound on **one submitted delivery run** (`NTF-FR-049`, `NTF-FR-042`) | The `JobRuntime` contract **requires** a `deadline` on every submission (`job_runtime.dart` **L113-114**), so `BC-22` cannot submit work without one | **Architecture Owner** | ⛔ **REQUIRED SLOT · UNIT `[AUTHORITATIVE]` · VALUE `[OWED — Architecture Owner]`.** Architecture Owner act, `ADR-0108` §5: no default, minimum or maximum ratified; value owed pending FCM/provider evidence (`NTF-GAP-017`, `NTF-GAP-026`). ⛔ **120 s expressly REJECTED.** ⛔⛔ **MUST NOT be described as a user-visible hard latency cap** — `services.dart` **L193** checks the deadline only *between* attempts and **L207** leaves `await work()` unbounded |
...
Registers, **re-measured at v0.11** (`grep -o | sort -u`, not asserted): `NTF-FR-*` (**66**) ·
`NTF-BR-*` (**3**) · `NTF-INV-*` (**11**) · `NTF-XC-*` (**6**) · `NTF-AC-*` (**10**) ·
`NTF-GAP-*` (**28**) · `NTF-CFG-*` (**7 declared — 5 configurable, 2 withdrawn as NOT configurable**). ⚠ **Counts are provisional; the AC register is
