...
|---|---|---|---|---|
| **`NTF-CFG-001`** | **Bulk recipient-count ceiling** and **dispatch rate limit** (`NTF-FR-017`) | Abuse and cost control that is **not** an entitlement — the `FIL-CFG-009` shape (*"maximum concurrent uploads per actor"*) | **Product Owner** | ⛔ **OWED** — `NTF-GAP-007`. **0** rate-limit or recipient-ceiling number exists at any rank |
...
| **`NTF-CFG-003`** | **Category classification register** — each catalogue entry's **mandatory / optional** class (`NTF-FR-038`, `NTF-FR-040`) | Adding a catalogue entry must not change the contract — the **`FIL-CFG-001`** shape (*"the closed set of upload purposes, **each with its isolation class**"*) | **Product Owner** | ⛔ **OWED** — `NTF-GAP-015`. ⚠ Range is enumerated **{mandatory, optional}**; the **per-entry default is the owed part**. ⛔ **`NTF-FR-038`'s no-opt-out guarantee for `mandatory` is NOT configurable** |
| **`NTF-CFG-004`** | **Retry attempt bound** (`NTF-FR-049`) | Bounds the retry loop so a permanently failing delivery reaches a terminal state rather than retrying forever | **Architecture Owner** | ⭐ **UNIT AUTHORITATIVE · FLOOR AUTHORITATIVE · VALUE OWED** — unit = **total attempts including the first** (rank-0 `JobRuntime` contract, `job_runtime.dart` **L104-106**); floor = **≥ 1**, enforced **executably** (`services.dart` **L152-159**, citing `INV-21`). ⭐ **`[RECOMMENDED — NOT AUTHORITATIVE]` 3 total attempts, range 1–5** (§20.3c). ⛔ **Value owed** — `NTF-GAP-018`; must be ratified **jointly** with the deadline (**`NTF-CFG-007`** / `NTF-GAP-026`) |
| **`NTF-CFG-005`** | **Retry backoff schedule** (`NTF-FR-049`) | Provider-pressure tuning without redeploy | **Architecture Owner** | ⛔ **OWED — and genuinely PREMATURE** (§20.3a): **0** backoff schedule exists at any rank, the **V1 adapter implements none** (`services.dart` **L189-221** retries immediately), and the EA places *"Retry & Backoff"* at **V2** (**L1808**) |
| ⭐ **`NTF-CFG-007`** | **Transport / attempt-run deadline** — the bound on **one submitted delivery run**, after which the run is terminal (`NTF-FR-049`, `NTF-FR-042`) | ⭐⭐ **ONE CLOCK IS SUFFICIENT** (§20.3e): quiet-hours deferral and cancellation occur in the **pre-submission `queued`** state (`NTF-FR-014`), so a deferred notification is **not** a running job and no validity-horizon slot is required. ⚠ The v0.5 *"split this slot"* proposal is **WITHDRAWN as unnecessary** | **Architecture Owner** | ⛔ **OWED — one well-defined number** (`NTF-GAP-026`). ⚠ Ratify **jointly** with `NTF-CFG-004` | ⭐⭐ **Published because auditing the retry budget revealed the obligation was missing**, exactly as `FIL-CFG-015` was *"added at v0.2 because writing the configurable exposed that the obligation was missing"*. The `JobRuntime` contract **requires** a `deadline` on every submission (`job_runtime.dart` **L107-110**), so `BC-22` cannot submit work without one, and the retry bound is **unbounded in time** without it | **Architecture Owner** | ⛔ **OWED** — `NTF-GAP-026`. ⚠ It is the **missing companion** that makes `NTF-CFG-004`'s number underivable |
| **`NTF-CFG-006`** | **Deduplication window** (`NTF-FR-045`, `NTF-FR-049`) | Operational bound on idempotency-record retention | **Architecture Owner** | ⛔ **OWED** — `NTF-GAP-018`. ⚠⚠ **Invariant-constrained, not free**: see §20.4 |
...
> — `packages/liboora_contracts/lib/src/ports/job_runtime.dart` **L104-106**
...
job**; on expiry the runtime moves it to `JobState.failed`"* (`job_runtime.dart` **L107-110**).
...
tension. ⭐ **`DeliveryMessage` is a `BC-22`-owned aggregate** (BC Map **L205**), so §18's lifecycle is
...
| **Retry unit in the port contract** | ⭐⭐ **`retryBudget` is *"the **total** attempts permitted, **including the first**"*** | `job_runtime.dart` **L104-106** |
...
| ⭐ `NTF-GAP-026` | ⭐⭐ **No delivery job deadline exists** (`NTF-CFG-007`), yet the `JobRuntime` contract **requires** one on every submission (`job_runtime.dart` **L107-110**). ⇒ `NTF-CFG-004`'s attempt count is **unbounded in time** and its upper bound is **underivable** until this is set — the `FIL-CFG-015` (120 s) companion that let `PRD-017` derive **3** has **no analogue here** | **Architecture Owner** | `NTF-CFG-004`, `NTF-CFG-007`, `NTF-FR-042` |
...
Registers, **re-measured at v0.3** (`grep -o | sort -u`, not asserted): `NTF-FR-*` (**65**) ·
`NTF-BR-*` (**3**) · `NTF-INV-*` (**11**) · `NTF-XC-*` (**6**) · `NTF-AC-*` (**9**) ·
`NTF-GAP-*` (**27**) · `NTF-CFG-*` (**7**). ⚠ **Counts are provisional; the AC register is
