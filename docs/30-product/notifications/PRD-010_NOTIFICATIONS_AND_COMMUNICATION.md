...
| ⭐ **`NTF-CFG-007`** | **Delivery job deadline** — ⚠⚠ **UNDER-SPECIFIED BY ITS OWN AUTHOR; see §20.3d Disproof 1.** Framed at v0.5 as *"the whole-job bound"*, but `NTF-FR-039`'s *"defer, never drop"* cannot share a clock with a transport bound ⇒ this slot must be **split** into an **attempt/transport budget** and a **notification validity horizon** before any value is meaningful (`NTF-GAP-027`) (`NTF-FR-049`, `NTF-FR-042`, `NTF-FR-039`) | ⭐⭐ **Published because auditing the retry budget revealed the obligation was missing**, exactly as `FIL-CFG-015` was *"added at v0.2 because writing the configurable exposed that the obligation was missing"*. The `JobRuntime` contract **requires** a `deadline` on every submission (`job_runtime.dart` **L107-110**), so `BC-22` cannot submit work without one, and the retry bound is **unbounded in time** without it | **Architecture Owner** | ⛔ **OWED** — `NTF-GAP-026`. ⚠ It is the **missing companion** that makes `NTF-CFG-004`'s number underivable |
