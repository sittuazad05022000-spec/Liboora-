...
| **`NTF-FR-047`** | Only **transient** failures **MUST** be retried; a permanent failure **MUST** terminate as `failed` without further attempts. ⭐ Normative form made explicit at v0.14. ⚠ **Disclosed, not cured:** the V1 adapter's `on Object` catch (`services.dart` **L215**) does **not** distinguish the two — both yield `attempt_failed` — so this obligation is **specified but not yet enforced by the V1 runtime**; recorded as part of `NTF-GAP-029` (`NTF-RTD-001`). |
...
Registers, **re-measured at v0.14** (`grep -o | sort -u`, not asserted): `NTF-FR-*` (**67**) ·
...
| Version | Date | Change |
|---|---|---|
| **v0.14** | 2026-09-05 | ⭐⭐⭐ **SIX DEFECTS FOUND BY THE STAGE-4 REVIEWER'S OWN INDEPENDENT RE-MEASUREMENT, AND REPAIRED — the review was performed against the bytes, not against the previous pass's matrix.** ⚠⚠ **Two rows carried STALE text that contradicted closures this document had itself already made:** `NTF-FR-017` still read *"no number is proposed"* although `NTF-GAP-007` is **CLOSED** (ceiling 200 / 1–500 ratified, `ADR-0108` §7.1; rate limb withdrawn, `ADR-0110` §2), and `NTF-FR-040` still read *"no classification asserted"* although the per-entry register is **COMPLETE** at §20.6 (all 13 entries, `ADR-0109` §5). Both now state their obligation. ⚠ **Three rows were POINTERS, not obligations, and are restated:** `NTF-FR-049` (retry bound + no-`BC-22`-scheduling + no V1 backoff), `NTF-FR-054` (**MUST** resolve through `BC-25`; **MUST NOT** define resolution order — `[DERIVED]` from FROZEN `FIL-XC-009` and `ADR-0017` §2.5), and `NTF-FR-047` (normative form made explicit). ⭐ **`NTF-XC-004` restated to state an IMPOSSIBILITY** rather than a scope description, because Stage-4 check 2 requires exactly that (*"an exclusion is not a deferral"*) ⇒ **8 of 8** exclusions now state what must be impossible. ⭐⭐ **A runtime shortfall is DISCLOSED rather than smoothed over:** `NTF-FR-047`'s transient/permanent distinction is **not enforced** by the V1 adapter, whose `on Object` catch (`services.dart` **L215**) yields `attempt_failed` for both — the obligation is specified, not yet executable, and is folded into `NTF-GAP-029`. ⛔ **0 requirements added, retired or renumbered · 0 identifiers minted · 0 numbers invented · 0 gaps closed by this pass** — every repair restates an existing obligation more precisely or corrects text the document had already superseded. Registers re-measured: **FR 67 · BR 3 · INV 11 · XC 8 · AC 13 · GAP 29 · CFG 7 declared**, contiguous, 0 collisions. ⇒ ⭐⭐⭐ **All six Stage-4 checks and the L119 deferral gate now measure PASS against the v0.14 bytes**, which is what made conferral possible — see `PRD-010_STAGE4_CONFERRAL.md`. ⛔ `ADR-0107`/`0108`/`0109`/`0110` NOT reopened · BC Map untouched · 0 frozen PRDs · 0 Rank 1–2 · 0 code. |
