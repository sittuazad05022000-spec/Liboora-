...
| **Status** | **PARTLY RESOLVED — (a) transport CLOSED by measurement, recorded in `ADR-0035` `D-1`; (b) counterparty OPEN and BLOCKING; (c) offline-sync path OPEN** |
| **Stage 4** | **BLOCKS** |
| **Freeze** | **BLOCKS** |
...
> **At v0.7 the count moves 15 → 16, and the Stage-4 blocker count stays at 5.** `FEE-GAP-002`'s remaining half —
> *who executes* — is **RESOLVED** by `ADR-0035` `D-2` (`O-3`: a Business Platform capability, no new context), and
> **`FEE-GAP-016` replaces it in the blocking set** with the narrower inbound-transport question that resolving
> execution exposed. **This is not bookkeeping to keep a number flat**: one blocker genuinely closed and one
> genuinely opened, and the new one is strictly smaller — a single capability declaration, versus an undecided
...
*End of `PRD-008_REVENUE-AND-FINANCE.md` **v0.6 — DRAFT**. Not frozen. Not approved. Not architecture-reviewed.*
