...
### 39.2 Renewal protection — what the frozen rules already forbid (`FEE-GAP-013`)
...
| Is there an aggregate that can hold a seat without allocating it? | Frozen `PRD-007` **L8** — *"Aggregates owned: `SeatAllocation` · `SeatLayout`"*. `grep -rn "SeatHold\|SeatReservation" docs` → **0** | **No seat-hold aggregate exists.** A protected-but-unallocated seat has no owning model. Architecture Owner + `BC-04` |
...
> **allocation and display**, not about money or entitlement arithmetic — and neither can be *specified* until the
> owning aggregate (`SeatHold`, which does not exist) and `Q-01` are settled. **A voluntary leave gets the normal
...
*End of `PRD-008_REVENUE-AND-FINANCE.md` **v0.9 — DRAFT**. Not frozen. Not approved. Not architecture-reviewed.*
