<!--
  ...
  This file exists so that the exact state of PRD-021A Part A2 v0.3 can be
  ...
  Identifiers   : Every LCF-* identifier in this document is PROPOSED and
                  ...
                  it is a Governance Owner act (LCF-DEC-001).

  One disclosed discrepancy, preserved rather than edited
  ------------------------------------------------------
  Section 0 below describes this document as a "chat artefact only" and records
  "Repository : UNCHANGED. 0 files written". Those statements were TRUE when the
  content was authored, and the commit that creates this file falsifies the
  second one.

  They are preserved VERBATIM and are NOT corrected, because the instruction
  under which this file was persisted required the produced content to be
  preserved exactly and forbade silent correction. The discrepancy is therefore
  disclosed here, in a block that is clearly not part of the specification,
  rather than smoothed away inside it.
...
**v0.3 — DRAFT (chat artefact only)** · supersedes the v0.1 chat draft in its entirety
...
```
Status        : DRAFT — v0.3. Chat artefact. NOT written to the repository.
Freeze status : NOT FROZEN. NOT FINAL. NOT APPROVED. NOT BASELINED.
Lifecycle     : No stage advanced. Stage 7 freeze happens only after A1–A8 exist.
Repository    : UNCHANGED. 0 files written, 0 commits, 0 pushes.
...
⚠ **The `LCF-` prefix used below is proposed, not conferred.** Every repository PRD carries a registered prefix (`MM-`, `SM-`, `FIL-`, `TSF-`, `CNF-`, `ENT-`, `LMD-`, `LCM-`). Ratifying `LCF-` is a **Governance Owner** act — `LCF-DEC-001`.
...
`LCF-XC-012` | Ranking, relevance scoring, personalisation, recommendation | **A3** (planned) — `LCF-DEP-003` |
...
`LCF-XC-025` | Cross-library search, global content discovery and semantic/vector search over
community content are out of scope for this part.
...
`LCF-GAP-005` | A3 does not exist on disk. The ranking seam is specified but untestable until
it does.
...
`LCF-XC-028` | Anti-spam heuristics, reputation scoring and abuse detection are `BC-13`-owned
(EA L919 `Anti-Spam & Rate Limits (V1)`) and out of scope here.
...
`LCF-GAP-009` | Until `BC-15` has a §8 row, these aggregates are specified but unregistered.
...
`LCF-XC-029` | `Community Analytics` is **V3** (EA L973). This part specifies only what the
event stream makes available; it does not specify the analytics product.
...
--- | --- | ---
`LCF-FR-*` | 86 | `001`–`110` with the gaps arising from renumbering; no duplicates
`LCF-BR-*` | 13 | `001`–`013`
`LCF-INV-*` | 16 | `001`–`016`
`LCF-AUTH-*` | 12 | `001`–`012`
`LCF-RM-*` | 5 | `001`–`005`
`LCF-SEC-*` | 16 | `001`–`016`
`LCF-API-*` | 14 | 9 provided, 5 NOT PROVIDED
`LCF-EVT-*` | 6 | `001`–`006`, set closed
`LCF-NFR-*` | 12 | `001`–`012`
`LCF-AC-*` | 36 | **33 verifiable · 1 unwritable (`LCF-AC-034`) · 2 deferred (`LCF-AC-035`, `LCF-AC-036`)**
`LCF-XC-*` | 29 | includes `LCF-XC-024a` position and `LCF-XC-012a` position
`LCF-RSK-*` | 18 | `001`–`018`
`LCF-GAP-*` | 14 | includes the `011a` position
`LCF-ADR-*` | 6 | requirements only; **no ADR created**
`LCF-DEC-*` | 7 | includes `LCF-DEC-003a`
`LCF-DEP-*` | 5 | `001`–`005`

**Total: 295 identifiers across 16 registers.**
