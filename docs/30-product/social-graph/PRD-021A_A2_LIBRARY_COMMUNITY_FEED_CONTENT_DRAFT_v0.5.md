<!--
  ...
  v0.5 supersedes v0.4. v0.4 and v0.3 are retained on disk and in git history;
  neither is deleted nor overwritten.
...
⚠ **`LCF-BR-010` is currently UNSATISFIED.** §22 declares the field a `Closed set` but does
**not** enumerate its members. Now that reactions ship (`LCF-GAP-012` RESOLVED), this
obligation is live. Enumerating the set is a Product Owner act — **`LCF-GAP-014`**. Until it
is enumerated, no individual reaction kind can be referenced by this or any dependent part.
...
`kind` | Closed set — ⛔ **members not yet enumerated** | `LCF-BR-010`; not client-extensible. The set is required to be defined here but is not; enumerating it is a Product Owner act (**`LCF-GAP-014`**)
`createdAt` | Timestamp | —
...
`LCF-GAP-*` | 12 | `001`, `004`–`014` | `001`, `004`–`011` defined in §33.2; `012`–`014` defined in §34. `012` is ✅ RESOLVED at v0.5 but retained (a resolved gap is not retired, and `PRD-023` `CNF-FR-008` forbids reusing a position); `014` is new at v0.5. Positions `002`, `003` unused
...
**Total: 317 identifiers across 16 registers.** v0.5 adds exactly one identifier
(`LCF-GAP-014`) and removes none.
...
`LCF-GAP-014` | `LCF-BR-010` (§17) requires the reaction kind set to "be defined in this part's data model (§22)", but §22 declares only `Closed set` and never enumerates a member. Contrast `LCF-FR-027`, which enumerates the post-type set exactly | 🔴 **OPEN — new at v0.5.** The `LCF-GAP-012` SHIP decision makes this obligation live. Enumerating the set would be **inventing requirements**, which v0.5 has no authority to do. No individual reaction kind can be referenced until it is enumerated | **Product Owner**
