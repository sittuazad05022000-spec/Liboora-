...
Decision owner | ✅ **Architecture Owner — DECIDED 2026-08-31 (AO-3).** `LCF-ADR-002` is **ACCEPTED**; the `BC-15 → BC-14` edge is admitted as **`E-29`** (BC Map **§15.1**)

`LCF-FR-043` | ✅ **SATISFIED — 2026-08-31.** `LCF-ADR-002` is **ACCEPTED** by ruling **AO-3**, so this clause's condition is discharged and `NOTE`, `MEDIA` and `LINK` posts are **no longer blocked by the missing edge**. ⭐ **The clause is retained, not deleted** — it correctly records that only `TEXT` shipped for as long as the edge was absent, and resolution is not retirement. ⛔ **No duplicate media owner was created in `BC-15`**, exactly as AO-3 requires: `BC-14` remains the sole owner of attachments and `BC-15` merely references them across `E-29`. ⚠ **Blocked-by-edge is not the same as implemented** — the three post types are now *specifiable*, and A2 is still a DRAFT. *(Prior text retained verbatim: Until `LCF-ADR-002` is accepted, `NOTE`, `MEDIA` and `LINK` posts SHALL be)*
...
`LCF-API-004` | Retrieve next feed page | ✅ **NOW PROVIDABLE — 2026-08-31.** Architecture Owner ruling **AO-2** accepts **opaque cursor pagination**: the cursor is **opaque to clients** and SHALL NOT expose internal ordering fields, database keys, offsets or any implementation detail. Clients treat it as a bare token, pass it back unmodified, and MUST NOT parse, construct or reason about it. The convention is recorded in BC Map **§15.3** as `PG-1`…`PG-4`. ⭐ **The contract in §14 was already correct and is UNCHANGED** — only the wire shape was missing, which is exactly what `LCF-ADR-003` said. ⚠ **This does not itself write the endpoint**: the API is now *specifiable*, not implemented, and A2 remains a DRAFT |
...
`LCF-NFR-006` | ⚠ **CONDITIONAL / DEFERRED as of 2026-08-31 — see AO-5.** The feed projection SHALL be rebuildable from the event stream **where the platform's capability band provides an event stream to rebuild from**. ⛔⛔ **In the V2 band it does NOT.** Architecture Owner ruling **AO-5** holds that **V2 SHALL NOT pull Event Replay forward from V3**, and that A8 SHALL NOT invent new V2 infrastructure to satisfy this requirement. The ruling's own words: *"mark the requirement as conditional/deferred rather than pretending V2 provides Event Replay."* ⭐⭐ **This is the honest disposition and it is deliberately NOT a pass.** The alternatives were to (a) claim rebuildability the band cannot deliver, (b) silently pull V3 infrastructure into V2, or (c) delete the requirement. All three were refused: (a) is false, (b) is forbidden by AO-5 and by the standing V2-scope rule, and (c) would lose a requirement that becomes correct and testable the moment Event Replay lands. ⚠ **Therefore: UNVERIFIABLE in V2, and its acceptance criterion SHALL NOT be recorded as passing, waived or approximated in the V2 band.** It reactivates automatically when the capability arrives, with no re-ruling needed. *(Prior text retained verbatim: The feed projection SHALL be rebuildable from the event stream)*
...
`LCF-ADR-002` | Admit the `BC-15 → BC-14` reference edge to BC Map §7 | Architecture Owner | ✅ **ACCEPTED — 2026-08-31 (AO-3), recorded in `ADR-0083` §4 and BC Map §15.1 as `E-29`** (`BC-14 Media → BC-15`, mode `CF`, synchronous port, **V2**). ⛔ **The edge id is `E-29`, NOT `E-27`** — `E-27` was minted by `ADR-0032`, withdrawn by `ADR-0033`, and is **permanently vacant** under `PRD_LIFECYCLE.md` §5 rule 5 (*"Numbers are never reused, even after withdrawal"*). ⛔ **No duplicate media owner in `BC-15`.** ⚠⚠ **ONE ITEM IS DISCLOSED AS UNDETERMINED, not closed**: whether this edge needs Matrix rule `L2` same-rank cluster cover is **unmeasurable**, because `BC-14` returns **0 hits** in *both* the Dependency Matrix and `tool/module_dependencies.yaml`, so its rank is unrecorded — and **AO-7 enumerates only `BC-11`/`BC-12`/`BC-13`/`BC-15`**, so adding `BC-14` to the cluster would exceed the ruling. Tracked as **`GAP-BC14-RANK`**, Architecture Owner. `LCF-API-002` and all non-`TEXT` post types unblocked
`LCF-ADR-003` | Ratify a repository-wide pagination convention | Architecture Owner | ✅ **RESOLVED — 2026-08-31 by AO-2 and `ADR-0083` §4** (`Accepted`). **Opaque cursor pagination**, recorded as `PG-1`…`PG-4` in BC Map **§15.3**. ⭐ **The convention really was absent, not merely unfound** — the prior measurement `grep -rniE 'cursor' docs/10-architecture/*.md` returned **0 results**, and §15.3 is the first cursor definition in the architecture layer. `LCF-API-004` unblocked; `LCF-AC-036` unblocked **on this axis only** (`LCF-GAP-011` still blocks it)
...
`LCF-ADR-007` | ✅ **ACCEPTED — 2026-08-31 (AO-1), and BOTH required acts were performed, not just the first.** Admit the `BC-15 → BC-11` reference edge so that `LCF-FR-034`, `LCF-AUTH-012` and `LCF-FR-096` may consume `RateLimitCounter` and `BlockList`. **Act 1:** the edge row is admitted as **`E-28`** (`BC-11 Graph → BC-15`, mode `C/S`, synchronous port, **V2**) in BC Map **§15.1**. **Act 2:** the declared **Social cluster** is **EXTENDED** to `BC-11`, `BC-12`, `BC-13`, **`BC-15`** in Dependency Matrix **L90** and §12 **L652**, and in `tool/module_dependencies.yaml` (`contexts: [graph, messaging, safety, community]`, plus the `E-28` row). ⭐⭐ **This part's v0.8 warning that TWO acts were required — the edge row AND the cluster extension — was CORRECT, and AO-1 names both by name.** Performing only the first would have left a live rule-`L2` violation that the boundary checker would eventually surface. ⭐ **The cluster was EXTENDED, not replaced, and no THIRD rank-8 cluster was created** — `ADR-0011` (Matrix **L92–95**) warns that a third R8 cluster *"dissolves the Separate Ways boundary"*; `domain/social` stays at **rank 8**. ✅ **Verified:** `dart run tool/check_module_boundaries.dart` returns the **9 pre-existing `ADR-0012` violations and no new `domain/social` entry**, and both Rank 4 edits were strictly in place (**670 → 670 lines**; **712 → 712 lines**). *(Prior text retained verbatim: ⛔ **NEW at v0.8.** Admit the `BC-15 → BC-11` reference edge to BC Map §7, so that `LCF-FR-034`, `LCF-AUTH-012` and `LCF-FR-096` may consume `RateLimitCounter` and `BlockList`. ⚠ **Two acts are required, not one**: BC Map **L292** requires the edge row, and `LIBOORA_MODULE_DEPENDENCY_MATRIX.md` rule `L2` (**L49**) additionally requires the declared **Social cluster** (Matrix **L90** = `BC-11`/`12`/`13`) to be extended)*
...
4 | Attachments need a `BC-15 → BC-14` edge, which BC Map §7 does not record | BC Map L292 ("If an edge is not in this table, it does not exist and adding it requires an ADR"); L331 (`E-22` consumers = `BC-01, BC-10, BC-12, BC-14`); `PRD-017` `FIL-FR-006` L405, `FIL-FR-007` L410 | ✅ **RESOLVED 2026-08-31 (AO-3)** — one reference edge added as **`E-29`** in BC Map **§15.1**. ⛔ **`E-22` untouched, `PRD-017` untouched, `BC-14` remains the sole media owner** — exactly as this row proposed. ⚠ `GAP-BC14-RANK` discloses that the edge's rule-`L2` clearance is undetermined | ✅ **Architecture Owner — DECIDED** — `LCF-ADR-002`
...
`LCF-DEC-005` | Ratify a pagination convention (Conflict 7) | Architecture Owner | ✅ **CLOSED — 2026-08-31 by AO-2.** Opaque cursor; BC Map **§15.3** `PG-1`…`PG-4`. `LCF-API-004` is **no longer NOT PROVIDED** *(prior text retained verbatim: `LCF-API-004` stays NOT PROVIDED)*
