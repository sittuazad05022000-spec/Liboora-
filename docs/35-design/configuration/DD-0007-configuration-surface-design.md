...
|---|---|
| **Design Doc** | `DD-0007` |
| **Version** | ⭐ **v0.9** — ⛔⛔ **COVERAGE UNCHANGED AT 79 / 104**, recording `Accepted` **[`ADR-0158`](../../00-governance/adr/ADR-0158-security-owner-designation-and-phase-7-disposition-of-the-25-remaining-bc-25-parameters.md)**, ⛔ **which authorizes NOTHING — deliberately**. ⭐ **SECURITY PLATFORM designated "Security Owner"** *(security review/decision **only**; ⛔ **no permission authority**, ⛔ not a runtime role)*. ⭐⭐ **ALL 25 REMAINING PARAMETERS NOW CARRY A DOCUMENTED REASON AND A NAMED OWNER**. ⛔⛔ **104/104 was NOT forced.** ⚠️⚠️ **D3 REMAINS `PARTIAL`** *(prior: **v0.8** — coverage 64 → 79)* |
...
**79 of 104**, and ⛔ the other **25 remain `NOT YET AUTHORIZED`**.
...
| ⛔ **Not authorized** | ⚠️ **25** | `HELD` | ⭐⭐ **All 25 now carry a DOCUMENTED REASON and a NAMED OWNER** *(`ADR-0158` §3)* — ⛔ but **0** are authorized |
| | **104** | | |
...
`SEAT-CFG-*` subset. ⭐ **56 + 23 = 79 authorized; 79 + 25 = 104.**
...
| 2 | ⛔ **Authorization-semantic — attendance** | `ATT-CFG-022`, `023`, `024` | **3** | ⛔ §19.2 **L1980**/**L1983** make capabilities *"subject to"* these values. ⭐ `023` is held **with** `022` — ⛔ deciding them apart could permit corrections **and** set the window to **0** | **Authorization Owner** |
| 3 | ⛔ **Authorization-semantic — seat** | `SEAT-CFG-005`, `014` | **2** | ⛔ Both are **`C`** cells — *"permitted **only where the named configurable grants it**"*; ⭐ the parameters **are** the grant | **Authorization Owner** |
| 4 | ⛔ **Invitation security** | `ICFG-1`…`10` | **10** | ⛔ **No `READ` statement anywhere** · ⛔ **no `WRITE` role** *(only `INV-SEC-018`'s prohibition)* · ⚠ **ambiguous scope** · ⛔ **`PR-1`/`PR-2` = ZERO** in the source. ⭐⭐ A `PR-n` allocation was **considered and REFUSED** — see §6.5a | **PO** + **SECURITY PLATFORM** |
| 5 | ⛔ **Retention / DPDP** | `CFG-10`, `SCFG-4` | **2** | ⭐ **One question, held together** | **Privacy Owner** |
| 6 | ⛔ **Definition undecided** | `SMCFG-5`, `SEAT-CFG-017` | **2** | ⛔ `SM-GAP-2` and `SEAT-GAP-005` — ⚠⚠ the **definition**, not merely the authority, is open | **PO** *(+ **`BC-06` Owner**)* |
...
⭐⭐ **79 authorized + 25 held = 104.** ⛔ No parameter is unaccounted, and ⛔ **none
...
| ⛔ `NOT YET AUTHORIZED` | ⚠️ **25** | ⛔ **NO** |
| **Total** | **104** | ⭐ **79 of 104 = 76.0% authorization coverage** |

⛔⛔ **76.0% is published as measured.** ⛔ It is not rounded, not described as
"most of the important ones", and ⛔ the 25 are **not** presented as authorized
...
+ ⭐⭐ held `SCFG-2`/`SCFG-4` **2** = **25** *(⭐ `SCFG` **11 → 2** via `ADR-0155`; ⭐ `SMCFG` **7 → 1** via `ADR-0156`; ⭐⭐ `SEAT-CFG` **18 → 3** via `ADR-0157`)*.
...
| ⚠️ The other **25** parameters *(was **40** at v0.7 — ⭐ **15** `SEAT-CFG-*` left this row via `ADR-0157`)* | ⛔ No operation×role source | — | ⛔ Not designed | ⭐ **Future / V2** |
...
79 / 104** — ⛔ **25 parameters are NOT authorized and MUST NOT be prototyped**.
...
| Authorization coverage | ⚠️ **79 / 104 = 76.0%** — ⭐ **22** `ADR-0151` + **17** `ADR-0153` + **10** `ADR-0154` + **9** `ADR-0155` + **6** `ADR-0156` + ⭐⭐ **15** `ADR-0157` |
...
**49 of 104**. ⛔ The
other **25 remain `NOT YET AUTHORIZED`** — ⛔ no role, ⛔ no action class, ⛔ no
