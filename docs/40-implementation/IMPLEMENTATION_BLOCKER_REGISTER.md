...
|---|---|---|---|---|---|
| `BLK-01` | `TASK-D10` — remove demo and debug authentication surfaces | **P0** | Code (production) | **Every release** · Matrix §10.4 **gate 3** | ⚠ **OPEN — PARTIALLY DONE, and the prior text was FALSE.** ⭐ **`D10-1`, `D10-2`, `D10-3` are COMPLETE**, performed by commit **`2970993`** (*"TASK-D10 partial — close the OTP bypass and delete the seeded accounts panel"*). Measured at `1485994`: **`_seededAccounts` = 0** and the **`'SEEDED ACCOUNTS'` literal = 0** occurrences in `lib/`. ⛔ **`D10-4`…`D10-10` remain OPEN** — measured in `lib/`: `otpHint` **×6** (`app/shared/session.dart` L29/L34/L86/L132/L140/L155), `challengePeekEnabled` **×4**, `debugPeekChallenge` **×2**, `kDemoChildPhone` **×2**. ⚠⚠ **AND THEY MUST NOT BE REMOVED YET:** `IMPL-020` §1.3 measures that the peek *"**cannot be removed until a real delivery path exists**, or the app loses its only sign-in route"* — and ⭐ **a runtime selection (`ADR-0124`) is not a delivery path**; DLT registration and an implemented adapter are still absent. ⭐ **`D10-10`** (`subjectStudentId` for `AccessRole.parent`) is **separable and available now** — it *"requires a product decision"* (`TASK-D10` L144), owner **Product Owner**, and needs no delivery path. *(Prior status, FALSE since `2970993`: "⬜ **OPEN — not started**".)* |
...
**The repository cannot produce a releasable build.** `BLK-01` and `BLK-02` are both **P0** and both **open**,
and `BLK-01` depends on `BLK-02`, which depends on a multi-week external approval that has not been started.

⚠ **One word in the prior version of this paragraph was wrong and is corrected:** it said both were
*"unstarted"*. **`BLK-01` is not unstarted** — `D10-1`…`D10-3` were completed by commit **`2970993`**
(measured: `_seededAccounts` and the `'SEEDED ACCOUNTS'` literal are both **0** in `lib/`). ⭐ **The
conclusion is unchanged**, because the remaining steps `D10-4`…`D10-10` are the ones that cannot proceed:
`IMPL-020` §1.3 records that the peek *"cannot be removed until a real delivery path exists, or the app loses
its only sign-in route."* ⭐ **A runtime selection (`ADR-0124`) is not a delivery path.**

**Start `BLK-02`'s DLT registration first.** It is the only item whose duration the team cannot compress.

⭐ **One further item is separable and needs no delivery path: `D10-10`** — resolving `subjectStudentId` for
`AccessRole.parent`, which `TASK-D10` **L144** records as *"requires a product decision"* (owner: **Product
Owner**; option **A**, returning `null`, is the documented recommendation).

> *(Prior text, retained verbatim and correct until `2970993`:)* **The repository cannot produce a releasable
> build.** `BLK-01` and `BLK-02` are both P0, both open, both unstarted, and `BLK-01` depends on `BLK-02`,
> which depends on a multi-week external approval that has not been started.
...
| **v1.15** | 2026-09-08 | ⭐ **`BLK-01`'s row corrected: it said "not started" and that was FALSE.** A re-audit of all six open items found **one** correctable statement and **zero** newly-closable blockers. ⭐ **`D10-1`/`D10-2`/`D10-3` were completed by commit `2970993`** — measured at `1485994`: `_seededAccounts` **= 0** and the `'SEEDED ACCOUNTS'` literal **= 0** in `lib/`. ⛔ **`BLK-01` stays OPEN**: `D10-4`…`D10-10` are still present (`otpHint` ×6, `challengePeekEnabled` ×4, `debugPeekChallenge` ×2, `kDemoChildPhone` ×2) and ⚠⚠ **they must NOT be removed**, because `IMPL-020` §1.3 records that the peek *"cannot be removed until a real delivery path exists, or the app loses its only sign-in route"* — ⭐ **a runtime selection is not a delivery path**. ⭐ **`D10-10` is flagged as separable and available now** (Product Owner decision; no delivery path required). ⭐⭐ **EVERY OTHER GATE WAS RE-MEASURED AND ALL REMAIN CLOSED — nothing was force-closed:** **`BLK-02`** — **0** SMS providers named outside candidate/negative phrasing (the single remaining `MSG91` mention is `IMPL-020` L159 stating it *"unblocks **nothing**"*), and DLT stays **External** under Rank-1 `MP-DEP-03`; ⛔ `ADR-0124` **not reopened**. **`BLK-03`** — prerequisites **A1**/**A2** measured absent (**0** index declarations in `lib/`/`packages/`, **0** table/index inventory keys in `tool/`) and **`ADR-0125` itself records "Implementation authorised: No"**, so ⛔ **nothing was implemented and `ADR-0125` was followed exactly**. **`BLK-04`** — re-verified as **plan-only**: `BRANCH_PROTECTION_STATUS.md` L35/L36 record **HTTP 403** on *both* the branch-protection and rulesets APIs with *"Upgrade to GitHub Pro or make this repository public"*, so it stays **external/commercial**. **Item 9b** — `ATT-GAP-006` still **🔴 OPEN** (⛔ Security Platform design not invented). **Item 7a** — `Q-04` still open (⛔ no retention period invented). ⭐ §13.5 acts **4** and **5** remain the only outstanding required acts, both awaiting non-repository authority. ⛔ **0** ADRs created (`ADR-0126` left free); **0** code, config, dependency, checker, manifest or frozen-PRD changes; `ADR-0119`…`ADR-0125` **byte-unchanged**; §7 rules 1–4 all unmet, **`BASELINE-2026-09-03-A` stands**. |
