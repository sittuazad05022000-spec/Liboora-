...
| **Date** | 2026-08-02 · extended 2026-08-03 · **extended and §2C corrected 2026-08-04** · **§2D added 2026-08-04** · **§2E added 2026-08-04** · **§2F added 2026-08-04** · **§2G added 2026-08-15** · **§2H added 2026-08-15** · **§2I added 2026-08-17** · **§2J added 2026-08-19** · **§2K added 2026-08-19** · **§2L added 2026-08-20** · **§2M added 2026-08-20** |
...
| Prefix | Meaning | Count | Range | Source |
|---|---|---|---|---|
| `FIL-FR-*` | Functional requirements — architectural position, the video/audio V1 line, the two frozen consumers, the consumed permission model, upload, the `FileRef` contract, metadata and ownership, read and access, tenant isolation on every surface, lifecycle operations, derivatives and thumbnails, failure/retry/idempotency, progress, extensibility, and shared-object access | **82** | `001`–`082` | `PRD-017` §2.1, §2.4, §2.5, §3.2, §4.1–§4.11 |
| `FIL-BR-*` | Business rules — the permission model is consumed and never defined, plus the fifteen rules governing bytes, ownership, isolation, retention and sharing | **17** | `001`–`017` | `PRD-017` §3.2, §5 |
| `FIL-INV-*` | Invariants — required properties of the stored-object record | **11** | `001`–`011` | `PRD-017` §6.2 |
...
| `FIL-XC-*` | Exclusions — what **MUST be impossible**: owning the permission model, minting authorisation decisions, defining conversations or messages, moderation, the friendship graph, virus-scanner implementation, transcoding, CDN policy, reference counting, de-duplication, tenant lifecycle, event publication, and nine more | **22** | `001`–`022` | `PRD-017` §2.3 |
| **`FIL-AC-*`** | **Acceptance criteria** | **78** | `001`–`078` | **`PRD-017` §14.1–§14.4** |
| `FIL-CFG-*` | Configurables — size ceilings, MIME allow-lists, scan timeouts, retention windows, derivative dimensions, share-grant limits. **Each carries a default and a bounded range**, the Stage 4 rule that *"unbounded configuration is a specification hole"* | **9** | `001`–`009` | `PRD-017` §8.5 |
| `FIL-GAP-*` | Open gaps — no `BC-29` aggregate in BC Map §8, no `platform/services` manifest block, **the `E-22` consumer list omitting `BC-12` (`FIL-GAP-012`)**, no RLS architecture document, the missing architecture tests, and eight more | **13** | `001`–`013` | `PRD-017` §16 |
| **Total** | | **232** | | **132 obligation-bearing + 78 `FIL-AC-*` + 9 `FIL-CFG-*` + 13 `FIL-GAP-*`; 0 retired** |
...
| Measure | Value |
|---|---|
| Obligation-bearing identifiers (Class A) | **132** (82 `FIL-FR-*` + 17 `FIL-BR-*` + 11 `FIL-INV-*` + 0 `FIL-EVT-*` + 22 `FIL-XC-*`) |
| Obligations carrying a `FIL-AC-*` | **94** |
