...
| **Date** | 2026-08-02 · extended 2026-08-03 · **extended and §2C corrected 2026-08-04** · **§2D added 2026-08-04** · **§2E added 2026-08-04** · **§2F added 2026-08-04** · **§2G added 2026-08-15** · **§2H added 2026-08-15** |
...
Registers the `ITG-*` prefix so that **56 identifiers in a Rank-3-registered document are not registered nowhere** —
the exact condition `PRD_LIFECYCLE.md` Stage 5 exists to prevent, and the one §2G was added to fix for `PRD-008`.

⛔ **This section does NOT claim Stage 5 for `PRD-019`, and the distinction is the point.** Stage 5 requires
registration **and** mechanically verified traceability. `PRD-019`'s `ITG-AC-*` register is **empty**, so coverage is
**0/54** and no obligation is *verified* in the sense `PRD-008` §0.2 requires (*"prose never counts"*).
**Registration is performed; the stage is withheld.** Recording it the other way round would produce a green gate on
a document with no acceptance criteria at all.
...
| `ITG-GAP-*` | Open questions | **2** | `001`–`002` | `PRD-019` §7 |
| `ITG-AC-*` | Acceptance criteria | **0** | *not published* | ⛔ **none authored** — `Q-B31-AC` |
...
| `ITG-` **cited** outside `PRD-019` | Governance citations only — `ADR-0043`, `ADR-0044`, `ADR-0045`, `ADR-0046`, `ADR-0047` and this section. **A citation is not a collision** (the §2C.1 principle) |
...
| Not done | Why |
|---|---|
| **Claim Stage 5 for `PRD-019`** | `ITG-AC-*` is empty; coverage **0/54**. See the callout above |
| **Publish a coverage percentage** | It would be **0%**, and printing a computed 0% as though it were a result would dress a shortfall as a metric. It is named instead |
...
| **Add a gate script for `ITG-*`** | No file under `tool/` is created or modified by this pass. `prd008_stage5.py` and `prd008_traceability.py` are **byte-unchanged**; a `PRD-019` gate is owed when `ITG-AC-*` exists, and is recorded as owed rather than stubbed |
