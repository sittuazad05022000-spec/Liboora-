...
>
> **Why archived: this report is resolved. Blocker A was answered by `AR-5` (now `ADR-0006`) and Blocker B by `AR-6` (now `ADR-0004`). Its status line "AWAITING RULING — IMPLEMENTATION HALTED" is stale; the rulings were made. References to chapters with "0 body characters" describe the archived empty slots.**
>
