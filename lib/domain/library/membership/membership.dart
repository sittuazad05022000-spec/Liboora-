...
/// `IMPL-431` — activation, wired to `MM-EVT-002`.
///
/// `MM-FR-054` requires activation to emit `MM-EVT-002`. That event is
/// emittable — the requirement notes BC Map §9 does not list it because §9 is
/// the *seed* of the Event Catalog rather than a closed set for `BC-02`, and
/// records `MM-GAP-007` as **closed**. This is the difference between it and
/// `MM-EVT-004`, whose omission is still open as `MM-GAP-007a`.
///
/// Idempotent on the payment reference, which `§15` row 4 requires: *"No
/// second activation, no second `MM-EVT-002`."* The status machine already
/// refuses a second transition, but a duplicate callback must be a *success*
/// returning the original, not an error — a payment gateway retrying is not
/// making a mistake.
final class ActivateMembership {
  ...
  /// Activates [membershipId] on today's business date.
  ///
  /// [paymentReference] is the idempotency anchor: §15 row 4 keys duplicate
  /// suppression on the payment reference, not on a client-chosen key, because
  /// the duplicate here is generated by the gateway rather than by the user.
  ///
  /// [manuallyAuthorised] covers `MM-FR-052`'s third alternative — `MM-PO-004`
  /// manual activation. It is a separate parameter rather than a magic
  /// reference value so the permission check stays visible.
  Future<Membership> call({
    required AccessRole actorRole,
    required String membershipId,
    required String paymentReference,
    bool manuallyAuthorised = false,
  }) async {
    final key = IdempotencyKey('activate::$paymentReference');
    final replay = idempotency.recall(key);
    if (replay != null) return replay;

    pdp.require(actorRole, Permission.createMembership);

    final m = repo.byId(membershipId);
    if (m == null) {
      // MM-NFR-005: an id from another tenant is NOT FOUND, never forbidden.
      throw DomainError(
        DomainErrorCode.notFound,
        'No membership with that id exists in this tenant.',
        context: {'membershipId': membershipId, 'field': 'membershipId'},
      );
    }

    // MM-FR-052's calendar gate. activateOn applies both the transition table
    // and the term window; the business date comes from the TENANT's
    // timezone (MM-FR-061), never the server's.
    final today = calendar.businessDateAt(clock.now());
    m.activateOn(today, by: tenant.actorId);
    repo.save(m);
    ...
    events.enqueue([
      DomainEvent(
        ...
        payload: {
          ...
          'planId': m.planId,
          'status': m.status.name,
          'activatedAt': clock.now().toIso8601String(),
          // MM-EVT-002's declared payload names activationReason.
          'activationReason': manuallyAuthorised
    ...
    idempotency.remember(key, m);
    await events.drain();
    return m;
  }
}

/// `IMPL-431` — void before activation, wired to `MM-EVT-007`.
///
/// `MM-FR-078`: permitted, audited with a reason, and emits `MM-EVT-007`.
/// `MM-FR-079` is the boundary — this is NOT "cancel membership", it triggers
/// no refund, and it must never be extended to an `Active` membership. The
/// aggregate enforces the status guard; this wires the fact.
final class VoidMembership {
  VoidMembership({
    required this.repo,
    required this.idempotency,
    required this.events,
  ...
  Future<Membership> call({
    ...
    m.voidBeforeActivation(reason: reason, by: tenant.actorId);
    repo.save(m);
    ...
    events.enqueue([
      DomainEvent(
        ...
        payload: {
          ...
          'planId': m.planId,
          'status': m.status.name,
          // MM-EVT-007's declared payload: reason, voidedBy.
          'reason': m.voidReason,
          'voidedBy': m.voidedBy,
        },
      ),
...
final class ExpireDueMemberships {
  ...
  ///
  /// This reports the due set without side effects. [notifyExpiringSoon]
  /// is what emits, because `MM-FR-108`'s *"exactly once per membership per
  /// threshold crossing"* requires a record of what has already been
  /// notified.
  ...
  /// `IMPL-431`/`MM-FR-108` — emits `MM-EVT-006` **exactly once per
  /// membership per threshold crossing**.
  ///
  /// The dedup anchor is `(membershipId, endDate)`, not `(membershipId,
  /// today)`. Keying on the day would re-emit tomorrow, and keying on the
  /// membership alone would never re-emit after a renewal moved `endDate` —
  /// but a renewal produces a NEW membership with a new id, and this
  /// membership's own `endDate` is immutable under `MM-FR-098`, so the pair
  /// identifies the crossing exactly.
  ///
  /// `MM-EVT-006` is *"a fact, not a command"* (§12). This publishes that a
  /// term is nearly over; whether anyone is told is `BC-22`'s decision, and
  /// `MM-BR-019` keeps Notification out of this domain entirely.
  ///
  /// Returns the memberships it notified on this run.
  Future<List<Membership>> notifyExpiringSoon() async {
    final due = expiringSoon();
    final notified = <Membership>[];

    for (final m in due) {
      final crossing = '${m.id}::${_isoDay(m.endDate)}';
      if (_notifiedCrossings.contains(crossing)) continue;
      _notifiedCrossings.add(crossing);
      notified.add(m);

      events.enqueue([
        DomainEvent(
          eventId: ids.next('evt'),
          eventType: MembershipEventType.expiringSoon.wireName,
          tenantId: tenant.tenantId,
          aggregateId: m.id,
          occurredAt: clock.now(),
          actorId: tenant.actorId,
          ...
          payload: {
            ...
            'studentRecordId': m.studentRecordId.value,
            'planId': m.planId,
            // MM-EVT-006's declared payload: endDate, daysRemaining.
            'endDate': m.endDate.toIso8601String(),
            'daysRemaining': m.remainingDaysFrom(clock.now()),
          },
        ),
      ]);
    }

    if (notified.isNotEmpty) await events.drain();
    return notified;
  }

  /// `MM-FR-108`'s "exactly once" ledger, keyed `membershipId::endDate`.
  final Set<String> _notifiedCrossings = {};
